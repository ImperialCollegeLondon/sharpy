...
class AerogridPlot(BaseSolver):
        ...
        for i_surf in range(aero_tstep.n_surf):
            ...
            for i_n in range(dims[1] + 1):
                for i_m in range(dims[0] + 1):
                    ...
                    if self.settings["include_forward_motion"]:
                        coords[counter, 0] -= (
                            self.settings["dt"] * self.ts * self.settings["u_inf"]
                        )
            ...
            for i_n in range(dims[1] + 1):
                ...
                for i_m in range(dims[0] + 1):
                    ...
                    try:
                        point_unsteady_cf[node_counter, :] = aero_tstep.dynamic_forces[
                            i_surf
                        ][0:3, i_m, i_n]
                    except AttributeError:
                        pass
                    try:
                        zeta_dot[node_counter, :] = aero_tstep.zeta_dot[i_surf][
                            0:3, i_m, i_n
                        ]
                    except AttributeError:
                        pass
                    ...
                    if self.settings["include_incidence_angle"]:
                        incidence_angle[counter] = aero_tstep.postproc_cell[
                            "incidence_angle"
                        ][i_surf][i_m, i_n]
    ...
    def plot_wake(self):
        for i_surf in range(self.data.aero.timestep_info[self.ts].n_surf):
            ...
            dims_star = (
                self.data.aero.timestep_info[self.ts].dimensions_star[i_surf, :].copy()
            )
            ...
            for i_n in range(dims_star[1] + 1):
                for i_m in range(dims_star[0] + 1):
                    ...
                    coords[counter, :] = self.data.aero.timestep_info[
                        self.ts
                    ].zeta_star[i_surf][:, i_m, i_n]
                    if self.settings["include_rbm"]:
                        coords[counter, :] += self.data.structure.timestep_info[
                            self.ts
                        ].for_pos[0:3]
                    if self.settings["include_forward_motion"]:
                        coords[counter, 0] -= (
