...
class Cleanup(BaseSolver):
    ...
    settings_description[
        "clean_structure"
    ] = "Clean-up :class:`~sharpy.utils.datastructures.StructTimeStepInfo`"

    settings_types["clean_aero"] = "bool"
    settings_default["clean_aero"] = True
    settings_description[
        "clean_aero"
    ] = "Clean-up :class:`~sharpy.utils.datastructures.AeroTimeStepInfo`"
    ...
    def run(self, **kwargs):
        ...
        if self.settings["clean_structure"]:
            self.clean(
                self.data.structure.timestep_info, self.settings["remaining_steps"]
            )
