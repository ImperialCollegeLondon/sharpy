...
class FrequencyResponse(solver_interface.BaseSolver):
    ...
    settings_description[
        "frequency_unit"
    ] = "Units of frequency, ``w`` for rad/s, ``k`` reduced."
    settings_options["frequency_unit"] = ["w", "k"]

    settings_types["frequency_scaling"] = "dict"
    settings_default["frequency_scaling"] = {}
    settings_description["frequency_scaling"] = (
        "Dictionary containing the frequency scaling factors, if the "
    ...
    settings_description[
        "frequency_spacing"
    ] = "Compute the frequency response in a ``linear`` or ``log`` grid."
    ...
    settings_description[
        "quick_plot"
    ] = "Produce array of ``.png`` plots showing response. Requires matplotlib."
    ...
    __doc__ += scaling_table.generate(
        scaling_types,
        scaling_default,
        scaling_description,
    ...
    def initialise(self, data, custom_settings=None, caller=None, restart=False):
        ...
        if self.settings["frequency_unit"] == "k":
            ...
            if self.data.linear.linear_system.uvlm.scaled:
                ...
                settings_utils.to_custom_types(
                    scaling, self.scaling_types, self.scaling_default, no_ctype=True
                )
        ...
        if self.settings["frequency_spacing"] == "linear":
            ...
            raise NotImplementedError(
                "Unrecognised frequency spacing setting %s"
                % self.settings["frequency_spacing"]
            )
    ...
    def run(self, **kwargs):
        ...
        if ss is None:
            ss_list = [
                find_target_system(self.data, system_name)
            ...
            raise TypeError(
                "StateSpace input must be either a libss.StateSpace instance or a list"
                " of libss.StateSpace"
            )
        ...
        for ith, system in enumerate(ss_list):
            ...
            if ss is None:
                try:
                    ...
                    if self.print_info:
                        cout.cout_wrap(
                            "\tComputing frequency response for %s system"
                            % system_name,
                            1,
                        )
                except IndexError:
                ...
                system_name = (
                    None  # For the case where the state-space is parsed in run().
                )
            ...
            if self.settings["compute_hinf"]:
                ...
                try:
                    hinf = frequencyutils.h_infinity_norm(
                        system, iter_max=50, print_info=self.settings["print_info"]
                    )
            ...
            self.save_freq_resp(
                self.wv * self.w_to_k, y_freq_fom, system_name=system_name, hinf=hinf
            )
    ...
    def save_freq_resp(self, wv, Yfreq, system_name=None, hinf=None):
        ...
        with open(self.folder + "/freqdata_readme.txt", "w") as outfile:
            ...
            outfile.write(
                "The units of frequency are rad/s\nThe frequency"
        ...
        if self.print_info:
            cout.cout_wrap(
                "Saved .h5 file to %s with frequency response data" % h5filename
            )
            ...
            for mj in range(m):
                for pj in range(p):
                    ...
                    if y_freq_fom is not None:
                        ax1[0].plot(
                            self.wv * self.w_to_k,
                            20 * np.log10(np.abs(y_freq_fom[pj, mj, :])),
                            color="C0",
                        )
                        ax1[1].plot(
                            self.wv * self.w_to_k,
                            np.angle(y_freq_fom[pj, mj, :]),
                            "-",
                            color="C0",
                        )
                    if self.scaled:
