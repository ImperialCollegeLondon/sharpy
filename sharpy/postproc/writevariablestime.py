...
class WriteVariablesTime(BaseSolver):
    ...
    settings_description["FoR_variables"] = (
        "Variables of :class:`~sharpy.utils.datastructures.StructTimeStepInfo`"
    ...
    settings_description["structure_variables"] = (
        "Variables of :class:`~sharpy.utils.datastructures.StructTimeStepInfo`"
        " associated to the frame of reference to be writen"
    )
    ...
    settings_description["aero_panels_variables"] = (
        "Variables of :class:`~sharpy.utils.datastructures.AeroTimeStepInfo` associated"
        " to panels to be writen"
    )
    ...
    settings_description["aero_nodes_variables"] = (
        "Variables of :class:`~sharpy.utils.datastructures.AeroTimeStepInfo` associated"
        " to nodes to be writen"
    )
    ...
    settings_default["vel_field_points"] = np.array([0.0, 0.0, 0.0])
    settings_description[
        "vel_field_points"
    ...
    def initialise(self, data, custom_settings=None, caller=None, restart=False):
        ...
        if not (
            (
                len(self.settings["aero_panels_isurf"])
                == len(self.settings["aero_panels_im"])
            )
            and (
                len(self.settings["aero_panels_isurf"])
                == len(self.settings["aero_panels_in"])
            )
        ):
            raise RuntimeError("aero_panels should be defined as [i_surf,i_m,i_n]")
        if not (
            (
                len(self.settings["aero_nodes_isurf"])
                == len(self.settings["aero_nodes_im"])
            )
            and (
                len(self.settings["aero_nodes_isurf"])
                == len(self.settings["aero_nodes_in"])
            )
        ):
        ...
        if len(self.settings["vel_field_variables"]) > 0:
            if not (len(self.settings["vel_field_points"]) % 3 == 0):
                raise RuntimeError(
                    "Number of entries in ``vel_field_points`` has to be a multiple"
                    " of 3"
                )
            else:
                ...
                for ipoint in range(self.n_vel_field_points):
                    self.vel_field_points[0][:, ipoint, 0] = self.settings[
                        "vel_field_points"
                    ][ipoint * 3 : (ipoint + 1) * 3]
        ...
        for ivariable in range(len(self.settings["FoR_variables"])):
            for ifor in range(len(self.settings["FoR_number"])):
                filename = (
                    self.folder
                    + "FoR_"
                    + "%02d" % self.settings["FoR_number"][ifor]
                    + "_"
                    + self.settings["FoR_variables"][ivariable]
                    + ".dat"
                )
        ...
        for ivariable in range(len(self.settings["structure_variables"])):
            for inode in range(len(self.settings["structure_nodes"])):
                ...
                filename = (
                    self.folder
                    + "struct_"
                    + self.settings["structure_variables"][ivariable]
                    + "_node"
                    + str(node)
                    + ".dat"
                )
        ...
        for ivariable in range(len(self.settings["aero_panels_variables"])):
            for ipanel in range(len(self.settings["aero_panels_isurf"])):
                ...
                filename = (
                    self.folder
                    + "aero_"
                    + self.settings["aero_panels_variables"][ivariable]
                    + "_panel"
                    + "_isurf"
                    + str(i_surf)
                    + "_im"
        ...
        for ivariable in range(len(self.settings["aero_nodes_variables"])):
            for inode in range(len(self.settings["aero_nodes_isurf"])):
                ...
                filename = (
                    self.folder
                    + "aero_"
                    + self.settings["aero_nodes_variables"][ivariable]
                    + "_node"
                    + "_isurf"
                    + str(i_surf)
                    + "_im"
        ...
        for ivariable in range(len(self.settings["vel_field_variables"])):
            for ipoint in range(self.n_vel_field_points):
                filename = (
                    self.folder
                    + "vel_field_"
                    + self.settings["vel_field_variables"][ivariable]
                    + "_point"
                    + str(ipoint)
                    + ".dat"
                )
                ...
                if not os.path.isfile(filename):
                    fid = open(filename, "w")
                    fid.write(
                        "#t[s]%suext_x[m/s]%suext_y[m/s]%suext_z[m/s]\n"
        ...
        self.caller = caller
        if (not self.caller is None) and (
            not len(self.settings["vel_field_variables"]) == 0
        ):
        ...
        for ivariable in range(len(self.settings["FoR_variables"])):
            ...
            for ifor in range(len(self.settings["FoR_number"])):
                ...
                with open(filename, "a") as fid:
                    var = np.atleast_2d(
                        getattr(tstep, self.settings["FoR_variables"][ivariable])
                    )
                    rows, cols = var.shape
                    if (cols == 1) and (rows == 1):
                        self.write_value_to_file(
                            fid, self.data.ts, var, self.settings["delimiter"]
                        )
                    elif (cols > 1) and (rows == 1):
                        self.write_nparray_to_file(
                            fid, self.data.ts, var, self.settings["delimiter"]
                        )
                    elif (cols == 1) and (rows >= 1):
                        self.write_value_to_file(
                            fid, self.data.ts, var[ifor], self.settings["delimiter"]
                        )
                    else:
                        self.write_nparray_to_file(
                            fid, self.data.ts, var[ifor, :], self.settings["delimiter"]
                ...
                for inode in range(len(self.settings["structure_nodes"])):
                    ...
                    with open(filename, "a") as fid:
                        if num_indices == 2:
                            ...
                            ielem, inode_in_elem = self.data.structure.node_master_elem[
                                node
                            ]
                            self.write_nparray_to_file(
                                fid,
                                self.data.ts,
                                var[ielem, inode_in_elem, :],
                                self.settings["delimiter"],
                            )
