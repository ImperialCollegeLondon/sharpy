...
class BeamPlot(BaseSolver):
    ...
    def write_beam(self, it):
        it_filename = self.filename + "%06u" % it + ".vtu"
        ...
        for i_node in range(num_nodes):
            ...
            forces_constraints_nodes[i_node, :] = np.dot(
                aero2inertial, np.dot(cab, tstep.forces_constraints_nodes[i_node, 0:3])
            )
            moments_constraints_nodes[i_node, :] = np.dot(
                aero2inertial, np.dot(cab, tstep.forces_constraints_nodes[i_node, 3:6])
            )

            if with_gravity:
                gravity_forces_g[i_node, 0:3] = np.dot(
                    aero2inertial, gravity_forces[i_node, 0:3]
                )
                gravity_forces_g[i_node, 3:6] = np.dot(
                    aero2inertial, gravity_forces[i_node, 3:6]
                )
            ...
            for k in postproc_cell_6vector:
                for i in range(0, 2):
                    ug.cell_data.add_array(
                        tstep.postproc_cell[k][:, 3 * i : 3 * (i + 1)]
                    )
        ...
        if self.settings["include_applied_moments"]:
            ...
            ug.point_data.get_array(
                point_vector_counter
            ).name = "moments_constraints_nodes"
            if with_gravity:
                point_vector_counter += 1
        ...
        if with_postproc_node:
            ...
            for k in postproc_node_6vector:
                for i in range(0, 2):
                    point_vector_counter += 1
                    ug.point_data.add_array(
                        tstep.postproc_node[k][:, 3 * i : 3 * (i + 1)]
                    )
                    ug.point_data.get_array(point_vector_counter).name = (
                        k + "_" + str(i) + "_point"
                    )
    ...
    def write_for(self, it):
        it_filename = self.filename_for + "%06u" % it
        ...
        for ibody in range(self.data.structure.num_bodies):
            FoR_coords[ibody, :] = (
                self.data.structure.timestep_info[it].mb_FoR_pos[ibody, 0:3] - offset
            )

        for ibody in range(self.data.structure.num_bodies):
            forces_constraints_FoR[ibody, :] = np.dot(
                aero2inertial,
                self.data.structure.timestep_info[it].forces_constraints_FoR[
                    ibody, 0:3
                ],
            )
            moments_constraints_FoR[ibody, :] = np.dot(
                aero2inertial,
                self.data.structure.timestep_info[it].forces_constraints_FoR[
                    ibody, 3:6
                ],
        ...
        FoRmesh.point_data.get_array(
            for_vector_counter
        ).name = "moments_constraints_FoR"
