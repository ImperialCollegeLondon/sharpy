...
class PlotFlowField(BaseSolver):
    ...
    def initialise(self, data, custom_settings=None, caller=None, restart=False):
        ...
        self.postproc_grid_generator.initialise(
            self.settings["postproc_grid_input"], restart=restart
        )
        self.caller = caller
    ...
    def output_velocity_field(self, ts):
        ...
        if self.settings["include_induced"]:
            ...
            for iz in range(nz):
                for ix in range(nx):
                    for iy in range(ny):
                        ipoint += 1
                        target_triads[ipoint, :] = grid[iz][:, ix, iy].astype(
                            dtype=ct.c_double, order="F", copy=True
                        )

            u_ind_points = uvlmlib.uvlm_calculate_total_induced_velocity_at_points(
                self.data.aero.timestep_info[ts],
                target_triads,
            ...
            vtk_info.point_data.add_array(
                u_ind.reshape((-1, u_ind.shape[-1]), order="F")
            )  # Reshape the array except from the last dimension
        ...
        if self.settings["include_external"]:
            ...
            # Write the data
            vtk_info.point_data.add_array(
                u_ext_out.reshape((-1, u_ext_out.shape[-1]), order="F")
            )  # Reshape the array except from the last dimension
