...
class AeroForcesCalculator(BaseSolver):
        ...
        if self.settings["screen_output"]:
            if self.settings["coefficients"]:
                self.table = cout.TablePrinter(
                    7, field_length=12, field_types=["g"] + 6 * ["f"]
                )
                ...
                self.table = cout.TablePrinter(
                    7, field_length=12, field_types=["g"] + 6 * ["e"]
                )
                self.table.print_header(
    ...
    def calculate_forces(self, ts):
        ...
        for i_surf in range(n_surf):
            ...
            self.data.aero.timestep_info[ts].inertial_steady_forces[
                i_surf, 0:3
            ] = total_steady_force
            self.data.aero.timestep_info[ts].inertial_unsteady_forces[
                i_surf, 0:3
            ] = total_unsteady_force
            self.data.aero.timestep_info[ts].body_steady_forces[i_surf, 0:3] = np.dot(
                rot.T, total_steady_force
            )
            self.data.aero.timestep_info[ts].body_unsteady_forces[i_surf, 0:3] = np.dot(
                rot.T, total_unsteady_force
            )

        # Forces expressed in the beam degrees of freedom
        try:
            steady_forces_b = self.data.structure.timestep_info[ts].postproc_node[
                "aero_steady_forces"
            ]
        except KeyError:
        ...
        try:
            unsteady_forces_b = self.data.structure.timestep_info[ts].postproc_node[
                "aero_unsteady_forces"
            ]
        except KeyError:
            unsteady_forces_b = self.map_forces_beam_dof(ts, unsteady_force)

        steady_forces_a = self.data.structure.timestep_info[ts].nodal_b_for_2_a_for(
            steady_forces_b, self.data.structure
        )

        unsteady_forces_a = self.data.structure.timestep_info[ts].nodal_b_for_2_a_for(
            unsteady_forces_b, self.data.structure
        )

        # Express total forces in A frame
        moment_reference_location = np.array([0.0, 0.0, 0.0])
        self.data.aero.timestep_info[
            ts
        ].total_steady_body_forces = mapping.total_forces_moments(
            steady_forces_a,
        ...
        self.data.aero.timestep_info[
            ts
        ].total_unsteady_body_forces = mapping.total_forces_moments(
            unsteady_forces_a,
            self.data.structure.timestep_info[ts].pos,
            ref_pos=moment_reference_location,
        )
        ...
        self.data.aero.timestep_info[ts].total_steady_inertial_forces = np.block(
            [[rot, np.zeros((3, 3))], [np.zeros((3, 3)), rot]]
        ).dot(self.data.aero.timestep_info[ts].total_steady_body_forces)

        self.data.aero.timestep_info[ts].total_unsteady_inertial_forces = np.block(
            [[rot, np.zeros((3, 3))], [np.zeros((3, 3)), rot]]
        ).dot(self.data.aero.timestep_info[ts].total_unsteady_body_forces)
    ...
    def map_forces_beam_dof(self, ts, force):
        ...
        aero_forces_beam_dof = mapping.aero2struct_force_mapping(
            force,
    ...
    def calculate_coefficients(self, fx, fy, fz, mx, my, mz):
        ...
        return (
            ...
            fy / qS,
            fz / qS,
            mx / qS / self.settings["b_ref"],
            my / qS / self.settings["c_ref"],
    ...
    def screen_output(self, ts):
        ...
        aero_tstep = self.data.aero.timestep_info[ts]
        fx, fy, fz = (
            aero_tstep.total_steady_inertial_forces[:3]
            + aero_tstep.total_unsteady_inertial_forces[:3]
        )
        mx, my, mz = (
            aero_tstep.total_steady_inertial_forces[3:]
            + aero_tstep.total_unsteady_inertial_forces[3:]
        ...
        if self.settings["coefficients"]:
            Cfx, Cfy, Cfz, Cmx, Cmy, Cmz = self.calculate_coefficients(
                fx, fy, fz, mx, my, mz
            )
