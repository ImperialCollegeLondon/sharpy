...
class StabilityDerivatives(solver_interface.BaseSolver):
    ...
    def initialise(self, data, custom_settings=None, caller=None, restart=False):
        ...
        self.data.linear.derivatives = (
            dict()
        )  # {str:Derivatives()} (sharpy.linear.utils.derivatives.Derivatives)
        self.data.linear.derivatives["aerodynamic"] = Derivatives(
            reference_dimensions,
            static_state=self.steady_aero_forces(),
            target_system="aerodynamic",
        )
        self.data.linear.derivatives["aeroelastic"] = Derivatives(
            reference_dimensions,
            static_state=self.steady_aero_forces(),
    ...
    def run(self, **kwargs):
        ...
        if self.data.linear.linear_system.beam.sys.modal:
            phi = self.data.linear.linear_system.linearisation_vectors[
                "mode_shapes"
            ].real
        else:
            phi = None

        steady_forces = self.data.linear.linear_system.linearisation_vectors[
            "forces_aero_beam_dof"
        ]
        ...
        if self.data.linear.linear_system.uvlm.scaled:
            raise NotImplementedError(
                "Stability Derivatives not yet implented for scaled system"
            )
        ...
        for target_system in ["aerodynamic", "aeroelastic"]:
            ...
            target_system_derivatives.initialise_derivatives(
                state_space,
                steady_forces,
                quat,
                v0,
            ...
            target_system_derivatives.dict_of_derivatives[
                "force_angle_velocity"
            ] = target_system_derivatives.new_derivative(
                "stability", "angle_derivatives", "Force/Angle via velocity"
            )
            ...
            target_system_derivatives.dict_of_derivatives[
                "force_velocity"
            ] = target_system_derivatives.new_derivative("body", "body_derivatives")

            target_system_derivatives.dict_of_derivatives[
                "force_cs"
            ] = target_system_derivatives.new_derivative(
                "body", "control_surface_derivatives"
            )
    ...
    def get_freestream_velocity(self):
        try:
            ...
            u_inf_direction = self.data.settings["StaticCoupled"][
                "aero_solver_settings"
            ]["u_inf_direction"]
        except KeyError:
            try:
                u_inf = self.data.settings["StaticCoupled"]["aero_solver_settings"][
                ...
                cout.cout_wrap(
                    ...
                    " StaticCoupled,please ensure these settings are provided in the"
                    " config .sharpy file. Ifyou are running a restart simulation make"
                    " sure they are included too, regardlessof these solvers being"
                    " present in the SHARPy flow",
                    4,
                )
                raise KeyError
    ...
    def steady_aero_forces(self):
        fx = np.sum(
            self.data.aero.timestep_info[0].inertial_steady_forces[:, 0], 0
        ) + np.sum(self.data.aero.timestep_info[0].inertial_unsteady_forces[:, 0], 0)

        fy = np.sum(
            self.data.aero.timestep_info[0].inertial_steady_forces[:, 1], 0
        ) + np.sum(self.data.aero.timestep_info[0].inertial_unsteady_forces[:, 1], 0)

        fz = np.sum(
            self.data.aero.timestep_info[0].inertial_steady_forces[:, 2], 0
