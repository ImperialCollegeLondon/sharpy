...
class StallCheck(BaseSolver):
    ...
    def check_stall(self):
        ...
        tstep.postproc_cell["incidence_angle"] = init_matrix_structure(
            dimensions=tstep.dimensions, with_dim_dimension=False
        )
        ...
        (
            tstep.postproc_cell["incidence_angle_ct_list"],
            tstep.postproc_cell["incidence_angle_ct_pointer"],
        ) = standalone_ctypes_pointer(tstep.postproc_cell["incidence_angle"])

        # call calculate
        uvlmlib.uvlm_calculate_incidence_angle(
            self.data.aero.timestep_info[self.ts],
        ...
        for i_elem in range(self.data.structure.num_elem):
            for i_local_node in range(self.data.structure.num_node_elem):
                airfoil_id = self.data.aero.aero_dict["airfoil_distribution"][
                    i_elem, i_local_node
                ]
                if self.settings["airfoil_stall_angles"]:
                    i_global_node = self.data.structure.connectivities[
                        i_elem, i_local_node
                    ]
                    for i_dict in self.data.aero.struct2aero_mapping[i_global_node]:
                        ...
                        if tstep.postproc_cell["incidence_angle"][i_surf][
                            0, i_n
                        ] < float(limits[0]):
                            stalled_panels = True
                            stalled_surfs[i_surf] += tstep.postproc_cell[
                                "incidence_angle"
                            ][i_surf].shape[1]
                        elif tstep.postproc_cell["incidence_angle"][i_surf][
                            0, i_n
                        ] > float(limits[1]):
                            stalled_panels = True
                            stalled_surfs[i_surf] += tstep.postproc_cell[
        ...
        if stalled_panels:
            if self.settings["print_info"]:
                cout.cout_wrap(
                    "Some panel has an incidence angle out of the linear region", 1
                )
