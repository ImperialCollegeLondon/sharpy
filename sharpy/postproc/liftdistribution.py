...
class LiftDistribution(BaseSolver):
    ...
    def lift_distribution(self, struct_tstep, aero_tstep):
        ...
        if self.settings["coefficients"]:
            ...
            header += ", y/s, cl"
            numb_col += 2
            lift_distribution = np.concatenate(
                (lift_distribution, np.zeros((N_nodes, 2))), axis=1
            )
        ...
        for inode in range(N_nodes):
            if self.data.aero.aero_dict["aero_node"][inode]:
                ...
                cgb = np.dot(cga, cab)
                # Get c_bs
                urel, dir_urel = aeroutils.magnitude_and_direction_of_relative_velocity(
                    struct_tstep.pos[inode, :],
                    struct_tstep.pos_dot[inode, :],
                    struct_tstep.for_vel[:],
                    cga,
                    aero_tstep.u_ext[i_surf][:, :, local_node],
                )
                dir_span, span, dir_chord, chord = aeroutils.span_chord(
                    local_node, aero_tstep.zeta[i_surf]
                )
                # Stability axes - projects forces in B onto S
                c_bs = aeroutils.local_stability_axes(
                ...
                if self.settings["coefficients"]:
                    ...
                    lift_distribution[inode, 5] = (
                        np.sign(lift_force)
                        * np.linalg.norm(lift_force)
                        / (
                            0.5
                            * self.settings["rho"]
                            * np.linalg.norm(urel) ** 2
                            * span
                            * chord
                        )
                    )  # strip_area[i_surf][local_node])
                    ...
                    lift_distribution[inode, 5] /= len(
                        self.data.aero.struct2aero_mapping[inode]
                    )

        # Export lift distribution data
        np.savetxt(
            os.path.join(self.folder, self.settings["text_file_name"]),
            lift_distribution,
            fmt="%10e," * (numb_col - 1) + "%10e",
            delimiter=", ",
            header=header,
        )
