...
class SaveData(BaseSolver):
    ...
    settings_description["save_linear_uvlm"] = (
        "Save linear UVLM state space system. Use with caution when dealing with "
        "large systems."
    )
    ...
    settings_description[
        "save_rom"
    ] = "Saves the ROM matrices and the reduced order model"
    ...
    settings_description[
        "format"
    ] = "Save linear state space to hdf5 ``.h5`` or Matlab ``.mat`` format."
    ...
    def initialise(self, data, custom_settings=None, caller=None, restart=False):
        ...
        self.settings["skip_attr"].extend(
            [
                "fortran",
                "airfoils",
                "airfoil_db",
        ...
        self.filename_linear = (
            self.folder + self.data.settings["SHARPy"]["case"] + ".linss.h5"
        )
        ...
        if self.settings["save_linear"] or self.settings["save_linear_uvlm"]:
            try:
                ...
                cout.cout_wrap(
                    "SaveData variables ``save_linear`` and/or ``save_linear`` are True"
                    " but no linear systembeen found",
                    3,
                )
            ...
            if self.settings["format"] == "h5":
                ...
                if online and file_exists:
                    ...
                    h5utils.add_as_grp(
                        ...
                        SkipAttr=skip_attr_init,
                        compress_float=self.settings["compress_float"],
                    )

                    if self.settings["save_struct"]:
                        h5utils.add_as_grp(
                            list(), hdfile["data"]["structure"], grpname="timestep_info"
                        )
                    if self.settings["save_aero"]:
                        h5utils.add_as_grp(
                            list(), hdfile["data"]["aero"], grpname="timestep_info"
                        )
                        ...
                        for k, rom in self.data.linear.linear_system.uvlm.rom.items():
                            rom.save(
                                self.filename.replace(
                                    ".data.h5", "_{:s}.rom.h5".format(k.lower())
                                )
                            )
                    except AttributeError:
                ...
                if self.settings["save_linear_uvlm"]:
                    ...
                    linearisation_vectors = (
                        self.data.linear.linear_system.uvlm.linearisation_vectors
                    )
