...
def balreal_direct_py(A, B, C, DLTI=True, Schur=False, full_outputs=False):
    ...
    if type(A) is not np.ndarray:
        try:
            ...
            raise TypeError(
                "Matrix needs to be in dense form. Unable to convert A matrix of type"
                f" {type(A)} to dense using method .todense()"
            )
    ...
    if lowrank:  # low-rank square-Smith iteration (with SVD)
        ...
        while DeltaNorm > tolSmith and DeltaNormNext > 1e-3 * tolSmith:
            ...
            if kmin == None or kmin < rC:
                ...
                Uc, svc = scalg.svd(
                    Qck, full_matrices=False, overwrite_a=True, lapack_driver="gesdd"
                )[:2]
...
def balreal_iter_old(
    A, B, C, lowrank=True, tolSmith=1e-10, tolSVD=1e-6, kmax=None, tolAbs=False
):
    """
    Find balanced realisation of DLTI system.
    ...
    if lowrank:  # low-rank square-Smith iteration (with SVD)
        ...
        while DeltaNorm > tolSmith:
            ...
            if kmax is None:
                # cheap SVD truncation
                if Qck.shape[1] > 0.4 * N or Qok.shape[1] > 0.4 * N:
    ...
    )  # as M is square, full_matrices has no effect
    sinv = s ** (-0.5)
...
def get_trapz_weights(k0, kend, Nk, knyq=False):
    ...
    assert k0 >= 0.0 and kend >= 0.0, "Frequencies must be positive!"
...
def get_gauss_weights(k0, kend, Npart, order):
    ...
    if Npart == 1:
        ...
        krange = kend - k0
        kv = 0.5 * (k0 + kend) + 0.5 * krange * xad
        wv = wad * (0.5 * krange) * np.sqrt(2)
        ...
        for ii in range(Npart):
            ...
            kv[iivec], wv[iivec] = get_gauss_weights(
                k0_part, kend_part, Npart=1, order=order
            )
...
def balfreq(SS, DictBalFreq):
    ...
    if DictBalFreq["method_high"] == "trapz":
        if Opt["points"] == 0:
            ...
            kv_high, wv_high = get_trapz_weights(
        ...
        if Opt["order"] * Opt["partitions"] == 0:
            ...
            kv_high, wv_high = get_gauss_weights(
    ...
    Zc = np.zeros(
        (SS.states, 2 * SS.inputs * len(kvdt)),
    )
    Zo = np.zeros(
        (SS.states, 2 * SS.outputs * Nk_low),
    )
    ...
    for kk in range(len(kvdt)):
        ...
        if DictBalFreq["get_frequency_response"] and kk < Nk_low:
            Yfreq[:, :, kk] = (1.0 / Intfact) * libsp.dot(
                SS.C, Qctrl, type_out=np.ndarray
            ) + SS.D
    ...
    if convergence == "all":
        ...
        while not Found:
            ...
            if N == Nmax and er > tol:
                warnings.warn(
                    "librom.tune_rom: error %.2e above tolerance %.2e and HSV bound"
                    " %.2e" % (er, tol, gv[N - 1])
                )
...
def eigen_dec(
    ...
    if URinv is None:
        try:
            ...
            print(
                "The A matrix can not be diagonalised as does not admit "
    ...
    if Nlist[-1] > N:
        warnings.warn(
            "Resizing the eigendecomposition from %.3d to %.3d states" % (N, Nlist[-1])
        )
        N = Nlist[-1]
    ...
    while nn < N:
        ...
        if (nn + 1 in Nlist) and np.abs(eigs[nn].imag) < tol:
            ...
            #
            Cproj[:, nn] = 2.0 * np.dot(C, UR[:, nn].real)
            Cproj[:, nn + 1] = -2.0 * np.dot(C, UR[:, nn].imag)
            nn += 2
