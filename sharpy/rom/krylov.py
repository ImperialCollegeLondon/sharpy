...
class Krylov(rom_interface.BaseRom):
    ...
    def save(self, filename):
        ...
        with h5py.File(filename, "a") as outfile:
            h5.add_as_grp(
                rom_projectors, outfile, grpname="projectors", compress_float=True
            )
            h5.add_as_grp(
                self.ssrom,
                outfile,
    ...
    def save_reduced_order_bases(self, file_name):
        ...
        if (
            self.settings["single_side"] == "observability"
            or self.settings["single_side"] == "controllability"
        ):
            # if single sided, the projector is V and W = V therefore no need to duplicate
            libss.Gain.save_multiple_gains(file_name, ("V", V))
            cout.cout_wrap(
                f"Saved Krylov reduced order bases, V to file: {file_name}", 1
            )
        else:
            libss.Gain.save_multiple_gains(file_name, ("V", V), ("W", W))
            cout.cout_wrap(
                f"Saved Krylov reduced order bases, V and W to file: {file_name}", 1
            )
    ...
    def run(self, ss):
        ...
        try:
            ...
            self.ssrom.state_variables = LinearVector(
                [StateVariable("krylov", size=self.ssrom.states, index=0)]
            )
        except AttributeError:
            pass
    ...
    def print_header(self):
        ...
        if self.frequency.dtype == complex:
            cout.cout_wrap(
                self.nfreq
                * "\t\tsigma = %4f + %4fj [rad/s]\n"
                % tuple(self.frequency.view(float)),
                1,
            )
        else:
    ...
    def real_rational_arnoldi(self, frequency, r):
        ...
        raise NotImplementedError(
            "Real valued rational Arnoldi Method Work in progress - use"
            " mimo_rational_arnoldi"
        )
    ...
    def dual_rational_arnoldi(self, frequency, r):
        ...
        if nu != 1:
            ...
            assert (
                right_tangent is not None and left_tangent is not None
            ), "Missing interpolation vectors for MIMO case"
        else:
        ...
        for i in range(len(fc)):
            ...
            V[:, we : we + rc[i]] = krylovutils.construct_krylov(
                rc[i], lu_A, B.dot(right_tangent[:, i : i + 1]), approx_type, "b"
            )
        ...
        for i in range(len(fo)):
            ...
            W[:, we : we + ro[i]] = krylovutils.construct_krylov(
                ro[i], lu_A, C.T.dot(left_tangent[:, i : i + 1]), approx_type, "c"
            )
    ...
    def mimo_rational_arnoldi(self, frequency, r):
        ...
        for i in range(self.nfreq):
            if (
                self.settings["single_side"] == "controllability"
                or self.settings["single_side"] == ""
            ):
                cout.cout_wrap("\tConstructing controllability space", 1)
                if i == 0:
                    V = krylovutils.build_krylov_space(
                        frequency[i], r_c, side="b", a=self.ss.A, b=self.ss.B
                    )
                else:
                    Vi = krylovutils.build_krylov_space(
                        frequency[i], r_c, side="b", a=self.ss.A, b=self.ss.B
                    )
                    V = np.hstack((V, Vi))
            ...
            if (
                ...
                if i == 0:
                    W = krylovutils.build_krylov_space(
                        frequency[i], r_o, side="c", a=self.ss.A, b=self.ss.C.T
                    )
                else:
                    Wi = krylovutils.build_krylov_space(
                        frequency[i], r_o, side="c", a=self.ss.A, b=self.ss.C.T
                    )
                    W = np.hstack((W, Wi))
        ...
        if (
            self.settings["single_side"] == "controllability"
            or self.settings["single_side"] == "observability"
        ):
        ...
        self.W = libss.Gain(
            W,
            input_vars=LinearVector(
                [InputVariable("krylov", size=W.shape[1], index=0)]
            ),
        ...
        self.V = libss.Gain(
            V,
            input_vars=LinearVector(
                [InputVariable("krylov", size=V.shape[1], index=0)]
            ),
            output_vars=LinearVector.transform(self.ss.state_variables, OutputVariable),
        )

        # for state recovery purposes
        self.projection_gain = libss.Gain(
            V,
            input_vars=LinearVector(
                [InputVariable("krylov", size=V.shape[1], index=0)]
            ),
    ...
    def check_stability(self, restart_arnoldi=False):
        ...
        if self.sstype == "dt":
            if any(eigs_abs > 1.0):
                unstable = True
                unstable_eigenvalues = eigs[eigs_abs > 1.0]
                try:
                    cout.cout_wrap(
        ...
        if unstable and restart_arnoldi:
            print(
                "Restarting the Arnoldi method - Reducing ROM order from r = %d to r"
                " = %d" % (self.r, self.r - 1)
            )
            ...
            for mu in unstable_eigenvalues:
                remove_unstable = np.matmul(
                    remove_unstable, mu * np.eye(self.ss.states) - self.ss.A
                )
    ...
    def stable_realisation(self, *args, **kwargs):
        ...
        cout.cout_wrap(
            "Stabilising system by removing unstable eigenvalues using a Schur"
            " decomposition",
            1,
        )
        ...
        if ct:
            assert (
                np.sum(np.real(eigsAr) <= 0.0) == n_stable
            ), "Number of stable eigvals computed not equal to those in Ar"
        else:
            assert (
                np.sum(np.abs(eigsAr) <= 1.0) == n_stable
            ), "Number of stable eigvals computed not equal to those in Ar"
    ...
    def restart(self):
        ...
        eigsAr = np.linalg.eigvals(Ar)
        n_stable_r = np.sum(np.abs(eigsAr) <= 1.0)
...
def check_eye(T, Tinv, msg=""):
    ...
    if max_diff != 0.0:
        ...
        assert (
            log_error < -6
        ), "T.dot(Tinv) not equal to identity, %s \nlog(error) = %.e" % (
            msg,
            np.log10(max_diff),
        )
