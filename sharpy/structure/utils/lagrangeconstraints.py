...
def lagrangeconstraint(arg):
    ...
    try:
        ...
        raise AttributeError(
            "Class defined as lagrange constraint has no _lc_id attribute"
        )
...
def equal_pos_node_FoR(
    ...
    """
    cout.cout_wrap(
        "WARNING: this equation constitutes a holonomic constraint which is not"
        " currently supported. Check ``equal_lin_vel_node_FoR``",
        3,
    )
    ...
    LM_K[sys_size + ieq : sys_size + ieq + num_LM_eq_specific, :sys_size] += (
        scalingFactor * B
    )
    LM_K[
        :sys_size, sys_size + ieq : sys_size + ieq + num_LM_eq_specific
    ] += scalingFactor * np.transpose(B)

    LM_Q[:sys_size] += scalingFactor * np.dot(
        np.transpose(B), Lambda[ieq : ieq + num_LM_eq_specific]
    )
    LM_Q[sys_size + ieq : sys_size + ieq + num_LM_eq_specific] += scalingFactor * (
        node_FoR_pos + np.dot(node_cga, node_pos) - FoR_pos
    )

    LM_C[
        node_dof : node_dof + 3, node_FoR_dof + 6 : node_FoR_dof + 10
    ] += scalingFactor * ag.der_CquatT_by_v(
    ...
    if penaltyFactor:
        ...
        LM_K[
            node_FoR_dof : node_FoR_dof + 3, node_FoR_dof : node_FoR_dof + 3
        ] += penaltyFactor * np.eye(3)
        LM_K[node_FoR_dof : node_FoR_dof + 3, node_dof : node_dof + 3] += (
            penaltyFactor * node_cga
        )
        LM_K[
            node_FoR_dof : node_FoR_dof + 3, FoR_dof : FoR_dof + 3
        ] += -penaltyFactor * np.eye(3)
        LM_C[
            node_FoR_dof : node_FoR_dof + 3, node_FoR_dof + 6 : node_FoR_dof + 10
        ] += penaltyFactor * ag.der_Cquat_by_v(MB_tstep[node_body].quat, node_pos)

        LM_K[node_dof : node_dof + 3, node_FoR_dof : node_FoR_dof + 3] += (
            penaltyFactor * node_cga.T
        )
        LM_K[
            node_dof : node_dof + 3, node_dof : node_dof + 3
        ] += penaltyFactor * np.eye(3)
        LM_K[node_dof : node_dof + 3, FoR_dof : FoR_dof + 3] += (
            -penaltyFactor * node_cga.T
        )
        LM_C[
            node_dof : node_dof + 3, node_FoR_dof + 6 : node_FoR_dof + 10
        ...
        LM_K[
            FoR_dof : FoR_dof + 3, node_FoR_dof : node_FoR_dof + 3
        ] += -penaltyFactor * np.eye(3)
        ...
        LM_C[
            FoR_dof : FoR_dof + 3, node_FoR_dof + 6 : node_FoR_dof + 10
        ] += -penaltyFactor * ag.der_Cquat_by_v(MB_tstep[node_body].quat, node_pos)
...
def equal_lin_vel_node_FoR(
    ...
    LM_C[sys_size + ieq : sys_size + ieq + num_LM_eq_specific, :sys_size] += (
        scalingFactor * Bnh
    )
    LM_C[
        :sys_size, sys_size + ieq : sys_size + ieq + num_LM_eq_specific
    ] += scalingFactor * np.transpose(Bnh)

    LM_Q[:sys_size] += scalingFactor * np.dot(
        np.transpose(Bnh), Lambda_dot[ieq : ieq + num_LM_eq_specific]
    )
    LM_Q[sys_size + ieq : sys_size + ieq + num_LM_eq_specific] += scalingFactor * (
        np.dot(FoR_cga, FoR_va)
        + -1.0
        * np.dot(
            node_cga,
            node_dot_Ra + node_FoR_va + -1.0 * np.dot(ag.skew(node_Ra), node_FoR_wa),
        )
    )

    LM_C[
        FoR_dof : FoR_dof + 3, FoR_dof + 6 : FoR_dof + 10
    ] += scalingFactor * ag.der_CquatT_by_v(
        MB_tstep[FoR_body].quat, Lambda_dot[ieq : ieq + num_LM_eq_specific]
    )
    ...
    if MB_beam[node_body].FoR_movement == "free":
        ...
        LM_C[
            node_FoR_dof + 3 : node_FoR_dof + 6, node_FoR_dof + 6 : node_FoR_dof + 10
        ] += scalingFactor * np.dot(
            ag.skew(node_Ra).T,
            ag.der_CquatT_by_v(
                MB_tstep[node_body].quat, Lambda_dot[ieq : ieq + num_LM_eq_specific]
            ),
        ...
        LM_K[
            node_FoR_dof + 3 : node_FoR_dof + 6, node_dof : node_dof + 3
        ] += scalingFactor * ag.skew(
            np.dot(node_cga.T, Lambda_dot[ieq : ieq + num_LM_eq_specific])
        )
    ...
    if penaltyFactor:
        ...
        LM_C[
            FoR_dof : FoR_dof + 3, FoR_dof + 6 : FoR_dof + 10
        ] -= penaltyFactor * ag.der_CquatT_by_v(
            MB_tstep[FoR_body].quat,
            np.dot(
                node_cga,
                node_dot_Ra + node_FoR_va + np.dot(ag.skew(node_Ra), node_FoR_wa),
            ),
        )

        LM_C[
            node_dof : node_dof + 3, FoR_dof + 6 : FoR_dof + 10
        ] -= penaltyFactor * np.dot(
            node_cga.T, ag.der_CquatT_by_v(MB_tstep[FoR_body].quat, FoR_va)
        )

        if MB_beam[node_body].FoR_movement == "free":
            LM_C[
                node_FoR_dof : node_FoR_dof + 3, FoR_dof + 6 : FoR_dof + 10
            ] -= penaltyFactor * np.dot(
                node_cga.T, ag.der_CquatT_by_v(MB_tstep[FoR_body].quat, FoR_va)
            )
            ...
            LM_C[
                node_FoR_dof + 3 : node_FoR_dof + 6, FoR_dof + 6 : FoR_dof + 10
            ] += penaltyFactor * np.dot(
                mat, ag.der_CquatT_by_v(MB_tstep[FoR_body].quat, FoR_va)
            )
        ...
        if MB_beam[node_body].FoR_movement == "free":
            ...
            LM_C[
                ...
                FoR_cga.T, ag.der_Cquat_by_v(MB_tstep[node_body].quat, vec)
            )

            derivative = -ag.der_CquatT_by_v(
                MB_tstep[node_body].quat, np.dot(FoR_cga, FoR_va)
            )
            LM_C[node_dof : node_dof + 3, node_FoR_dof + 6 : node_FoR_dof + 10] += (
                penaltyFactor * derivative
            )
            LM_C[
                node_FoR_dof : node_FoR_dof + 3, node_FoR_dof + 6 : node_FoR_dof + 10
            ] += (penaltyFactor * derivative)
            LM_C[
                node_FoR_dof + 3 : node_FoR_dof + 6,
                node_FoR_dof + 6 : node_FoR_dof + 10,
            ] -= penaltyFactor * np.dot(ag.skew(node_Ra), derivative)

        # Derivatives wrt the node Ra
        LM_K[
            FoR_dof : FoR_dof + 3, node_dof : node_dof + 3
        ] -= penaltyFactor * ag.multiply_matrices(
            FoR_cga.T, node_cga, ag.skew(node_FoR_wa)
        )
        LM_K[
            node_dof : node_dof + 3, node_dof : node_dof + 3
        ] += penaltyFactor * ag.skew(node_FoR_wa)
        if MB_beam[node_body].FoR_movement == "free":
            LM_K[
                node_FoR_dof : node_FoR_dof + 3, node_dof : node_dof + 3
            ] += penaltyFactor * ag.skew(node_FoR_wa)
            vec = (
                ag.multiply_matrices(node_cga.T, FoR_cga, FoR_va)
                - node_dot_Ra
                - node_FoR_va
            )
            LM_K[
                node_FoR_dof + 3 : node_FoR_dof + 6, node_dof : node_dof + 3
            ] += penaltyFactor * ag.skew(vec)
            LM_K[
                node_FoR_dof + 3 : node_FoR_dof + 6, node_dof : node_dof + 3
            ] -= penaltyFactor * ag.der_skewp_skewp_v(node_Ra, node_FoR_wa)

    ieq += 3
    return ieq


def rel_rot_vel_node_FoR(
    MB_tstep,
    MB_beam,
    FoR_body,
    node_body,
    ...
    Bnh[:, FoR_dof + 3 : FoR_dof + 6] -= ag.multiply_matrices(
        cab.T, node_cga.T, FoR_cga
    )
    if MB_beam[node_body].FoR_movement == "free":
    ...
    LM_Q[sys_size + ieq : sys_size + ieq + num_LM_eq_specific] += scalingFactor * (
        np.dot(tan, MB_tstep[node_body].psi_dot[ielem, inode_in_elem, :])
    ...
    LM_K[
        node_dof + 3 : node_dof + 6, node_dof + 3 : node_dof + 6
    ] += scalingFactor * ag.der_TanT_by_xv(
        psi, Lambda_dot[ieq : ieq + num_LM_eq_specific]
    )
    if MB_beam[node_body].FoR_movement == "free":
        LM_K[
            node_FoR_dof + 3 : node_FoR_dof + 6, node_dof + 3 : node_dof + 6
        ] += scalingFactor * ag.der_Ccrv_by_v(
            psi, Lambda_dot[ieq : ieq + num_LM_eq_specific]
        )

    LM_K[
        FoR_dof + 3 : FoR_dof + 6, node_dof + 3 : node_dof + 6
    ] -= scalingFactor * ag.der_Ccrv_by_v(
        psi,
        ag.multiply_matrices(
            node_cga, FoR_cga.T, Lambda_dot[ieq : ieq + num_LM_eq_specific]
        ),
    )

    LM_C[
        FoR_dof + 3 : FoR_dof + 6, node_FoR_dof + 6 : node_FoR_dof + 10
    ] -= scalingFactor * np.dot(
        cab,
        ag.der_Cquat_by_v(
            MB_tstep[node_body].quat,
            np.dot(FoR_cga.T, Lambda_dot[ieq : ieq + num_LM_eq_specific]),
        ),
    )
    LM_C[
        FoR_dof + 3 : FoR_dof + 6, FoR_dof + 6 : FoR_dof + 10
    ] -= scalingFactor * ag.multiply_matrices(
        cab,
        node_cga,
        ag.der_CquatT_by_v(
            MB_tstep[FoR_body].quat, Lambda_dot[ieq : ieq + num_LM_eq_specific]
...
def def_rot_axis_FoR_wrt_node_general(
    MB_tstep,
    MB_beam,
    FoR_body,
    node_body,
    ...
    """
    cout.cout_wrap(
        "WARNING: this function is missing the contribution of the rotation velocity of"
        " the reference node. See ``def_rot_axis_FoR_wrt_node_general``",
        3,
    )
    ...
    if not indep:
        aux_Bnh = ag.multiply_matrices(ag.skew(rot_axisB), cab.T, node_cga.T, FoR_cga)
    ...
    Bnh[:, FoR_dof + 3 : FoR_dof + 6] = ag.multiply_matrices(
        ag.skew(rot_axisB), cab.T, node_cga.T, FoR_cga
    )[indep, :]
    ...
    LM_Q[sys_size + ieq : sys_size + ieq + num_LM_eq_specific] += (
        scalingFactor
        * ag.multiply_matrices(ag.skew(rot_axisB), cab.T, node_cga.T, FoR_cga, FoR_wa)[
            indep
        ]
    )
    ...
    if MB_beam[node_body].FoR_movement == "free":
        LM_C[
            ...
            ag.der_Cquat_by_v(
                MB_tstep[node_body].quat,
                ag.multiply_matrices(cab, ag.skew(rot_axisB).T, new_Lambda_dot),
            ),
        )
    ...
    LM_C[
        ...
        MB_tstep[FoR_body].quat,
        ag.multiply_matrices(node_cga, cab, ag.skew(rot_axisB).T, new_Lambda_dot),
    )

    LM_K[
        FoR_dof + 3 : FoR_dof + 6, node_dof + 3 : node_dof + 6
    ] += scalingFactor * ag.multiply_matrices(
        FoR_cga.T, node_cga, cab, ag.skew(rot_axisB).T, new_Lambda_dot
    )
    ...
    if penaltyFactor:
        ...
        vec = ag.multiply_matrices(
            node_cga, cab, sq_rot_axisB, cab.T, node_cga.T, FoR_cga, FoR_wa
        )
        LM_C[
            FoR_dof + 3 : FoR_dof + 6, FoR_dof + 6 : FoR_dof + 10
        ...
        mat = ag.multiply_matrices(
            FoR_cga.T, node_cga, cab, sq_rot_axisB, cab.T, node_cga.T
        )
        LM_C[
            FoR_dof + 3 : FoR_dof + 6, FoR_dof + 6 : FoR_dof + 10
        ] += penaltyFactor * np.dot(
            mat, ag.der_Cquat_by_v(MB_tstep[FoR_body].quat, FoR_wa)
        )

        if MB_beam[node_body].FoR_movement == "free":
            # Derivatives with the quaternion of the FoR of the node
            vec = ag.multiply_matrices(
                cab, sq_rot_axisB, cab.T, node_cga.T, FoR_cga, FoR_wa
            )
            LM_C[
                FoR_dof + 3 : FoR_dof + 6, node_FoR_dof + 6 : node_FoR_dof + 10
            ] += penaltyFactor * np.dot(
            ...
            mat = ag.multiply_matrices(FoR_cga.T, node_cga, cab, sq_rot_axisB, cab.T)
            vec = np.dot(FoR_cga, FoR_wa)
            LM_C[
                FoR_dof + 3 : FoR_dof + 6, node_FoR_dof + 6 : node_FoR_dof + 10
            ] += penaltyFactor * np.dot(
                mat, ag.der_CquatT_by_v(MB_tstep[node_body].quat, vec)
            )

        # Derivatives with the CRV
        mat = np.dot(FoR_cga.T, node_cga)
        vec = ag.multiply_matrices(sq_rot_axisB, cab.T, node_cga.T, FoR_cga, FoR_wa)
        LM_K[
            FoR_dof + 3 : FoR_dof + 6, node_dof + 3 : node_dof + 6
        ] += penaltyFactor * np.dot(
            mat, ag.der_Ccrv_by_v(MB_tstep[node_body].psi[ielem, inode_in_elem, :], vec)
        )

        mat = ag.multiply_matrices(FoR_cga.T, node_cga, cab, sq_rot_axisB)
        vec = ag.multiply_matrices(node_cga.T, FoR_cga, FoR_wa)
        LM_K[
            FoR_dof + 3 : FoR_dof + 6, node_dof + 3 : node_dof + 6
        ] += penaltyFactor * np.dot(
            mat,
            ag.der_CcrvT_by_v(MB_tstep[node_body].psi[ielem, inode_in_elem, :], vec),
        )
...
def def_rot_axis_FoR_wrt_node_xyz(
    MB_tstep,
    MB_beam,
    FoR_body,
    node_body,
    ...
    LM_Q[
        sys_size + ieq : sys_size + ieq + num_LM_eq_specific
    ] += scalingFactor * ag.multiply_matrices(Z, cab.T, node_cga.T, FoR_cga, FoR_wa)
    LM_Q[
        sys_size + ieq : sys_size + ieq + num_LM_eq_specific
    ] -= scalingFactor * ag.multiply_matrices(Z, ag.crv2tan(psi), psi_dot)
    LM_Q[
        sys_size + ieq : sys_size + ieq + num_LM_eq_specific
    ] -= scalingFactor * ag.multiply_matrices(
        Z, cab.T, MB_tstep[node_body].for_vel[3:6]
    )
    ...
    vec = ag.multiply_matrices(
        node_cga, cab, Z.T, Lambda_dot[ieq : ieq + num_LM_eq_specific]
    )
    LM_C[
        FoR_dof + 3 : FoR_dof + 6, FoR_dof + 6 : FoR_dof + 10
    ...
    LM_K[
        ...
        node_cga,
        ag.der_Ccrv_by_v(
            MB_tstep[node_body].psi[ielem, inode_in_elem, :],
    ...
    LM_K[
        ...
        psi, ag.multiply_matrices(Z.T, Lambda_dot[ieq : ieq + num_LM_eq_specific])
    )
    LM_K[
        node_FoR_dof + 3 : node_FoR_dof + 6, node_dof + 3 : node_dof + 6
    ] -= scalingFactor * ag.der_Ccrv_by_v(
    ...
    if penaltyFactor:
        ...
        # Derivatives with the quaternion of the FoR
        vec = ag.multiply_matrices(
            node_cga, cab, ZTZ, cab.T, node_cga.T, FoR_cga, FoR_wa
        )
        LM_C[
            FoR_dof + 3 : FoR_dof + 6, FoR_dof + 6 : FoR_dof + 10
        ] += penaltyFactor * ag.der_CquatT_by_v(MB_tstep[FoR_body].quat, vec)

        mat = ag.multiply_matrices(FoR_cga.T, node_cga, cab, ZTZ, cab.T, node_cga.T)
        LM_C[
            FoR_dof + 3 : FoR_dof + 6, FoR_dof + 6 : FoR_dof + 10
        ] += penaltyFactor * np.dot(
            mat, ag.der_Cquat_by_v(MB_tstep[FoR_body].quat, FoR_wa)
        ...
        if MB_beam[node_body].FoR_movement == "free":
            # Derivatives with the quaternion of the FoR of the node
            vec = ag.multiply_matrices(cab, ZTZ, cab.T, node_cga.T, FoR_cga, FoR_wa)
            LM_C[
                FoR_dof + 3 : FoR_dof + 6, node_FoR_dof + 6 : node_FoR_dof + 10
            ] += penaltyFactor * np.dot(
                FoR_cga.T, ag.der_Cquat_by_v(MB_tstep[node_body].quat, vec)
            ...
            mat = ag.multiply_matrices(FoR_cga.T, node_cga, cab, ZTZ, cab.T)
            vec = np.dot(FoR_cga, FoR_wa)
            LM_C[
                FoR_dof + 3 : FoR_dof + 6, node_FoR_dof + 6 : node_FoR_dof + 10
            ] += penaltyFactor * np.dot(
        ...
        # Derivatives with the CRV
        mat = np.dot(FoR_cga.T, node_cga)
        vec = ag.multiply_matrices(ZTZ, cab.T, node_cga.T, FoR_cga, FoR_wa)
        LM_K[
            FoR_dof + 3 : FoR_dof + 6, node_dof + 3 : node_dof + 6
        ] += penaltyFactor * np.dot(
            mat, ag.der_Ccrv_by_v(MB_tstep[node_body].psi[ielem, inode_in_elem, :], vec)
        ...
        mat = ag.multiply_matrices(FoR_cga.T, node_cga, cab, ZTZ)
        vec = ag.multiply_matrices(node_cga.T, FoR_cga, FoR_wa)
        LM_K[
            FoR_dof + 3 : FoR_dof + 6, node_dof + 3 : node_dof + 6
        ] += penaltyFactor * np.dot(
...
def def_rot_vel_mod_FoR_wrt_node(
    MB_tstep,
    MB_beam,
    FoR_body,
    node_body,
    ...
    LM_Q[
        sys_size + ieq : sys_size + ieq + num_LM_eq_specific
    ] += scalingFactor * ag.multiply_matrices(Znon, cab.T, node_cga.T, FoR_cga, FoR_wa)
    LM_Q[sys_size + ieq : sys_size + ieq + num_LM_eq_specific] -= (
        scalingFactor * rot_vel
    )
    ...
    vec = ag.multiply_matrices(
        node_cga, cab, Znon.T, Lambda_dot[ieq : ieq + num_LM_eq_specific]
    )
    LM_C[
        FoR_dof + 3 : FoR_dof + 6, FoR_dof + 6 : FoR_dof + 10
    ] += scalingFactor * ag.der_CquatT_by_v(MB_tstep[FoR_body].quat, vec)
    ...
    if MB_beam[node_body].FoR_movement == "free":
        vec = ag.multiply_matrices(
            cab, Znon.T, Lambda_dot[ieq : ieq + num_LM_eq_specific]
        )
        LM_C[
            FoR_dof + 3 : FoR_dof + 6, node_FoR_dof + 6 : node_FoR_dof + 10
        ] += scalingFactor * ag.multiply_matrices(
...
def def_rot_vect_FoR_wrt_node(
    ...
    LM_Q[sys_size + ieq : sys_size + ieq + num_LM_eq_specific] += scalingFactor * (
        np.dot(Bnh[:, FoR_dof + 3 : FoR_dof + 6], FoR_wa) - rot_vect
    )
    ...
    if penaltyFactor:
        LM_C[
            FoR_dof + 3 : FoR_dof + 6, FoR_dof + 3 : FoR_dof + 6
        ] += penaltyFactor * np.eye(3)
...
class hinge_node_FoR(BaseLagrangeConstraint):
    ...
    def initialise(self, MBdict_entry, ieq, print_info=True):
        ...
        self._ieq = ieq
        self.scalingFactor = set_value_or_default(MBdict_entry, "scalingFactor", 1.0)
        self.penaltyFactor = set_value_or_default(MBdict_entry, "penaltyFactor", 0.0)
        ...
        if (self.rot_axisB[[1, 2]] == 0).all():
            ...
            raise NotImplementedError(
                "Hinges should be parallel to the xB, yB or zB of the reference node"
            )
    ...
    def dynamicmat(
        ...
        if self.rot_dir == "general":
            ieq = def_rot_axis_FoR_wrt_node_general(
                MB_tstep,
                MB_beam,
                self.FoR_body,
                self.node_body,
            ...
            ieq = def_rot_axis_FoR_wrt_node_xyz(
                MB_tstep,
                MB_beam,
                self.FoR_body,
                self.node_body,
    ...
    def dynamicpost(self, lc_list, MB_beam, MB_tstep):
        MB_tstep[self.FoR_body].for_pos[0:3] = (
            np.dot(
                MB_tstep[self.node_body].cga(),
...
class hinge_node_FoR_constant_vel(BaseLagrangeConstraint):
    ...
    def dynamicmat(
        ...
        ieq = def_rot_vel_mod_FoR_wrt_node(
            MB_tstep,
            MB_beam,
            self.FoR_body,
            self.node_body,
    ...
    def dynamicpost(self, lc_list, MB_beam, MB_tstep):
        ielem, inode_in_elem = MB_beam[self.node_body].node_master_elem[
            self.node_number
        ]
...
class hinge_node_FoR_pitch(BaseLagrangeConstraint):
    ...
    def initialise(self, MBdict_entry, ieq, print_info=True):
        ...
        pitch_vel = set_value_or_default(MBdict_entry, "pitch_vel", 0.0)
    ...
    def dynamicmat(
        ...
        # rel_vel in B FoR
        rel_vel = np.array([0.0, 0.0, self.rotor_vel])
        rel_vel += ag.multiply_matrices(
            cab.T, node_cga.T, FoR_cga, np.array([self.pitch_vel, 0.0, 0.0])
        )
...
class spherical_FoR(BaseLagrangeConstraint):
    ...
    def dynamicmat(
        ...
        LM_C[sys_size + ieq : sys_size + ieq + num_LM_eq_specific, :sys_size] += (
            self.scalingFactor * Bnh
        )
        LM_C[
            :sys_size, sys_size + ieq : sys_size + ieq + num_LM_eq_specific
        ] += self.scalingFactor * np.transpose(Bnh)

        LM_Q[:sys_size] += self.scalingFactor * np.dot(
            np.transpose(Bnh), Lambda_dot[ieq : ieq + num_LM_eq_specific]
        )

        LM_Q[sys_size + ieq : sys_size + ieq + 3] += self.scalingFactor * MB_tstep[
            self.body_FoR
        ].for_vel[0:3].astype(dtype=ct.c_double, copy=True, order="F")
...
class hinge_FoR(BaseLagrangeConstraint):
    ...
    def dynamicmat(
        ...
        if self.rot_dir == "general":
            LM_Q[
                sys_size + ieq + 3 : sys_size + ieq + 5
            ] += self.scalingFactor * np.dot(
                skew_rot_axis[[row0, row1], :], MB_tstep[self.body_FoR].for_vel[3:6]
            )
        else:
            LM_Q[sys_size + ieq + 3 : sys_size + ieq + 5] += (
                self.scalingFactor * MB_tstep[self.body_FoR].for_vel[3 + self.zero_comp]
            )

        if self.penaltyFactor:
            LM_Q[FoR_dof : FoR_dof + 3] += (
                self.penaltyFactor * MB_tstep[self.body_FoR].for_vel[0:3]
            )
            LM_C[
                FoR_dof : FoR_dof + 3, FoR_dof : FoR_dof + 3
            ] += self.penaltyFactor * np.eye(3)

            if self.rot_dir == "general":
                sq_rot_axis = np.dot(ag.skew(self.rot_axis).T, ag.skew(self.rot_axis))
                LM_Q[FoR_dof + 3 : FoR_dof + 6] += self.penaltyFactor * np.dot(
                    sq_rot_axis, MB_tstep[self.body_FoR].for_vel[3:6]
                )
                LM_C[FoR_dof + 3 : FoR_dof + 6, FoR_dof + 3 : FoR_dof + 6] += (
                    self.penaltyFactor * sq_rot_axis
                )
            else:
                LM_Q[FoR_dof + 3 : FoR_dof + 6] += (
                    self.penaltyFactor * MB_tstep[self.body_FoR].for_vel[3:6]
                )
                LM_C[
                    FoR_dof + 3 : FoR_dof + 6, FoR_dof + 3 : FoR_dof + 6
                ] += self.penaltyFactor * np.eye(3)
...
class hinge_FoR_wrtG(BaseLagrangeConstraint):
    ...
    def dynamicmat(
        ...
        LM_C[
            FoR_dof : FoR_dof + 3, FoR_dof + 6 : FoR_dof + 10
        ] += self.scalingFactor * ag.der_CquatT_by_v(
            MB_tstep[self.body_FoR].quat, Lambda_dot[ieq : ieq + 3]
        )
        ...
        LM_Q[sys_size + ieq : sys_size + ieq + 3] += self.scalingFactor * np.dot(
            MB_tstep[self.body_FoR].cga(), MB_tstep[self.body_FoR].for_vel[0:3]
        )
        LM_Q[sys_size + ieq + 3 : sys_size + ieq + 5] += self.scalingFactor * np.dot(
            skew_rot_axis[[row0, row1], :], MB_tstep[self.body_FoR].for_vel[3:6]
        )
...
class zero_lin_vel_sine_rot_vel_FoR(BaseLagrangeConstraint):
    ...
    def initialise(self, MBdict_entry, ieq, print_info=True):
        ...
        if MBdict_entry["xyz"] == "x":
            ...
            raise NotImplementedError(
                "FoR rotation velocity shouldd be parallel to x, y or z"
            )
        self._ieq = ieq
        self.scalingFactor = set_value_or_default(MBdict_entry, "scalingFactor", 1.0)
        self.penaltyFactor = set_value_or_default(MBdict_entry, "penaltyFactor", 0.0)
...
class lin_vel_node_wrtA(BaseLagrangeConstraint):
    ...
    def staticmat(
        ...
        LM_K[sys_size + ieq : sys_size + ieq + num_LM_eq_specific, :sys_size] += (
            self.scalingFactor * B
        )
        LM_K[
            :sys_size, sys_size + ieq : sys_size + ieq + num_LM_eq_specific
        ] += self.scalingFactor * np.transpose(B)
...
class lin_vel_node_wrtG(BaseLagrangeConstraint):
    ...
    def staticmat(
        ...
        B[:num_LM_eq_specific, node_dof : node_dof + 3] = MB_tstep[
            self.body_number
        ].cga()
        ...
        LM_Q[
            ...
            np.dot(
                MB_beam[self.body_number].ini_info.cga(),
                MB_beam[self.body_number].ini_info.pos[self.node_number, :],
            )
    ...
    def dynamicmat(
        ...
        if MB_beam[self.body_number].FoR_movement == "free":
            Bnh[:num_LM_eq_specific, FoR_dof : FoR_dof + 3] = MB_tstep[
                self.body_number
            ].cga()
            Bnh[:num_LM_eq_specific, FoR_dof + 3 : FoR_dof + 6] = -np.dot(
                MB_tstep[self.body_number].cga(),
                ag.skew(MB_tstep[self.body_number].pos[self.node_number, :]),
            )
        Bnh[:num_LM_eq_specific, node_dof : node_dof + 3] = MB_tstep[
            self.body_number
        ].cga()
        ...
        if MB_beam[self.body_number].FoR_movement == "free":
            LM_C[
                ...
                MB_tstep[self.body_number].quat,
                Lambda_dot[ieq : ieq + num_LM_eq_specific],
            )
            LM_C[
                node_dof : node_dof + 3, FoR_dof + 6 : FoR_dof + 10
            ...
            LM_K[
                FoR_dof + 3 : FoR_dof + 6, node_dof : node_dof + 3
            ] -= self.scalingFactor * ag.skew(
                np.dot(
                    MB_tstep[self.body_number].cga().T,
                    Lambda_dot[ieq : ieq + num_LM_eq_specific],
                )
            )
...
def initialize_constraints(MBdict):
    ...
    for iconstraint in range(num_constraints):
        lc_list.append(
            lc_from_string(MBdict["constraint_%02d" % iconstraint]["behaviour"])()
        )
        MBdict_entry = MBdict["constraint_%02d" % iconstraint]
        if "penaltyFactor" in MBdict_entry.keys():
            if not MBdict_entry["penaltyFactor"] == 0.0:
                raise NotImplementedError(
                    "Penalty method not completely implemented for Lagrange Constraints"
                )
...
def generate_lagrange_matrix(
    ...
    # Initialize matrices
    LM_C = np.zeros(
        (sys_size + num_LM_eq, sys_size + num_LM_eq), dtype=ct.c_double, order="F"
    )
    LM_K = np.zeros(
        (sys_size + num_LM_eq, sys_size + num_LM_eq), dtype=ct.c_double, order="F"
    )
