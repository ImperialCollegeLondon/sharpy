...
try:
    ...
    xbeamlib = ct_utils.import_ctypes_lib(SharpyDir + "/lib/xbeam/lib", "libxbeam")
...
def xbeam_solv_couplednlndyn(beam, settings):
    ...
    pos_def_history = np.zeros(
        (n_tsteps.value, beam.num_node, 3), order="F", dtype=ct.c_double
    )
    pos_dot_def_history = np.zeros(
        (n_tsteps.value, beam.num_node, 3), order="F", dtype=ct.c_double
    )
    psi_def_history = np.zeros(
        (n_tsteps.value, beam.num_elem, 3, 3), order="F", dtype=ct.c_double
    )
    psi_dot_def_history = np.zeros(
        (n_tsteps.value, beam.num_elem, 3, 3), order="F", dtype=ct.c_double
    )

    dynamic_force = np.zeros(
        (n_nodes.value, 6, n_tsteps.value), dtype=ct.c_double, order="F"
    )
...
def xbeam_step_couplednlndyn(beam, settings, ts, tstep=None, dt=None):
    ...
    f_xbeam_solv_nlndyn_step_python(
        ct.byref(numdof),
        ct.byref(ctypes_ts),
        ct.byref(n_elem),
        ct.byref(n_nodes),
...
def xbeam_solv_state2disp(beam, tstep, cbeam3=False):
    ...
    if not cbeam3:
        tstep.for_pos[0:3] = tstep.q[numdof : numdof + 3].astype(
            dtype=ct.c_double, order="F", copy=True
        )
        tstep.for_vel = tstep.dqdt[numdof : numdof + 6].astype(
            dtype=ct.c_double, order="F", copy=True
        )
        # tstep.for_acc = tstep.dqddt[numdof:numdof+6].astype(dtype=ct.c_double, order='F', copy=True)
        tstep.quat = algebra.unit_vector(tstep.dqdt[numdof + 6 :]).astype(
            dtype=ct.c_double, order="F", copy=True
        )
...
def xbeam_solv_state2accel(beam, tstep, cbeam3=False):
    ...
    if not cbeam3:
        tstep.for_acc = tstep.dqddt[numdof : numdof + 6].astype(
            dtype=ct.c_double, order="F", copy=True
        )
...
def cbeam3_asbly_dynamic(beam, tstep, settings):
    ...
    f_cbeam3_asbly_dynamic_python(
        ct.byref(ct.c_int(num_dof)),
        ct.byref(n_nodes),
        ct.byref(n_elem),
        ct.byref(dt),
        beam.ini_info.pos.ctypes.data_as(doubleP),
...
def cbeam3_correct_gravity_forces(beam, tstep, settings):
    ...
    # library load
    f_cbeam3_correct_gravity_forces_python = (
        xbeamlib.cbeam3_correct_gravity_forces_python
    )
...
def xbeam_step_coupledrigid(beam, settings, ts, tstep=None, dt=None):
    ...
    f_xbeam_solv_rigid_step_python(
        ct.byref(numdof),
        ct.byref(ctypes_ts),
        ct.byref(n_elem),
        ct.byref(n_nodes),
