...
class Polar:
    ...
    def redefine_aoa(self, new_aoa):
        ...
        for icol in range(1, 4):
            table[:, icol] = np.interp(
                table[:, 0], self.table[:, 0], self.table[:, icol]
            )
    ...
    def get_cdcm_from_cl(self, cl):
        ...
        if cl_max < cl or cl_min > cl:
            print(
                "cl = %.2f out of range, forces at this point will not be corrected"
                % cl
            )
            cd = 0.0
            cm = 0.0
            ...
            if cl == 0.0:
                ...
                while self.table[i, 1] < cl:
                    i += 1
                cd = np.interp(
                    cl, self.table[i - 1 : i + 1, 1], self.table[i - 1 : i + 1, 2]
                )
                cm = np.interp(
                    cl, self.table[i - 1 : i + 1, 1], self.table[i - 1 : i + 1, 3]
                )
            else:
                dist = np.abs(self.table[:, 0] - self.aoa_cl0_deg)
                min_dist = np.min(dist)
...
def interpolate(polar1, polar2, coef=0.5):
    ...
    table = (1.0 - coef) * new_polar1.table + coef * new_polar2.table
