...
class Aerogrid(object):
    ...
    def generate(self, aero_dict, beam, aero_settings, ts):
        ...
        self.ini_info = AeroTimeStepInfo(
            self.aero_dimensions, self.aero_dimensions_star
        )
        ...
        for i_elem in range(self.n_elem):
            for i_local_node in range(self.beam.num_node_elem):
                try:
                    self.airfoil_db[
                        self.aero_dict["airfoil_distribution"][i_elem, i_local_node]
                    ]
                except KeyError:
                    airfoil_coords = self.aero_dict["airfoils"][
                        str(
                            self.aero_dict["airfoil_distribution"][i_elem, i_local_node]
                        )
                    ]
        ...
        if len(aero_settings["control_surface_deflection"]) != self.n_control_surfaces:
            undef_ctrl_sfcs = [""] * (
                ...
                - len(aero_settings["control_surface_deflection"])
            )
        ...
        for i_cs in range(self.n_control_surfaces):
            if aero_settings["control_surface_deflection"][i_cs] == "":
                ...
                cout.cout_wrap(
                    "Initialising Control Surface {:g} generator".format(i_cs), 1
                )
                # check that the control surface is not static
                if self.aero_dict["control_surface_type"][i_cs] == 0:
                    raise TypeError(
                        "Control surface {:g} is defined as static but there is a"
                        " control surface generatorassociated with it".format(i_cs)
                    )
                ...
                try:
                    self.cs_generators[i_cs].initialise(
                        aero_settings["control_surface_deflection_generator_settings"][
                            str(i_cs)
                        ]
                    )
                except KeyError:
                    ...
                    cout.cout_wrap(
                        "Error, unable to locate a settings dictionary for control"
                        " surface {:g}".format(i_cs),
                        4,
                    )

        if with_error_initialising_cs:
            raise KeyError(
                "Unable to locate settings for at least one control surface."
            )
    ...
    def output_info(self):
        ...
        cout.cout_wrap(
            "  In total: %u wake panels"
            % (sum(self.aero_dimensions_star[:, 0] * self.aero_dimensions_star[:, 1]))
        )
        cout.cout_wrap(
    ...
    def generate_zeta_timestep_info(
        self, structure_tstep, aero_tstep, beam, aero_settings, it=None, dt=None
    ):
        if it is None:
        ...
        if "first_twist" not in self.aero_dict:
            self.aero_dict["first_twist"] = [True] * self.aero_dict["surface_m"].shape[
                0
            ]
        ...
        for i_elem in range(self.n_elem):
            ...
            for i_local_node in range(
                len(self.beam.elements[i_elem].global_connectivities)
            ):
                ...
                if i_n == -1 or ii_surf == -1:
                    raise AssertionError(
                        "Error 12958: Something failed with the mapping in aerogrid.py."
                        " Check/report!"
                    )
                ...
                if with_control_surfaces:
                    ...
                    if self.aero_dict["control_surface"][i_elem, i_local_node] >= 0:
                        i_control_surface = self.aero_dict["control_surface"][
                            i_elem, i_local_node
                        ]
                        # 2) type of control surface + write info
                        control_surface_info = dict()
                        if (
                            self.aero_dict["control_surface_type"][i_control_surface]
                            == 0
                        ):
                            control_surface_info["type"] = "static"
                            control_surface_info["deflection"] = self.aero_dict[
                                "control_surface_deflection"
                            ][i_control_surface]
                            control_surface_info["chord"] = self.aero_dict[
                                "control_surface_chord"
                            ][i_control_surface]
                            try:
                                control_surface_info["hinge_coords"] = self.aero_dict[
                                    "control_surface_hinge_coords"
                                ][i_control_surface]
                            except KeyError:
                            ...
                            params = {"it": it}
                            (
                                control_surface_info["deflection"],
                            ...
                            try:
                                old_deflection = self.data.aero.timestep_info[
                                    -1
                                ].control_surface_deflection[i_control_surface]
                            except AttributeError:
                                try:
                                    old_deflection = (
                                        aero_tstep.control_surface_deflection[
                                            i_control_surface
                                        ]
                                    )
                                except IndexError:
                                    old_deflection = self.aero_dict[
                                        "control_surface_deflection"
                                    ][i_control_surface]
                            ...
                            if dt is not None:
                                control_surface_info["deflection_dot"] = (
                                    control_surface_info["deflection"] - old_deflection
                                ) / dt
                            else:
                            ...
                            raise NotImplementedError(
                                str(
                                    self.aero_dict["control_surface_type"][
                                        i_control_surface
                                    ]
                                )
                ...
                node_info["eaxis"] = self.aero_dict["elastic_axis"][
                    i_elem, i_local_node
                ]
                ...
                node_info["M_distribution"] = self.aero_dict["m_distribution"].decode(
                    "ascii"
                )
                node_info["airfoil"] = self.aero_dict["airfoil_distribution"][
                    i_elem, i_local_node
                ]
                ...
                node_info["for_delta"] = beam.frame_of_reference_delta[
                    i_elem, i_local_node, :
                ]
                ...
                if node_info["M_distribution"].lower() == "user_defined":
                    ...
                    node_info["user_defined_m_distribution"] = self.aero_dict[
                        "user_defined_m_distribution"
    ...
    def generate_zeta(self, beam, aero_settings, ts=-1, beam_ts=-1):
        self.generate_zeta_timestep_info(
            beam.timestep_info[beam_ts], self.timestep_info[ts], beam, aero_settings
        )
    ...
    def generate_mapping(self):
        ...
        for i_elem in range(self.n_elem):
            ...
            if i_surf == -1:
                continue
            for i_global_node in self.beam.elements[
                i_elem
            ].reordered_global_connectivities:
                if not self.aero_dict["aero_node"][i_global_node]:
                    continue
                ...
                self.struct2aero_mapping[i_global_node].append(
                    {"i_surf": i_surf, "i_n": i_n}
                )
    ...
    def compute_gamma_dot(dt, tstep, previous_tsteps):
        ...
        if part_of_fsi:
            for i_surf in range(tstep.n_surf):
                tstep.gamma_dot[i_surf] = (
                    tstep.gamma[i_surf] - previous_tsteps[-1].gamma[i_surf]
                ) / dt
        else:
            for i_surf in range(tstep.n_surf):
                tstep.gamma_dot[i_surf] = (
                    tstep.gamma[i_surf] - previous_tsteps[-2].gamma[i_surf]
                ) / dt
...
def generate_strip(
    ...
    if node_info["control_surface"] is not None:
        b_frame_hinge_coords = strip_coordinates_b_frame[
        ...
        for i_M in range(
            node_info["M"] - node_info["control_surface"]["chord"], node_info["M"] + 1
        ):
            relative_coords = strip_coordinates_b_frame[:, i_M] - b_frame_hinge_coords
            # rotate the control surface
            relative_coords = np.dot(
                algebra.rotation3d_x(-node_info["control_surface"]["deflection"]),
                relative_coords,
            )
            # deflection velocity
            try:
                cs_velocity[:, i_M] += np.cross(
                    np.array(
                        [-node_info["control_surface"]["deflection_dot"], 0.0, 0.0]
                    ),
                    relative_coords,
                )
            except KeyError:
    ...
    for i_M in range(node_info["M"] + 1):
        if first_twist:
            strip_coordinates_b_frame[:, i_M] = np.dot(
                c_sweep, np.dot(Crot, np.dot(Ctwist, strip_coordinates_b_frame[:, i_M]))
            )
        else:
            strip_coordinates_b_frame[:, i_M] = np.dot(
                Ctwist, np.dot(Crot, np.dot(c_sweep, strip_coordinates_b_frame[:, i_M]))
            )
    ...
    if calculate_zeta_dot:
        ...
        for i_M in range(node_info["M"] + 1):
            zeta_dot_a_frame[:, i_M] += np.dot(
                algebra.skew(omega_a), strip_coordinates_a_frame[:, i_M]
            )
    ...
    delta_c = (
        strip_coordinates_a_frame[:, -1] - strip_coordinates_a_frame[:, 0]
    ) / node_info["M"]
    if node_info["M_distribution"] == "uniform":
        for i_M in range(node_info["M"] + 1):
        ...
        warnings.warn(
            "No quarter chord disp of grid for non-uniform grid distributions"
            " implemented",
            UserWarning,
        )
    ...
    for i_M in range(node_info["M"] + 1):
        ...
        zeta_dot_a_frame[:, i_M] = np.dot(node_info["cga"], zeta_dot_a_frame[:, i_M])
