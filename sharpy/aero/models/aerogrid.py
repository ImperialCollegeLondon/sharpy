...
class Aerogrid(Grid):
    ...
    def generate(self, data_dict, beam, settings, ts):
        ...
        if settings['gust_vanes']:
            generator_type = gen_interface.generator_from_string('GustVanes')
            self.gust_vanes = generator_type()
            self.gust_vanes.initialise(settings['gust_vanes_generator_settings'])
            self.ini_info = AeroTimeStepInfo(np.concatenate((self.dimensions, self.gust_vanes.dimensions)),
                                             np.concatenate((self.dimensions_star, self.gust_vanes.dimensions_star)),
    ...
    def generate_zeta_timestep_info(self, structure_tstep, aero_tstep, beam, settings, it=None, dt=None):
        ...
        if self.aero_settings['gust_vanes']:
            self.gust_vanes.generate_zeta(it,
                                          aero_tstep,
                                          self.airfoil_db,
                                          freestream_dir = self.aero_settings['freestream_dir'])
