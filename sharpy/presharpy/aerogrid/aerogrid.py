...
class AeroGrid(object):
    def __init__(self, beam, aero_dict, settings):
        ...
        for i_node in range(self.total_nodes):
            ...
            node_info['M_distribution'] = self.settings['m_distribution']
            ...
            generate_strip(node_info, self.airfoil_db)


def generate_strip(node_info, airfoil_db):
    strip_coordinates_a_frame = np.zeros((node_info['M'] + 1, 3), dtype=ct.c_double)

    # airfoil coordinates
    if node_info['M_distribution'] == 'uniform':
        x_interp = np.linspace(0.0, 1.0, node_info['M'] + 1)
    else:
        raise NotImplemented('M_distribution is ' + node_info['M_distribution'] +
                             ' and it is not yet supported')
    y_interp = airfoil_db[node_info['airfoil']](x_interp)
