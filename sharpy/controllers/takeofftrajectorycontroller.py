...
class TakeOffTrajectoryController(controller_interface.BaseController):
    ...
    settings_description["trajectory_input_file"] = (
        "Route and file name of the trajectory file given as a csv with columns: time,"
        " x, y, z"
    )
    ...
    settings_description["controlled_constraint"] = (
        "Name of the controlled constraint in the multibody context"
    ...
    def control(self, data, controlled_state):
        ...
        try:
            constraint = controlled_state["structural"].mb_dict[
        ...
        if end_of_traj:
            lc.remove_constraint(
                controlled_state["structural"].mb_dict,
                self.settings["controlled_constraint"],
            )

            if not self.settings["free_trajectory_structural_solver"] == "":
                controlled_state["info"]["structural_solver"] = self.settings[
                    "free_trajectory_structural_solver"
                ]

            controlled_state["info"]["structural_substeps"] = self.settings[
        ...
        if self.settings["write_controller_log"]:
            self.log.write(
                ("{:>6d}," + 3 * "{:>12.6f}," + "{:>12.6f}\n").format(
                    i_current, time, traj_command[0], traj_command[1], traj_command[2]
                )
            )

        if self.settings["initial_ramp_length_structural_substeps"] >= 0:
            if i_current < self.settings["initial_ramp_length_structural_substeps"]:
                controlled_state["info"]["structural_substeps"] = ct.c_int(
                    i_current - 1
                )
            elif i_current == self.settings["initial_ramp_length_structural_substeps"]:
