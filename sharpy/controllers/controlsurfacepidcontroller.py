...
class ControlSurfacePidController(controller_interface.BaseController):
    ...
    settings_default["controlled_surfaces_coeff"] = [1.0]
    ...
    def initialise(self, data, in_dict, controller_id=None, restart=False):
        ...
        if not valid:
            cout.cout_wrap(
                "The input_type {} is not supported by {}".format(
                    self.settings["input_type"], self.controller_id
                ),
                3,
            )
        ...
        try:
            self.prescribed_input_time_history = np.loadtxt(
                self.settings["time_history_input_file"], delimiter=","
            )
        except OSError:
            raise OSError(
                "File {} not found in Controller".format(
                    self.settings["time_history_input_file"]
                )
            )
        ...
        if len(self.settings["controlled_surfaces_coeff"]) == self.n_control_surface:
            # All good, pass checks
            pass
        elif (
            len(self.settings["controlled_surfaces_coeff"]) == 1
            and self.settings["controlled_surfaces_coeff"][0] == 1.0
        ):
            ...
            raise ValueError(
                "controlled_surfaces_coeff does not have as many"
    ...
    def control(self, data, controlled_state):
        ...
        self.real_state_input_history.append(
            self.extract_time_history(controlled_state)
        )
    ...
    def extract_time_history(self, controlled_state):
        ...
        if self.settings["input_type"] == "pitch":
            ...
            raise NotImplementedError(
                "input_type {} is not yet implemented in extract_time_history()".format(
                    self.settings["input_type"]
                )
            )
        return output
