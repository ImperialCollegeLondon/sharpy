...
class BladePitchPid(controller_interface.BaseController):
    ...
    settings_default["lp_cut_freq"] = 0.0
    settings_description["lp_cut_freq"] = (
        "Cutting frequency of the low pass filter of the process value in Hz. Choose 0"
        " for no filter"
    )

    settings_types["anti_windup_lim"] = "list(float)"
    settings_default["anti_windup_lim"] = [-1.0, -1.0]
    settings_description["anti_windup_lim"] = (
        "Limits of actuation to apply anti windup."
    ...
    settings_default["blade_num_body"] = [
        0,
    ]
    ...
    settings_description[
        "gen_model_const_var"
    ] = "Generator metric to be kept constant at a value `target_gen_value`"
    ...
    ] = "Constant value of the generator metric to be kept constant "

    settings_types["GBR"] = "float"
    settings_default["GBR"] = 97.0
    ...
    settings_description[
        "newmark_damp"
    ] = "Damping of the time integration newmark-beta scheme"
    ...
    def initialise(self, data, in_dict, controller_id=None, restart=False):
        ...
        settings.to_custom_types(
            self.in_dict, self.settings_types, self.settings_default, no_ctype=True
        )
        ...
        if self.settings["sp_source"] == "file":
            self.prescribed_sp_time_history = np.loadtxt(
                self.settings["sp_time_history_file"]
            )
        # Init PID controller
        self.controller_implementation = control_utils.PID(
            self.settings["P"],
            self.settings["I"],
            self.settings["D"],
            self.settings["dt"],
        ...
        if (
            not self.settings["anti_windup_lim"][0]
            == self.settings["anti_windup_lim"][1]
        ):
            self.controller_implementation.set_anti_windup_lim(
                self.settings["anti_windup_lim"]
            )

        if self.settings["lp_cut_freq"] == 0.0:
            self.filter_pv = False
        else:
            self.filter_pv = True
            w0 = self.settings["lp_cut_freq"] * 2 * np.pi
            self.filter = TransferFunction(np.array([w0]), np.array([1.0, w0]))
            self.min_it_filter = int(
                1.0 / (self.settings["lp_cut_freq"] * self.settings["dt"])
            )
    ...
    def control(self, data, controlled_state):
        ...
        if len(self.system_pv) == 0:
            for it in range(data.ts - 1):
                self.system_pv.append(0.0)
                self.prescribed_sp.append(0.0)
        ...
        if True:
            ...
            self.rotor_vel, self.rotor_acc = self.drive_train_model(
                aero_torque, self.rotor_vel, self.rotor_acc
            )
        else:
        ...
        sys_pv = self.compute_system_pv(
            struct_tstep, data.structure, gen_vel=self.rotor_vel * self.settings["GBR"]
        )
        ...
        if self.filter_pv and (len(self.system_pv) > self.min_it_filter):
            ...
            T, filtered_pv, xout = forced_response(
                self.filter, T=time, U=self.system_pv
            )
        else:
        ...
        if change_quat:
            for ibody in self.settings["blade_num_body"]:
                quat = algebra.rotate_quaternion(
                    struct_tstep.mb_quat[ibody, :],
                    delta_pitch * np.array([1.0, 0.0, 0.0]),
                )
        ...
        if change_vel:
            for ibody in self.settings["blade_num_body"]:
                ...
                struct_tstep.mb_FoR_acc[ibody, 3] = (
                    data.structure.timestep_info[data.ts - 1].mb_FoR_vel[ibody, 3]
                    - struct_tstep.mb_FoR_vel[ibody, 3]
                ) / self.settings["dt"]
                if ibody == 0:
                    struct_tstep.for_vel[3] = struct_tstep.mb_FoR_vel[ibody, 3]
                    struct_tstep.for_acc[3] = struct_tstep.mb_FoR_acc[ibody, 3]

            data.structure.dynamic_input[data.ts - 1][
                "for_vel"
            ] = struct_tstep.for_vel.copy()
            data.structure.dynamic_input[data.ts - 1][
                "for_acc"
            ] = struct_tstep.for_acc.copy()

        if False:
            data.aero.generate_zeta_timestep_info(
                struct_tstep,
                aero_tstep,
                data.structure,
    ...
    def compute_system_pv(self, struct_tstep, beam, **kwargs):
        ...
        if self.settings["sp_type"] == "pitch":
            pitch = algebra.quat2euler(
                struct_tstep.mb_quat[self.settings["blade_num_body"][0]]
            )[0]
            self.system_pv.append(pitch)
        elif self.settings["sp_type"] == "rbm":
            steady, unsteady, grav = struct_tstep.extract_resultants(
                beam, force_type=["steady", "unsteady", "gravity"], ibody=0
            )
    ...
    def drive_train_model(self, aero_torque, ini_rot_vel, ini_rot_acc):
        ...
        if self.settings["gen_model_const_var"] == "power":
            gen_torque = (
                self.settings["gen_model_const_value"]
                / self.settings["GBR"]
                / ini_rot_vel
            )
        ...
        rot_acc = (aero_torque - self.settings["GBR"] * gen_torque) / self.settings[
            "inertia_dt"
        ]
    ...
    def compute_aero_torque(self, beam, struct_tstep):
        ...
        for ibody in self.settings["blade_num_body"]:
            steady, unsteady, grav = struct_tstep.extract_resultants(
                beam, force_type=["steady", "unsteady", "grav"], ibody=ibody
            )
            # total_forces += steady + unsteady + grav
            total_forces += steady + unsteady

        # Compute equivalent forces at hub position
        hub_elem = np.where(beam.body_number == self.settings["blade_num_body"][0])[0][
            0
        ]
    ...
    def compute_blade_pitch(beam, struct_tstep, tower_ibody=0, blade_ibody=1):
        ...
        zg_tower_top = algebra.multiply_matrices(cga0, ca0b, np.array([0.0, 0.0, 1.0]))

        # blade root
        cga = algebra.quat2rotation(struct_tstep.mb_quat[blade_ibody, :])
        zg_hub = algebra.multiply_matrices(cga, np.array([0.0, 0.0, 1.0]))
