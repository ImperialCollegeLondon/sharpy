...
class HortenWing:
    ...
    def __init__(
        ...
        if case_name_format == 0:
            ...
            self.case_name = case_name + "_u_inf%.4d_%s" % (
                int(u_inf * 100),
                case_remarks,
            )
    ...
    def update_mass_stiffness(self, sigma=1.0):
        ...
        j_root = np.array(
            [[0.30378797, 0.0, 0.0], [0.0, 0.0122422, 0.0], [0.0, 0.0, 0.30016065]]
        )

        j_root = np.diag([0.1, 0.1, 0.2])
        mass_root_right = sclalg.block_diag(
            np.diag([mu_root, mu_root, mu_root]), j_root
        )
        ...
        j_tip = np.array(
            [
                [9.06829766e-04, 0.00000000e00, 0.00000000e00],
                [0.00000000e00, 6.34780836e-05, 0.00000000e00],
                [0.00000000e00, 0.00000000e00, 8.16146789e-04],
            ]
        )
        ...
        for i_elem in range(self.n_elem_wing):
            base_mass[i_elem, :, :] = (
                mass_root_right * (1 - alpha[i_elem]) + mass_tip_right * alpha[i_elem]
            )
        ...
        # Pilot mass
        lumped_mass[2] = 16.06
        lumped_mass_position[2] = np.array([0.0, -0.254, -0.254])
    ...
    def update_fem_prop(self):
        ...
        y[wn : wn + n_node_fuselage] = np.linspace(
            0, fuselage_width / 2, n_node_fuselage
        )
        ...
        app_forces[thrust_nodes[0]] = [0, thrust, 0, 0, 0, 0]
        ...
        y[wn : wn + n_node_wing] = np.linspace(
            fuselage_width / 2, span / 2, n_node_wing + 1
        )[1:]
        x[wn : wn + n_node_wing] = 0 + (
            y[wn : wn + n_node_wing] - fuselage_width / 2
        ) * np.tan(sweep_LE)
        ...
        # coordinates
        y[wn : wn + n_node_fuselage - 1] = np.linspace(
            0, -fuselage_width / 2, n_node_fuselage
        )[1:]
        ...
        app_forces[thrust_nodes[1]] = [0, -thrust, 0, 0, 0, 0]
    ...
    def create_linear_simulation(self, delta_e=None, delta_dot=None):
        ...
        u = np.zeros(
            (
                self.n_tstep,
                n_states_struct + n_inputs_struct + 2 * self.n_control_surfaces,
            )
        )
        # u[0:3, -7] = -1000
        if delta_e is not None:
            ...
            u[
                :,
                n_states_struct
                + self.n_control_surfaces : n_states_struct
    ...
    def generate_linear_sim_files(self, x0, input_vec):
        ...
        with h5.File(
            self.case_route + "/" + self.case_name + ".lininput.h5", "a"
        ) as h5file:
    ...
    def set_default_config_dict(self, route=None, case_name=None):
        ...
        settings["BeamLoader"] = {
            ...
            "orientation": algebra.euler2quat(
                np.array([self.roll, self.alpha, self.beta])
            ),
        }
...
if __name__ == "__main__":
    ws = HortenWing(M=4, N=11, Mstarfactor=5, u_inf=28, rho=1.225, alpha_deg=4)
