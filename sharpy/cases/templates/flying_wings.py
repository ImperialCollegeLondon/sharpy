...
class FlyingWing:
    ...
    def __init__(
        ...
        assert N % 2 != 1, "UVLM spanwise panels must be even when using 3-noded FEs!"
        ...
        ### other params
        self.u_inf_direction = np.array([1.0, np.sin(beta * np.pi / 180), 0.0])
        ...
        self.control_surface_hinge_coord = np.zeros_like(
            self.control_surface_type, dtype=float
        )
    ...
    def load_polar(self, file):
        ...
        self.polars = np.column_stack(
            (
                polar_raw_data[:, 0] * np.pi / 180,  # aoa
                polar_raw_data[:, 1],  # cl
                polar_raw_data[:, 2],  # cd
                polar_raw_data[:, 4],
            )
    ...
    def update_derived_params(self):
        ...
        self.roll_rad = self.roll * np.pi / 180.0
        self.beta_rad = self.beta * np.pi / 180.0
        self.sweep_rad = self.sweep * np.pi / 180.0
        self.yaw_rad = self.yaw * np.pi / 180.0

        # FoR A orientation
        # if self.RollNodes:
        self.quat = algebra.euler2quat(
            np.array([self.roll_rad, self.alpha_rad, self.yaw_rad])
        )
        # else:
        ...
        assert (
            self.num_elem_tot % self.n_surfaces != 1
        ...
        self.control_surface = (
            np.zeros((self.num_elem_tot, self.num_node_elem), dtype=int) - 1
        )
    ...
    def update_fem_prop(self):
        ...
        if n_surfaces == 1:
            ...
            x01 = np.sin(sweep_rad) * np.linspace(
                0.0, half_span, (num_node_surf + 1) // 2
            )
            y01 = np.cos(sweep_rad) * np.linspace(
                0.0, half_span, (num_node_surf + 1) // 2
            )
            # and mirrow
        ...
        if n_surfaces == 2:
            ...
            x01 = np.sin(sweep_rad) * np.linspace(0.0, half_span, num_node_surf)
            y01 = np.cos(sweep_rad) * np.linspace(0.0, half_span, num_node_surf)
            # and mirrow
    ...
    def update_aero_prop(self):
        ...
        if n_surfaces == 2:
            ...
            airfoil_distribution = np.concatenate(
                [airfoil_distribution_surf, airfoil_distribution_surf[::-1, [1, 0, 2]]]
            )
        if n_surfaces == 1:
            ...
            airfoil_distribution = np.concatenate(
                [airfoil_distribution_surf[::-1, [1, 0, 2]], airfoil_distribution_surf]
            )
    ...
    def set_default_config_dict(self):
        ...
        config["AsymptoticStability"] = {
            "print_info": True,
            "velocity_analysis": [30, 180, 151],
        }

        config["LinDynamicSim"] = {
            "dt": self.dt,
            ...
            if self.polars is not None:
                ...
                for i_airfoil in range(len(self.Airfoils_surf)):
                    polars_group.create_dataset(
                        "{:g}".format(i_airfoil), data=self.polars
                    )
    ...
    def generate_fem_file(self):
        ...
        with h5.File(self.route + "/" + self.case_name + ".fem.h5", "a") as h5file:
            ...
            num_nodes_handle = h5file.create_dataset("num_node", data=self.num_node_tot)
            ...
            beam_handle = h5file.create_dataset("beam_number", data=self.surface_number)
    ...
    def generate_rom_files(self, left_tangent, right_tangent, ro, rc, fo, fc):
        with h5.File(self.route + "/" + self.case_name + ".rom.h5", "a") as h5file:
            lt_handle = h5file.create_dataset("left_tangent", data=left_tangent)
            rt_handle = h5file.create_dataset("right_tangent", data=right_tangent)
...
class Smith(FlyingWing):
    ...
    def __init__(
        self,
        M,
        N,  # chord/span-wise discretisations
        Mstar_fact,
        u_inf,  # flight cond
        alpha,
        rho=0.08891,
    ...
    def update_mass_stiff(self):
        ...
        self.mass = np.zeros((1, 6, 6))
        self.mass[0, :, :] = np.diag([0.75, 0.75, 0.75, 0.1, 0.1, 0.1])
...
class Goland(FlyingWing):
    ...
    def __init__(
        ...
        alpha,
        rho=1.02,
        b_ref=2.0 * 6.096,  # geometry
        main_chord=1.8288,
        aspect_ratio=(2.0 * 6.096) / 1.8288,
        roll=0.0,
        yaw=0.0,
        beta=0.0,
        sweep=0.0,
    ...
    def update_mass_stiff(self):
        ...
        m_unit = 35.71
        j_tors = 8.64
        pos_cg_b = np.array([0.0, self.c_ref * (self.main_cg - self.main_ea), 0.0])
...
class GolandControlSurface(Goland):
    def __init__(
        ...
        self.control_surface_chord = (
            M // 2 * np.ones(self.n_control_surfaces, dtype=int)
        )
        self.control_surface_type = np.zeros(self.n_control_surfaces, dtype=int)
        self.control_surface_hinge_coord = np.zeros_like(
            self.control_surface_type, dtype=float
    ...
    def create_linear_files(self, x0, input_vec):
        with h5.File(self.route + "/" + self.case_name + ".lininput.h5", "a") as h5file:
            x0 = h5file.create_dataset("x0", data=x0)
            u = h5file.create_dataset("u", data=input_vec)
...
class QuasiInfinite(FlyingWing):
    ...
    def update_mass_stiff(self):
        ...
        m_unit = 1.0
        self.mass = np.zeros((1, 6, 6))
        self.mass[0, :, :] = np.diag([m_unit, m_unit, m_unit, 1.0, 0.5, 0.5])
        self.elem_stiffness = np.zeros((self.num_elem_tot,), dtype=int)
        self.elem_mass = np.zeros((self.num_elem_tot,), dtype=int)
...
class Pazy(FlyingWing):
    ...
    def __init__(
        ...
        rho=1.225,
        tip_rod=True,
        b_ref=2.0 * 0.55,  # geometry
        main_chord=0.1,
        aspect_ratio=(2.0 * 0.55) / 0.1,
        roll=0.0,
        yaw=0.0,
        beta=0.0,
        sweep=0.0,
    ...
    def update_mass_stiff(self):
        ...
        base_stiffness = (
            np.diag([ea, ga_oup, ga_inp, gj, ei_oup, ei_inp]) * sigma_scale_stiff
        )
        ...
        pos_cg_b = (
            np.array([0.0, self.c_ref * (self.main_cg - self.main_ea), 0.0])
            * sigma_scale_I
        )
        m_chi_cg = algebra.skew(m_unit * pos_cg_b)
        self.mass = np.zeros((1, 6, 6))
        ...
        self.mass[0, :, :] = (
            np.diag([m_unit, m_unit, m_unit, Js, 0.5 * Js, 12 * Js]) * sigma_scale_I
        )
        self.mass[0, :3, 3:] = m_chi_cg
        self.mass[0, 3:, :3] = -m_chi_cg
        ...
        if self.tip_rod:
            ...
            self.lumped_mass_nodes[0] = self.N // 2
            self.lumped_mass_inertia[0, :, :] = np.diag(
                [1.2815e-04, 2.87e-07, 1.17e-04]
            )
            ...
            self.lumped_mass_position[1] = np.array(
                [-0.005, -0.005, 0]
            )  # positive x now ascending towards root
            self.lumped_mass_nodes[1] = self.N // 2 + 1
            self.lumped_mass_inertia[1, :, :] = np.diag(
                [1.2815e-04, 2.87e-07, 1.17e-04]
            )
...
if __name__ == "__main__":
    ...
    ws = Goland(M=4, N=20, Mstar_fact=12, u_inf=25.0, alpha=2.0, route="./test")
    ws.clean_test_files()
    ws.update_derived_params()
    ws.generate_fem_file()
    ws.generate_aero_file()
