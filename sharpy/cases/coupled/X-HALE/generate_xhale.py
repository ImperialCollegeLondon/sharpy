...
for case in cases:
    ...
    if vertical_tail:
        ...
        beta = 0.0 * np.pi / 180  # sideslip angle  [rad]
    ...
    def read_beam_data(filename=route + "/inputs/beam_properties.xlsx"):
        ...
        # mass
        mass_sheet = pd.read_excel(
            filename, sheet_name="mass", header=1, skip_rows=1, index_col=0
        )
        # remove units
        ...
        for index, row in mass_sheet.iterrows():
            ...
            mass_data[index]["full_matrix"] = model_utils.mass_matrix_generator(
                mass_data[index]["mass"],
                mass_data[index]["xcg"],
                mass_data[index]["inertia"],
            )
        # stiffness
        stiff_sheet = pd.read_excel(
            filename, sheet_name="stiffness", header=1, skip_rows=1, index_col=0
        )
        # remove units
    ...
    def read_lumped_mass_data(filename=route + "/inputs/lumped_mass.xlsx"):
        ...
        sheets = {
            sheet_name: xl.parse(sheet_name, header=0, skiprows=(0, 2))
    ...
    def generate_fem():
        ...
        for i in range(len(lumped_mass_indices[lumped_mass_id])):
            ...
            lumped_mass_position[lumped_mass_indices[lumped_mass_id][i]] = np.dot(
                rotation_mat_x,
                np.dot(
                    np.eye(3),
        ...
        app_forces[0, 0:3] = thrustC * np.array(
            [0.0, np.cos(in_structural_twist), -np.sin(in_structural_twist)]
        )
        thrust_nodes[0] = 0
        ...
        for ielem in range(n_elem_section):
            ...
            conn[we + ielem, :] = (
                np.ones((3,)) * (we + ielem) * (n_node_elem - 1)
            ) + np.array([0, 2, 1])
        ...
        app_forces[wn - 1, 0:3] = thrustC * np.array(
            [0.0, np.cos(in_structural_twist), -np.sin(in_structural_twist)]
        )
        thrust_nodes[1] = wn - 1

        beam_number[we : we + n_elem_section] = 1
        y[wn : wn + n_node_section - 1] = (
            y[wn - 1] + np.linspace(0.0, span_section, n_node_section)[1:]
        )
        structural_twist[we : we + n_elem_section, :] = in_structural_twist
        for ielem in range(n_elem_section):
            conn[we + ielem, :] = (
        ...
        app_forces[wn - 1, 0:3] = (
            thrustC
            * (1 + differential)
            * np.array([0.0, np.cos(in_structural_twist), -np.sin(in_structural_twist)])
        )
        thrust_nodes[1] = wn - 1

        beam_number[we : we + n_elem_section_dihedral] = 2
        structural_twist[we : we + n_elem_section_dihedral, :] = in_structural_twist
        y[wn : wn + n_node_section_dihedral - 1] = (
            y[wn - 1]
            + np.linspace(
                0.0, np.cos(dihedral_outer) * span_section, n_node_section_dihedral
            )[1:]
        )
        z[wn : wn + n_node_section_dihedral - 1] = (
            z[wn - 1]
            + np.linspace(
                0.0, np.sin(dihedral_outer) * span_section, n_node_section_dihedral
            )[1:]
        )
        ...
        structural_twist[we : we + n_elem_section, :] = -in_structural_twist
        y[wn : wn + n_node_section - 1] = np.linspace(
            0.0, -span_section, n_node_section
        )[1:]
        for ielem in range(n_elem_section):
            conn[we + ielem, :] = (
                np.ones((3,)) * (we + ielem) * (n_node_elem - 1)
        ...
        x[wn : wn + n_node_centre_tail - 1] = np.linspace(
            0.0, length_centre_tail, n_node_centre_tail
        )[1:]
        ...
        if vertical_tail:
            ...
            z[wn : wn + n_node_tail - 1] = (
                z[wn - 1] + np.linspace(0.0, span_ctail_R, n_node_tail)[1:]
            )
            ...
            y[wn : wn + n_node_tail - 1] = y[wn - 1]
            z[wn : wn + n_node_tail - 1] = (
                z[end_of_centre_tail_node]
                + np.linspace(0.0, -span_ctail_L, n_node_tail)[1:]
            )
            for ielem in range(n_elem_tail):
                conn[we + ielem, :] = (
                    np.ones((3,)) * (we + ielem) * (n_node_elem - 1)
            ...
            tail_beam_numbersC[1] = 7
            x[wn : wn + n_node_tail - 1] = x[wn - 1]
            y[wn : wn + n_node_tail - 1] = (
                y[wn - 1] + np.linspace(0.0, span_ctail_R, n_node_tail)[1:]
            )
            for ielem in range(n_elem_tail):
                conn[we + ielem, :] = (
                    np.ones((3,)) * (we + ielem) * (n_node_elem - 1)
        ...
        x[wn : wn + n_node_outer_tail - 1] = np.linspace(
            0.0, length_outer_tail, n_node_outer_tail
        )[1:]
        ...
        y[wn : wn + n_node_tail - 1] = (
            y[end_tails_nodesR[0]] + np.linspace(0.0, span_tail, n_node_tail)[1:]
        )
        for ielem in range(n_elem_tail):
            conn[we + ielem, :] = (
                np.ones((3,)) * (we + ielem) * (n_node_elem - 1)
        ...
        y[wn : wn + n_node_tail - 1] = (
            y[end_tails_nodesR[1]] + np.linspace(0.0, span_tail, n_node_tail)[1:]
        )
        for ielem in range(n_elem_tail):
            conn[we + ielem, :] = (
                np.ones((3,)) * (we + ielem) * (n_node_elem - 1)
        ...
        y[wn : wn + n_node_tail - 1] = (
            y[end_tails_nodesL[0]] + np.linspace(0.0, span_tail, n_node_tail)[1:]
        )
        for ielem in range(n_elem_tail):
            conn[we + ielem, :] = (
                np.ones((3,)) * (we + ielem) * (n_node_elem - 1)
        ...
        y[wn : wn + n_node_tail - 1] = (
            y[end_tails_nodesL[1]] + np.linspace(0.0, span_tail, n_node_tail)[1:]
        )
        for ielem in range(n_elem_tail):
            conn[we + ielem, :] = (
                np.ones((3,)) * (we + ielem) * (n_node_elem - 1)
        ...
        z[wn : wn + n_node_fin - 1] = (
            z[end_nodesL[0]] + np.linspace(0.0, -span_fin, n_node_fin)[1:]
        )
        for ielem in range(n_elem_fin):
            conn[we + ielem, :] = (
                np.ones((3,)) * (we + ielem) * (n_node_elem - 1)
        ...
        z[wn : wn + n_node_fin - 1] = (
            z[end_nodesR[0]] + np.linspace(0.0, -span_fin, n_node_fin)[1:]
        )
        for ielem in range(n_elem_fin):
            conn[we + ielem, :] = (
                np.ones((3,)) * (we + ielem) * (n_node_elem - 1)
        ...
        z[wn : wn + n_node_fin - 1] = (
            z[end_nodesL[1]] + np.linspace(0.0, -span_fin, n_node_fin)[1:]
        )
        for ielem in range(n_elem_fin):
            conn[we + ielem, :] = (
                np.ones((3,)) * (we + ielem) * (n_node_elem - 1)
        ...
        z[wn : wn + n_node_fin - 1] = (
            z[end_nodesR[1]] + np.linspace(0.0, -span_fin, n_node_fin)[1:]
        )
        for ielem in range(n_elem_fin):
            conn[we + ielem, :] = (
                np.ones((3,)) * (we + ielem) * (n_node_elem - 1)
        ...
        z[wn : wn + n_node_fin - 1] = (
            z[end_of_centre_tail_node] + np.linspace(0.0, -span_vfin, n_node_fin)[1:]
        )
        for ielem in range(n_elem_fin):
            conn[we + ielem, :] = (
                np.ones((3,)) * (we + ielem) * (n_node_elem - 1)
        ...
        z[wn : wn + n_node_fin - 1] = (
            z[end_tails_nodesL[0]] + np.linspace(0.0, -span_vfin, n_node_fin)[1:]
        )
        for ielem in range(n_elem_fin):
            conn[we + ielem, :] = (
                np.ones((3,)) * (we + ielem) * (n_node_elem - 1)
        ...
        z[wn : wn + n_node_fin - 1] = (
            z[end_tails_nodesR[0]] + np.linspace(0.0, -span_vfin, n_node_fin)[1:]
        )
        for ielem in range(n_elem_fin):
            conn[we + ielem, :] = (
                np.ones((3,)) * (we + ielem) * (n_node_elem - 1)
    ...
    def read_aero_data(filename=route + "/inputs/aero_properties.xlsx"):
        ...
        xl = pd.ExcelFile(filename)
        sheets = {
            sheet_name: xl.parse(sheet_name, header=0, index_col=0)
            for sheet_name in xl.sheet_names
        }
    ...
    def generate_aero_file():
        ...
        i_surf = 0
        airfoil_distribution[initial_elem : final_elem + 1, :] = aero_data[
            "airfoil_indices"
        ][aero_data[type]["airfoil"]]
        ...
        ctail_m = m_tail
        elements = np.linspace(0, n_elem - 1, n_elem, dtype=int)[
            beam_number == tail_beam_numbersC[1]
        ]
        i_surf += 1
        for i_elem in elements:
            for i_node in range(n_node_elem):
                airfoil_distribution[i_elem, :] = aero_data["airfoil_indices"][
                    aero_data[type]["airfoil"]
                ]
        ...
        elements = np.linspace(0, n_elem - 1, n_elem, dtype=int)[
            beam_number == tail_beam_numbersC[2]
        ]
        i_surf += 1
        for i_elem in elements:
            for i_node in range(n_node_elem):
        ...
        tail_m = m_tail
        elements = np.linspace(0, n_elem - 1, n_elem, dtype=int)[
            beam_number == tail_beam_numbersR[0, 1]
        ]
        i_surf += 1
        for i_elem in elements:
            for i_node in range(n_node_elem):
        ...
        elements = np.linspace(0, n_elem - 1, n_elem, dtype=int)[
            beam_number == tail_beam_numbersR[0, 2]
        ]
        i_surf += 1
        for i_elem in elements:
            for i_node in range(n_node_elem):
        ...
        type = "0Rtail"
        elements = np.linspace(0, n_elem - 1, n_elem, dtype=int)[
            beam_number == tail_beam_numbersR[1, 1]
        ]
        i_surf += 1
        for i_elem in elements:
            for i_node in range(n_node_elem):
        ...
        elements = np.linspace(0, n_elem - 1, n_elem, dtype=int)[
            beam_number == tail_beam_numbersR[1, 2]
        ]
        i_surf += 1
        for i_elem in elements:
            for i_node in range(n_node_elem):
        ...
        type = "0Ltail"
        elements = np.linspace(0, n_elem - 1, n_elem, dtype=int)[
            beam_number == tail_beam_numbersL[0, 1]
        ]
        i_surf += 1
        for i_elem in elements:
            for i_node in range(n_node_elem):
        ...
        elements = np.linspace(0, n_elem - 1, n_elem, dtype=int)[
            beam_number == tail_beam_numbersL[0, 2]
        ]
        i_surf += 1
        for i_elem in elements:
            for i_node in range(n_node_elem):
        ...
        type = "0Ltail"
        elements = np.linspace(0, n_elem - 1, n_elem, dtype=int)[
            beam_number == tail_beam_numbersL[1, 1]
        ]
        i_surf += 1
        for i_elem in elements:
            for i_node in range(n_node_elem):
        ...
        elements = np.linspace(0, n_elem - 1, n_elem, dtype=int)[
            beam_number == tail_beam_numbersL[1, 2]
        ]
        i_surf += 1
        for i_elem in elements:
            for i_node in range(n_node_elem):
        ...
        with h5.File(route + "/" + case_name + ".aero.h5", "a") as h5file:
            ...
            # add one airfoil
            emx07_main = airfoils_group.create_dataset(
                "0", data=load_airfoil(route + "/inputs/EMX-07_camber.txt")
            )
            flat_tail = airfoils_group.create_dataset(
    ...
    def generate_solver_file():
        ...
        settings["BeamLoader"] = {
            "unsteady": "on",  # it's ok to leave it
            # 'on' for static
        ...
        settings["DynamicCoupled"] = {
            ...
            "postprocessors_settings": {
                "BeamLoads": {},  # you need to specify the dict even if empty
