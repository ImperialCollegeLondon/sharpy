    ...
    try:
        ...
        if sharpy_input_dict is None:
            parser = argparse.ArgumentParser(
                prog="SHARPy",
                description="""This is the executable for Simulation of High Aspect Ratio Planes.\n
            Imperial College London 2021""",
            )
            parser.add_argument(
                "input_filename",
                help="path to the *.sharpy input file",
                type=str,
                default="",
            )
            parser.add_argument(
            ...
            parser.add_argument(
                ...
                "--docs",
                help=(
                    "generates the solver documentation in the specified location. "
                    "Code does not execute if running this flag"
                ),
                action="store_true",
            )
            if args is not None:
        ...
        if args.docs:
            ...
            subprocess.Popen(
                ["make", "html"], stdout=None, cwd=sharpydir.SharpyDir + "/docs"
            )
            ...
            try:
                with open(args.restart, "rb") as restart_file:
                    ...
                    try:
                        ...
                        solvers = dict()
                        cout.cout_wrap(
                            "Solvers not found in Pickle file. Using the settings in"
                            " *.sharpy file."
                        )
                ...
                raise FileNotFoundError(
                    "The file specified for the snapshot                     restart"
                    " (-r) does not exist. Please check."
                )
        ...
        for solver_name in settings["SHARPy"]["flow"]:
            if (
                ...
                or (solver_name not in solvers.keys())
                or (missing_solvers)
            ):
        ...
        cout.cout_wrap(
            "Exception raised, writing error log in %s/error.log" % logdir, 4
        )
        logging.basicConfig(
            filename="%s/error.log" % logdir,
            filemode="w",
