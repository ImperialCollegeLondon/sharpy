...
class NonLinearDynamicCoupledStep(_BaseStructural):
    ...
    def initialise(self, data, custom_settings=None, restart=False):
        ...
        if self.settings["initial_velocity"]:
            new_direction = np.dot(
                self.data.structure.timestep_info[-1].cag(),
                self.settings["initial_velocity_direction"],
            )
            self.data.structure.timestep_info[-1].for_vel[0:3] = (
                new_direction * self.settings["initial_velocity"]
            )

        # generate q, dqdt and dqddt
        xbeamlib.xbeam_solv_disp2state(
            self.data.structure, self.data.structure.timestep_info[-1]
        )
    ...
    def run(self, **kwargs):
        ...
        previous_structural_step = settings_utils.set_value_or_default(
            kwargs, "previous_structural_step", self.data.structure.timestep_info[-1]
        )
        dt = settings_utils.set_value_or_default(kwargs, "dt", self.settings["dt"])

        xbeamlib.xbeam_step_couplednlndyn(
