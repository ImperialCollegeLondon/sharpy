...
class LinDynamicSim(BaseSolver):
    ...
    settings_default["physical_time"] = 2.0
    ...
    def run(self, **kwargs):
        ...
        if len(x0) != ss.states:
            warnings.warn(
                "Number of states in the initial state vector not equal to the number"
                " of states"
            )
        ...
        if u.shape[1] != ss.inputs:
            ...
            cout.cout_wrap(
                ...
                % self.data.linear.linear_system.uvlm.ss.inputs,
                3,
            )
            cout.cout_wrap(
                "Number of beam inputs: %g"
                % self.data.linear.linear_system.beam.ss.inputs,
                3,
            )
            breakpoint()
        ...
        if self.settings["write_dat"]:
            cout.cout_wrap(
                "Writing linear simulation output .dat files to %s" % self.folder
            )
    ...
    def read_files(self):
        self.input_file_name = (
            self.data.settings["SHARPy"]["route"]
            + "/"
...
def state_to_timestep(data, x, u=None, y=None):
    ...
    if (
        data.settings["LinearAssembler"]["linear_system_settings"]["beam_settings"][
            "modal_projection"
        ]
        and data.settings["LinearAssembler"]["linear_system_settings"]["beam_settings"][
            "inout_coords"
        ]
        == "modes"
    ):
    ...
    beam_state = x[
        -data.linear.linear_system.beam.ss.states :
    ]  # after aero all the rest is beam, but should not use
    ...
    (
        ...
        gamma_star,
        gust_state_vec,
    ) = data.linear.linear_system.uvlm.unpack_ss_vector(
        data,
        x_n=aero_state,
        u_aero=aero_input,
    ...
    if data.linear.linear_system.uvlm.gust_assembler:
        ...
        u_ext_gust = gust_assembler.state_to_uext.dot(
            gust_state_vec
        ) + gust_assembler.uin_to_uext.dot(u_in_gust)
    else:
        u_ext_gust = np.array([])

    uvlm_input_variables = LinearVector.transform(Kas.output_variables, InputVariable)
    # Unpack input
    zeta, zeta_dot, u_ext = data.linear.linear_system.uvlm.unpack_input_vector(
        aero_input, u_ext_gust, input_variables=uvlm_input_variables
    )

    current_aero_tstep = data.aero.timestep_info[-1].copy()
    current_aero_tstep.forces = [
        forces[i_surf] + data.linear.tsaero0.forces[i_surf]
        for i_surf in range(len(gamma))
    ]
    current_aero_tstep.gamma = [
        gamma[i_surf] + data.linear.tsaero0.gamma[i_surf]
        for i_surf in range(len(gamma))
    ]
    current_aero_tstep.gamma_dot = [
        gamma_dot[i_surf] + data.linear.tsaero0.gamma_dot[i_surf]
        for i_surf in range(len(gamma))
    ]
    current_aero_tstep.gamma_star = [
        gamma_star[i_surf] + data.linear.tsaero0.gamma_star[i_surf]
        for i_surf in range(len(gamma))
    ]
    ...
    aero_forces_vec = np.concatenate(
        [
            forces[i_surf][:3, :, :].reshape(-1, order="C")
            for i_surf in range(len(forces))
        ]
    )
    ...
    current_struct_step = data.linear.linear_system.beam.unpack_ss_vector(
        x_s, y_s, data.linear.tsstruct0
    )
