...
class DynamicPrescribedCoupled(BaseSolver):
    ...
    def initialise(self, data):
        ...
        self.residual_table.field_length[1] = 6
        self.residual_table.print_header(['ts', 't', 'kiter', 'residual', 'residual velocity'])
    ...
    def run(self):
        ...
        for self.data.ts in range(1, self.settings['n_time_steps'].value + 1):
            ...
            for k in range(self.settings['fsi_substeps'].value + 1):
                ...
                if k > 0:
                    res = (np.linalg.norm(structural_kstep.pos -
                                          previous_kstep.pos) /
                           np.linalg.norm(previous_kstep.pos))
                    if np.isnan(res):
                        res = np.linalg.norm(structural_kstep.pos)

                    res_dot = (np.linalg.norm(structural_kstep.pos_dot -
                ...
                # convergence
                if (k > 0) and (k > self.settings['minimum_steps'].value - 1):
                    if (res < self.settings['fsi_tolerance'].value):
                        if (res_dot < self.settings['fsi_vel_tolerance'].value):
                            break
            ...
            # i_elem = self.data.structure.node_master_elem[11, 0]
            # i_local_node = self.data.structure.node_master_elem[11, 1]
            # cab = algebra.crv2rot(self.data.structure.timestep_info[-1].psi[i_elem, i_local_node, :])
            # print(np.dot(np.transpose(cab),self.data.structure.timestep_info[-1].psi[i_elem, i_local_node, :]))
