...
@solver
class DynamicWeaklyCoupled(BaseSolver):
    """
    The ``DynamicWeaklyCoupled`` solver couples the aerodynamic and structural solvers of choice to march forward in time
    the aeroelastic system's solution.

    Using the ``DynamicWeaklyCoupled`` solver requires that an instance of the ``StaticCoupled`` solver is called in the
    ...
    Attributes:
        settings (dict): Contains the solver's ``settings``. See below for acceptable values:
            ...
            ``structural_substeps``                  ``int``        Number of structural steps per aero step (0 for 1 to 1 simulation).        ``0``
    ...
    """
    solver_id = 'DynamicWeaklyCoupled'
            ...
            for k in range(self.settings['fsi_substeps'].value + 1):
                ...
                for i_substep in range(self.settings['structural_substeps'].value + 1):
                    ...
                    self.map_forces(aero_kstep,
                                    structural_kstep,
                                    force_coeff)
                    structural_kstep.steady_applied_forces[:] = ((coeff)*(structural_kstep.steady_applied_forces) +
                        (1.0 - coeff)*self.data.structure.timestep_info[-1].steady_applied_forces)
                    structural_kstep.unsteady_applied_forces[:] = ((coeff)*(structural_kstep.unsteady_applied_forces) +
