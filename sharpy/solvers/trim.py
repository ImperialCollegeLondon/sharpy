...
class Trim(BaseSolver):
    ...
    settings_description["refine_solution"] = (
        "If ``True`` and the optimiser routine allows for it, the optimiser will try to"
        " improve the solution with hybrid methods"
    )
    ...
    def initialise(self, data, restart=False):
        ...
        self.solver.initialise(
            self.data, self.settings["solver_settings"], restart=restart
        )
        ...
        if self.with_special_case:
            if self.settings["special_case"]["case_name"] == "differential_thrust":
                ...
                counter += 1
                self.x_info["initial_base_thrust"] = self.settings["special_case"][
                    "initial_base_thrust"
                ]
                ...
                self.x_info["base_thrust_nodes"] = [
                    int(e) for e in self.settings["special_case"]["base_thrust_nodes"]
                ]
                self.x_info["negative_thrust_nodes"] = [
                    int(e)
                    for e in self.settings["special_case"]["negative_thrust_nodes"]
                ]
                self.x_info["positive_thrust_nodes"] = [
                    int(e)
                    for e in self.settings["special_case"]["positive_thrust_nodes"]
                ]
        ...
        for i_cs in range(n_control_surfaces):
            self.initial_state[self.x_info["i_control_surfaces"][i_cs]] = self.settings[
                "initial_cs_deflection"
            ][i_cs]
        for i_thrust in range(n_thrust_nodes):
            self.initial_state[self.x_info["i_thrust"][i_thrust]] = self.settings[
                "initial_thrust"
            ][i_thrust]
        if self.with_special_case:
            if self.settings["special_case"]["case_name"] == "differential_thrust":
                ...
                self.initial_state[
                    self.x_info["i_differential_parameter"]
                ] = self.x_info["initial_differential_parameter"]
        ...
        for k, v in self.x_info.items():
            if k == "i_alpha":
                ...
                for ii, i in enumerate(v):
                    self.bounds[i] = (
                        self.initial_state[self.x_info["i_control_surfaces"][ii]]
                        - 4 * np.pi / 180,
                        self.initial_state[self.x_info["i_control_surfaces"][ii]]
                        + 4 * np.pi / 180,
                    )
                ...
                if self.with_special_case:
                    if (
                        self.settings["special_case"]["case_name"]
                        == "differential_thrust"
                    ):
                        self.bounds[v] = (
...
def solver_wrapper(x, x_info, solver_data, i_dim=-1):
    ...
    solver_data.data.structure.timestep_info[
        solver_data.data.ts
    ] = solver_data.data.structure.ini_info.copy()
    ...
    for i_cs in range(len(x_info["i_control_surfaces"])):
        solver_data.data.aero.aero_dict["control_surface_deflection"][
            x_info["control_surfaces_id"][i_cs]
        ] = x[x_info["i_control_surfaces"][i_cs]]
    # thrust input
    ...
    try:
        ...
        for i_thrust in range(len(x_info["i_thrust"])):
            ...
            solver_data.data.structure.ini_info.steady_applied_forces[i_node, 0:3] = (
                thrust * x_info["thrust_direction"][i_thrust]
            )
    else:
