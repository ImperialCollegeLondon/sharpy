...
class PrescribedUvlm(BaseSolver):
    ...
    settings_description["postprocessors_settings"] = (
        "Dictionary with the applicable settings for every ``postprocessor``. Every"
        " ``postprocessor`` needs its entry, even if empty"
    )
    ...
    def run(self, **kwargs):
        ...
        for self.data.ts in range(1, self.settings["n_time_steps"] + 1):
            ...
            if ts > 0:
                self.data.structure.timestep_info[ts].for_vel[
                    :
                ] = self.data.structure.dynamic_input[ts - 1]["for_vel"]
                self.data.structure.timestep_info[ts].for_acc[
                    :
                ] = self.data.structure.dynamic_input[ts - 1]["for_acc"]
            ...
            self.aero_solver.update_custom_grid(
                self.data.structure.timestep_info[-1], self.data.aero.timestep_info[-1]
            )
            # run the solver
            ...
            self.residual_table.print_line([self.data.ts, self.data.ts * self.dt])
