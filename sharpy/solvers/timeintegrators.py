...
class NewmarkBeta(_BaseTimeIntegrator):
    ...
    def predictor(self, q, dqdt, dqddt):
        ...
        q[:sys_size] += (
            self.dt * dqdt[:sys_size]
            + (0.5 - self.beta) * self.dt * self.dt * dqddt[:sys_size]
        )
    ...
    def build_matrix(self, M, C, K, Q, kBnh, LM_Q):
        ...
        Asys[:sys_size, :sys_size] = (
            K
            + C * self.gamma / (self.beta * self.dt)
            + M / (self.beta * self.dt * self.dt)
        )
...
class GeneralisedAlpha(_BaseTimeIntegrator):
    ...
    def initialise(self, data, custom_settings=None, restart=False):
        ...
        self.om_am = 1.0 - self.am
        self.om_af = 1.0 - self.af

        self.gamma = 0.5 - self.am + self.af
        self.beta = 0.25 * (1.0 - self.am + self.af) ** 2
    ...
    def build_matrix(self, M, C, K, Q, kBnh, LM_Q):
        ...
        Asys[sys_size:, :sys_size] = (
            self.gamma * self.om_af / self.beta / self.dt
        ) * kBnh
        Asys[:sys_size, sys_size:] = kBnh.T
        Qout[sys_size:] = LM_Q.copy()
