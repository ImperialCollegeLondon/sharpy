...
class NonLinearDynamicMultibody(_BaseStructural):
    ...
    settings_default["relax_factor_lm"] = 0.0
    settings_description[
        "relax_factor_lm"
    ...
    settings_description[
        "allow_skip_step"
    ] = "Allow skip step when NaN is found while solving the system"

    settings_types["rigid_bodies"] = "bool"
    settings_default["rigid_bodies"] = False
    settings_description[
        "rigid_bodies"
    ] = "Set to zero the changes in flexible degrees of freedom (not very efficient)"

    settings_types["zero_ini_dot_ddot"] = "bool"
    settings_default["zero_ini_dot_ddot"] = False
    settings_description[
        "zero_ini_dot_ddot"
    ] = "Set to zero the position and crv derivatives at the first time step"
    ...
    def assembly_MB_eq_system(
        self, MB_beam, MB_tstep, ts, dt, Lambda, Lambda_dot, MBdict
    ):
        """
        ...
        for ibody in range(len(MB_beam)):
            ...
            MB_M[first_dof:last_dof, first_dof:last_dof] = M.astype(
                dtype=ct.c_double, copy=True, order="F"
            )
            MB_C[first_dof:last_dof, first_dof:last_dof] = C.astype(
                dtype=ct.c_double, copy=True, order="F"
            )
            MB_K[first_dof:last_dof, first_dof:last_dof] = K.astype(
                dtype=ct.c_double, copy=True, order="F"
            )
    ...
    def integrate_position(self, MB_beam, MB_tstep, dt):
        ...
        for ibody in range(0, len(MB_tstep)):
            ...
            if True:
                acc[0:3] = (0.5 - self.beta) * np.dot(
                    MB_beam[ibody].timestep_info.cga(),
                    MB_beam[ibody].timestep_info.for_acc[0:3],
                ) + self.beta * np.dot(
                    MB_tstep[ibody].cga(), MB_tstep[ibody].for_acc[0:3]
                )
                vel[0:3] = np.dot(
                    MB_beam[ibody].timestep_info.cga(),
                    MB_beam[ibody].timestep_info.for_vel[0:3],
                )
                MB_tstep[ibody].for_pos[0:3] += dt * (vel[0:3] + dt * acc[0:3])
            else:
                MB_tstep[ibody].for_pos[0:3] += dt * np.dot(
                    MB_tstep[ibody].cga(), MB_tstep[ibody].for_vel[0:3]
                )
    ...
    def compute_forces_constraints(self, MB_beam, MB_tstep, ts, dt, Lambda, Lambda_dot):
        ...
        for ibody in range(len(MB_beam)):
            ...
            MB_tstep[ibody].forces_constraints_nodes[(MB_beam[ibody].vdof > -1), :] = F[
                first_dof:last_dof
            ].reshape(body_freenodes, 6, order="C")

            # Forces associated to the frame of reference
            if MB_beam[ibody].FoR_movement == "free":
                # TODO: How are the forces in the quaternion equation interpreted?
                MB_tstep[ibody].forces_constraints_FoR[ibody, :] = F[
                    last_dof : last_dof + 10
                ]
                last_dof += 10
    ...
    def write_lm_cond_num(
        self, iteration, Lambda, Lambda_dot, Lambda_ddot, cond_num, cond_num_lm
    ):
    ...
    def run(self, **kwargs):
        ...
        MB_beam, MB_tstep = mb.split_multibody(
            self.data.structure, structural_step, MBdict, self.data.ts
        )
        ...
        if self.data.ts == 1 and self.settings["zero_ini_dot_ddot"]:
            for ibody in range(len(MB_tstep)):
                MB_beam[ibody].ini_info.pos_dot *= 0.0
                MB_beam[ibody].ini_info.pos_ddot *= 0.0
                MB_beam[ibody].ini_info.psi_dot *= 0.0
                MB_beam[ibody].ini_info.psi_dot_local *= 0.0
                MB_beam[ibody].ini_info.psi_ddot *= 0.0
                MB_tstep[ibody].pos_dot *= 0.0
                MB_tstep[ibody].pos_ddot *= 0.0
                MB_tstep[ibody].psi_dot *= 0.0
                MB_tstep[ibody].psi_dot_local *= 0.0
                MB_tstep[ibody].psi_ddot *= 0.0
        ...
        if not num_LM_eq == 0:
            ...
            Lambda_ddot = self.Lambda_ddot.astype(
                dtype=ct.c_double, copy=True, order="F"
            )
        else:
            Lambda = np.zeros((1,))
        ...
        # Predictor step
        q, dqdt, dqddt = mb.disp_and_accel2state(
            MB_beam, MB_tstep, Lambda, Lambda_dot, self.sys_size, num_LM_eq
        )
        ...
        for iteration in range(self.settings["max_iterations"]):
            ...
            if iteration == self.settings["max_iterations"] - 1:
                error = (
                    ...
                    " = %e" % (iteration, res, LM_res)
                )
                raise exc.NotConvergedSolver(error)

            # Update positions and velocities
            Lambda, Lambda_dot = mb.state2disp_and_accel(
                q, dqdt, dqddt, MB_beam, MB_tstep, num_LM_eq
            )

            if self.settings["write_lm"] and iteration:
                self.write_lm_cond_num(
                    iteration, Lambda, Lambda_dot, Lambda_ddot, cond_num, cond_num_lm
                )

            MB_M, MB_C, MB_K, MB_Q, kBnh, LM_Q = self.assembly_MB_eq_system(
                MB_beam, MB_tstep, self.data.ts, dt, Lambda, Lambda_dot, MBdict
            )

            Asys, Q = self.time_integrator.build_matrix(
                MB_M, MB_C, MB_K, MB_Q, kBnh, LM_Q
            )
            ...
            if self.settings["rigid_bodies"]:
                rigid_LM_dofs = (
                    self.rigid_dofs
                    + (np.arange(self.num_LM_eq, dtype=int) + self.sys_size).tolist()
                )
            ...
            relax_Dq[self.sys_size :] = (1.0 - self.settings["relax_factor_lm"]) * Dq[
                self.sys_size :
            ...
            if num_LM_eq:
                LM_res = (
                    np.max(np.abs(Dq[self.sys_size : self.sys_size + num_LM_eq]))
                    / LM_old_Dq
                )
            else:
                LM_res = 0.0

            if (res < self.settings["min_delta"]) and (
                LM_res < self.settings["min_delta"]
            ):
                break
        ...
        if skip_step:
            ...
            for ibody in range(0, len(MB_tstep)):
                Temp = np.linalg.inv(
                    np.eye(4)
                    + 0.25 * algebra.quadskew(MB_tstep[ibody].for_vel[3:6]) * dt
                )
                MB_tstep[ibody].quat = np.dot(
                    Temp,
                    np.dot(
                        np.eye(4)
                        - 0.25 * algebra.quadskew(MB_tstep[ibody].for_vel[3:6]) * dt,
        ...
        self.compute_forces_constraints(
            MB_beam, MB_tstep, self.data.ts, dt, Lambda, Lambda_dot
        )
        if self.settings["gravity_on"]:
            for ibody in range(len(MB_beam)):
                xbeamlib.cbeam3_correct_gravity_forces(
                    MB_beam[ibody], MB_tstep[ibody], self.settings
                )
        mb.merge_multibody(
            MB_tstep, MB_beam, self.data.structure, structural_step, MBdict, dt
        )
