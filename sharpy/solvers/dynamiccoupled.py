...
class DynamicCoupled(BaseSolver):
    ...
    settings_description["structural_substeps"] = (
        "Number of extra structural time steps per aero time step. ``0`` "
        "is a fully coupled simulation."
    )

    settings_types["relaxation_factor"] = "float"
    settings_default["relaxation_factor"] = 0.2
    settings_description["relaxation_factor"] = (
        "Relaxation parameter in the FSI iteration. ``0`` is no relaxation "
        "and -> ``1`` is very relaxed"
    )

    settings_types["final_relaxation_factor"] = "float"
    settings_default["final_relaxation_factor"] = 0.0
    settings_description["final_relaxation_factor"] = (
        "Relaxation factor reached in ``relaxation_steps`` with "
    ...
    settings_description["relaxation_steps"] = (
    ...
    settings_description["postprocessors_settings"] = (
    ...
    settings_description["cleanup_previous_solution"] = (
        "Controls if previous ``timestep_info`` arrays are "
    ...
    settings_description["include_unsteady_force_contribution"] = (
        "If on, added mass contribution is added to the "
    ...
    settings_description["steps_without_unsteady_force"] = (
        "Number of initial timesteps that don't include unsteady "
    ...
    settings_description["pseudosteps_ramp_unsteady_force"] = (
        "Length of the ramp with which unsteady force "
    ...
    settings_description["correct_forces_method"] = (
        "Function used to correct aerodynamic forces. "
        "See :py:mod:`sharpy.generators.polaraeroforces`"
    )
    settings_options["correct_forces_method"] = [
    ...
    settings_description["network_settings"] = (
    ...
    settings_description["runtime_generators"] = (
        "The dictionary keys are the runtime generators to be used. "
    ...
    def initialise(self, data, custom_settings=None, restart=False):
        ...
        self.substep_dt = self.dt / (self.settings["structural_substeps"] + 1)
        ...
        self.structural_solver.initialise(
            self.data, self.settings["structural_solver_settings"], restart=restart
        )
        self.aero_solver.initialise(
            self.structural_solver.data,
        ...
        if self.settings["correct_forces_method"] != "":
            ...
            self.correct_forces_generator = gen_interface.generator_from_string(
                self.settings["correct_forces_method"]
            )()
            self.correct_forces_generator.initialise(
                in_dict=self.settings["correct_forces_settings"],
                aero=self.data.aero,
        ...
        for rg_id, param in self.settings["runtime_generators"].items():
            ...
            self.runtime_generators[rg_id].initialise(
                param, data=self.data, restart=restart
            )
        ...
        for info_k, info_v in info.items():
            if info_k in self.settings:
                if info_k == "structural_substeps":
                    if info_v is not None:
                        self.substep_dt = self.settings["dt"] / (
    ...
    def run(self, **kwargs):
        ...
        if self.network_loader is not None:
            ...
            with concurrent.futures.ThreadPoolExecutor(max_workers=2) as executor:
                netloop = executor.submit(
                    self.network_loop, incoming_queue, outgoing_queue, finish_event
                )
                timeloop = executor.submit(
                    self.time_loop,
                    incoming_queue,
                    outgoing_queue,
                    finish_event,
    ...
    def time_loop(self, in_queue=None, out_queue=None, finish_event=None, solvers=None):
        ...
        for self.data.ts in range(
            len(self.data.structure.timestep_info), self.settings["n_time_steps"] + 1
        ):
            ...
            if self.with_runtime_generators:
                structural_kstep.runtime_steady_forces.fill(0.0)
                structural_kstep.runtime_unsteady_forces.fill(0.0)
                params = dict()
            ...
            for k in range(self.settings["fsi_substeps"] + 1):
                if k == self.settings["fsi_substeps"] and self.settings["fsi_substeps"]:
                    print_res = 0 if self.res == 0.0 else np.log10(self.res)
                    print_res_dqdt = (
                        0 if self.res_dqdt == 0.0 else np.log10(self.res_dqdt)
                    )
                    cout.cout_wrap(
                        "The FSI solver did not converge!!! residuals: %f %f"
                        % (print_res, print_res_dqdt)
                    )
                ...
                if self.settings["include_unsteady_force_contribution"]:
                    if self.data.ts > self.settings["steps_without_unsteady_force"]:
                        ...
                        if k < self.settings["pseudosteps_ramp_unsteady_force"]:
                            force_coeff = (
                                k / self.settings["pseudosteps_ramp_unsteady_force"]
                            )
                        else:
                            force_coeff = 1.0

                previous_runtime_steady_forces = (
                    structural_kstep.runtime_steady_forces.astype(
                        dtype=ct.c_double, order="F", copy=True
                    )
                )
                previous_runtime_unsteady_forces = (
                    structural_kstep.runtime_unsteady_forces.astype(
                        dtype=ct.c_double, order="F", copy=True
                    )
                )
                # Add external forces
                ...
                structural_kstep.runtime_steady_forces = (
                    previous_kstep.runtime_steady_forces.astype(
                        dtype=ct.c_double, order="F", copy=True
                    )
                )
                structural_kstep.runtime_unsteady_forces = (
                    previous_kstep.runtime_unsteady_forces.astype(
                        dtype=ct.c_double, order="F", copy=True
                    )
                )
                previous_kstep.runtime_steady_forces = (
                    previous_runtime_steady_forces.astype(
                        dtype=ct.c_double, order="F", copy=True
                    )
                )
                previous_kstep.runtime_unsteady_forces = (
                    previous_runtime_unsteady_forces.astype(
                        dtype=ct.c_double, order="F", copy=True
                    )
                )
                ...
                relax(
                    self.data.structure, structural_kstep, previous_kstep, relax_factor
                )
                ...
                for i_substep in range(self.settings["structural_substeps"] + 1):
                    # run structural solver
                    coeff = (i_substep + 1) / (self.settings["structural_substeps"] + 1)
            ...
            if self.print_info:
                print_res = 0 if self.res_dqdt == 0.0 else np.log10(self.res_dqdt)
            ...
            if self.with_postprocessors:
                for postproc in self.postprocessors:
                    self.data = self.postprocessors[postproc].run(
                        online=True, solvers=solvers
                    )
    ...
    def convergence(
        ...
        if not k:
            ...
            if with_runtime_generators:
                self.base_res_forces = np.linalg.norm(
                    tstep.runtime_steady_forces + tstep.runtime_unsteady_forces
                )
                if self.base_res_forces == 0:
                    self.base_res_forces = 1.0
            return False
        ...
        self.res_dqdt = (
            np.linalg.norm(tstep.dqdt - previous_tstep.dqdt) / self.base_dqdt
        )

        if with_runtime_generators:
            res_forces = (
                np.linalg.norm(
                    tstep.runtime_steady_forces
        ...
        if "rigid" in struct_solver.solver_id.lower():
            ...
            "NonLinearDynamicMultibody" == struct_solver.solver_id.lower()
            and struct_solver.settings["rigid_bodies"]
        ):
            rigid_solver = True
    ...
    def map_forces(self, aero_kstep, structural_kstep, unsteady_forces_coeff=1.0):
        ...
        if self.correct_forces:
            struct_forces = self.correct_forces_generator.generate(
                aero_kstep=aero_kstep,
        ...
        if len(self.data.structure.dynamic_input) > 0:
            structural_kstep.unsteady_applied_forces += (
                self.data.structure.dynamic_input[max(self.data.ts - 1, 0)][
                    "dynamic_forces"
                ]
            )
        structural_kstep.unsteady_applied_forces += (
    ...
    def interpolate_timesteps(step0, step1, out_step, coeff):
        ...
        * `velocity` input in Lagrange constraints
        ...
        out_step.steady_applied_forces[:] = (
            1.0 - coeff
        ) * step0.steady_applied_forces + (coeff) * (step1.steady_applied_forces)

        out_step.unsteady_applied_forces[:] = (
            1.0 - coeff
        ) * step0.unsteady_applied_forces + (coeff) * (step1.unsteady_applied_forces)
                ...
                if "constraint_" in key:
                    try:
                        out_step.mb_dict[key]["velocity"][:] = (
                            1.0 - coeff
                        ) * step0.mb_dict[key]["velocity"] + (coeff) * step1.mb_dict[
                            key
                        ][
                            "velocity"
                        ]
...
def relax(beam, timestep, previous_timestep, coeff):
    timestep.steady_applied_forces = (
        (1.0 - coeff) * timestep.steady_applied_forces
        + coeff * previous_timestep.steady_applied_forces
    )
    timestep.unsteady_applied_forces = (
        (1.0 - coeff) * timestep.unsteady_applied_forces
        + coeff * previous_timestep.unsteady_applied_forces
    )
    timestep.runtime_steady_forces = (
        (1.0 - coeff) * timestep.runtime_steady_forces
