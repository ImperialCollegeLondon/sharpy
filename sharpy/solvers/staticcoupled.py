...
class StaticCoupled(BaseSolver):
    ...
    settings_default["relaxation_factor"] = 0.0
    settings_description["relaxation_factor"] = (
        "Relaxation parameter in the FSI iteration. 0 is no relaxation and -> 1 is very"
        " relaxed"
    )
    ...
    def cleanup_timestep_info(self):
        if (
            ...
            self.data.structure.timestep_info[0] = self.data.structure.timestep_info[
                -1
            ].copy()
            # delete all the rest
    ...
    def run(self, **kwargs):
        for i_step in range(self.settings["n_load_steps"] + 1):
            if (
                i_step == self.settings["n_load_steps"]
                and self.settings["n_load_steps"] > 0
            ):
                break
            ...
            for i_iter in range(self.settings["max_iter"]):
                ...
                if self.correct_forces:
                    struct_forces = self.correct_forces_generator.generate(
                        aero_kstep=self.data.aero.timestep_info[self.data.ts],
                ...
                self.data.aero.timestep_info[
                    self.data.ts
                ].aero_steady_forces_beam_dof = struct_forces
                self.data.structure.timestep_info[self.data.ts].postproc_node[
                    "aero_steady_forces"
                ] = struct_forces  # B
                ...
                if self.with_runtime_generators:
                    self.data.structure.timestep_info[
                        self.data.ts
                    ].runtime_steady_forces.fill(0.0)
                    self.data.structure.timestep_info[
                        self.data.ts
                    ].runtime_unsteady_forces.fill(0.0)
                    params = dict()
                    params["data"] = self.data
                    params["struct_tstep"] = self.data.structure.timestep_info[
                        self.data.ts
                    ]
                    ...
                    struct_forces += self.data.structure.timestep_info[
                        self.data.ts
                ...
                if not self.settings["relaxation_factor"] == 0.0:
                    if i_iter == 0:
                        self.previous_force = struct_forces.copy()

                    temp = struct_forces.copy()
                    struct_forces = (
                        1.0 - self.settings["relaxation_factor"]
                    ) * struct_forces + self.settings[
                        "relaxation_factor"
                    ] * self.previous_force
                ...
                self.structural_solver.settings["gravity"] = (
                    old_g * load_step_multiplier
                )
