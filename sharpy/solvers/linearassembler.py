...
class LinearAssembler(BaseSolver):
    ...
    settings_description["modal_tstep"] = (
        "Timestep in which modal information is stored. Useful if the ``Modal`` solver"
    ...
    def initialise(self, data, custom_settings=None, restart=False):
        ...
        try:
            ...
            raise AttributeError(
                "Unable to find modal information at desired timestep {:g}".format(
                    self.settings["modal_tstep"]
                )
            )
    ...
    def run(self, **kwargs):
        ...
        if self.settings["recover_accelerations"]:
            gain = self.data.linear.linear_system.beam.recover_accelerations(
                self.data.linear.ss
            )
        ...
        if len(self.settings["retain_inputs"]) != 0:
            self.data.linear.ss.retain_inout_channels(
                self.settings["retain_inputs"], where="in"
            )
        if len(self.settings["retain_outputs"]) != 0:
            self.data.linear.ss.retain_inout_channels(
                self.settings["retain_outputs"], where="out"
            )
