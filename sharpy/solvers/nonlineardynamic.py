...
class NonLinearDynamic(_BaseStructural):
    ...
    def initialise(self, data, restart=False):
        ...
        for i in range(self.settings["num_steps"]):
            ...
            if i > 0:
                self.data.structure.timestep_info[i].unsteady_applied_forces[
                    :
                ] = self.data.structure.dynamic_input[i - 1]["dynamic_forces"]
            self.data.structure.timestep_info[i].steady_applied_forces[
                :
