...
class BeamLoader(BaseSolver):
    ...
    settings_description["unsteady"] = (
        "If ``True`` it will be a dynamic problem and the solver will look for the"
        " ``.dyn.h5`` file that contains the time varying input to the problem."
    )

    settings_types["orientation"] = "list(float)"
    settings_default["orientation"] = [1.0, 0, 0, 0]
    settings_description["orientation"] = (
        "Initial attitude of the structure given as the quaternion that parametrises"
        " the rotation from G to A frames of reference."
    )

    settings_types["for_pos"] = "list(float)"
    settings_default["for_pos"] = [0.0, 0, 0]
    ...
    def read_files(self):
        ...
        # first, file names
        self.fem_file_name = (
            self.data.case_route + "/" + self.data.case_name + ".fem.h5"
        )
        if self.settings["unsteady"]:
            self.dyn_file_name = (
                self.data.case_route + "/" + self.data.case_name + ".dyn.h5"
            )
        ...
        if os.path.isfile(self.mb_file_name):
            ...
            for iconstraint in range(self.mb_data_dict["num_constraints"]):
                self.mb_data_dict["constraint_%02d" % iconstraint][
                    "behaviour"
                ] = self.mb_data_dict["constraint_%02d" % iconstraint][
                    "behaviour"
                ].decode()
            for ibody in range(self.mb_data_dict["num_bodies"]):
                self.mb_data_dict["body_%02d" % ibody][
                    "FoR_movement"
                ] = self.mb_data_dict["body_%02d" % ibody]["FoR_movement"].decode()

    def validate_fem_file(self):
        raise NotImplementedError(
            "validation of the fem file in beamloader is not yet implemented!"
        )

    def validate_dyn_file(self):
        raise NotImplementedError(
            "validation of the dyn file in beamloader is not yet implemented!"
        )
