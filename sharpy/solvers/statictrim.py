...
class StaticTrim(BaseSolver):
    ...
    settings_default["initial_deflection"] = 0.0
    ...
    settings_default["initial_thrust_eps"] = 2.0
    ...
    def trim_algorithm(self):
        ...
        for self.i_iter in range(self.settings["max_iter"] + 1):
            if self.i_iter == self.settings["max_iter"]:
                raise Exception(
                    "The Trim routine reached max iterations without convergence!"
                )
            ...
            if not self.i_iter:
                ...
                self.input_history[self.i_iter][1] = (
                    self.settings["initial_deflection"] + self.settings["initial_alpha"]
                )
                ...
                self.gradient_history[self.i_iter][0] = (
                    l - self.output_history[self.i_iter][0]
                ) / self.settings["initial_angle_eps"]
                ...
                self.gradient_history[self.i_iter][1] = (
                    m - self.output_history[self.i_iter][1]
                ) / self.settings["initial_angle_eps"]
                ...
                self.gradient_history[self.i_iter][2] = (
                    d - self.output_history[self.i_iter][2]
                ) / self.settings["initial_thrust_eps"]
            ...
            if convergence[0]:
                ...
                self.input_history[self.i_iter][0] = self.input_history[
                    self.i_iter - 1
                ][0]
                self.gradient_history[self.i_iter][0] = self.gradient_history[
                    self.i_iter - 1
                ][0]
            else:
                self.input_history[self.i_iter][0] = self.input_history[
            ...
            if convergence[1]:
                ...
                self.input_history[self.i_iter][1] = self.input_history[
                    self.i_iter - 1
                ][1]
                self.gradient_history[self.i_iter][1] = self.gradient_history[
                    self.i_iter - 1
                ][1]
            else:
            ...
            if convergence[2]:
                ...
                self.input_history[self.i_iter][2] = self.input_history[
                    self.i_iter - 1
                ][2]
                self.gradient_history[self.i_iter][2] = self.gradient_history[
                    self.i_iter - 1
                ][2]
            else:
                # compute next gamma with the previous gradient
            ...
            if self.settings["relaxation_factor"]:
                for i_dim in range(3):
                    self.input_history[self.i_iter][i_dim] = (
                        self.input_history[self.i_iter][i_dim]
                        * (1 - self.settings["relaxation_factor"])
                        + self.input_history[self.i_iter][i_dim]
                        * self.settings["relaxation_factor"]
                    )
