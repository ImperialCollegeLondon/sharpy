...
class Modal(BaseSolver):
    ...
    settings_types[
        "write_modes_vtk"
    ] = "bool"  # write displacements mode shapes in vtk file
    ...
    settings_default["max_rotation_deg"] = 15.0
    settings_description[
        "max_rotation_deg"
    ...
    settings_description["rigid_modes_ppal_axes"] = (
        "Modify the ridid body modes such that they are defined wrt "
    ...
    def initialise(self, data, custom_settings=None, restart=False):
        ...
        self.filename_freq = (
            self.folder + "tstep" + ("%06d" % self.data.ts) + "_ModalFrequencies.dat"
        )
        self.filename_damp = (
            self.folder + "tstep" + ("%06d" % self.data.ts) + "_ModalDamping.dat"
        )
        self.filename_shapes = (
            self.folder + "tstep" + ("%06d" % self.data.ts) + "_ModalShape"
        )
    ...
    def run(self, **kwargs):
        ...
        # Initialize matrices
        FullMglobal = np.zeros((num_dof, num_dof), dtype=ct.c_double, order="F")
        FullKglobal = np.zeros((num_dof, num_dof), dtype=ct.c_double, order="F")
        FullCglobal = np.zeros((num_dof, num_dof), dtype=ct.c_double, order="F")
        ...
        if self.rigid_body_motion:
            ...
            full_matrix_settings = (
                basestructuralsolver._BaseStructural().settings_default
            )
            settings_utils.to_custom_types(
                full_matrix_settings,
                basestructuralsolver._BaseStructural().settings_types,
                full_matrix_settings,
            )
        ...
        if self.settings["use_undamped_modes"]:
            ...
            eigenvalues, eigenvectors_left, eigenvectors = sc.linalg.eig(
                A, left=True, right=True
            )
            freq_natural = np.abs(eigenvalues)
            damping = np.zeros_like(freq_natural)
            iiflex = freq_natural > 1e-16 * np.mean(
                freq_natural
            )  # Pick only structural modes
            ...
            while ii < 2 * NumLambda:
                # check complex
                if np.abs(eigenvalues[ii].imag) > 0.0:
                    if (
                        np.abs(eigenvalues[ii + 1].real - eigenvalues[ii].real)
                        > tol_rel
                        or np.abs(eigenvalues[ii + 1].imag + eigenvalues[ii].imag)
                        > tol_rel
                    ):
            ...
            if self.settings["continuous_eigenvalues"]:
                if self.settings["dt"] == 0.0:
                    raise ValueError(
                        "Cannot compute the continuous eigenvalues without a dt value"
                    )
        ...
        eigenvectors = modalutils.mode_sign_convention(
            self.data.structure.boundary_conditions,
            eigenvectors,
            self.rigid_body_motion,
        )
        if not eigenvectors_left:
            if self.settings["rigid_modes_ppal_axes"]:
                eigenvectors, t_pa, r_pa = modalutils.free_modes_principal_axes(
                    eigenvectors, FullMglobal, return_transform=True
                )
            else:
                t_pa = None  # Transformation matrix from the A frame to the P frame (principal axes of inertia)
                r_pa = None
        # Scaling
        eigenvectors, eigenvectors_left = self.scale_modes_unit_mass_matrix(
            eigenvectors, FullMglobal, eigenvectors_left
        )
        ...
        if self.settings["save_data"]:
            if type(eigenvalues) == complex:
                np.savetxt(
                    self.folder + "eigenvalues.dat",
                    eigenvalues.view(float).reshape(-1, 2),
                    fmt="%.12f",
                    delimiter="\t",
                    newline="\n",
                )
                ...
                np.savetxt(
                    self.folder + "eigenvalues.dat",
                    eigenvalues.view(float),
                    fmt="%.12f",
                    delimiter="\t",
                    newline="\n",
                )
            np.savetxt(
                self.folder + "eigenvectors.dat",
                eigenvectors[:num_dof].real,
                fmt="%.12f",
                delimiter="\t",
                newline="\n",
            )
            ...
            if not self.settings["use_undamped_modes"]:
                np.savetxt(
                    self.folder + "frequencies.dat",
                    freq_damped[:NumLambda],
                    fmt="%e",
                    delimiter="\t",
                    newline="\n",
                )
        ...
        if self.settings["write_modes_vtk"]:
            try:
                ...
                warnings.warn(
                    "No aerodynamic model found - unable to project the mode onto"
                    " aerodynamic grid"
                )
            else:
        ...
        if self.settings["use_undamped_modes"]:
            ...
            if not zero_FullCglobal:
                outdict["warning"] = (
                    "system with damping: mode shapes and natural frequencies do not"
                    " account for damping!"
                )
        else:
    ...
    def scale_modes_unit_mass_matrix(
        self, eigenvectors, FullMglobal, eigenvectors_left=None
    ):
        if self.settings["use_undamped_modes"]:
            # mass normalise (diagonalises M and K)
            eigenvectors = modalutils.scale_mass_normalised_modes(
                eigenvectors, FullMglobal
            )
        else:
            ...
            if not self.rigid_body_motion:
                ...
                ):  # Issue - dot product = 0 when you have arbitrary damping
                    fact = 1.0 / np.sqrt(
                        np.dot(eigenvectors_left[:, ii], eigenvectors[:, ii])
                    )
    ...
    def free_free_modes(self, phi, M):
        ...
        warnings.warn(
            ...
            " sharpy.structure.utils.modalutils.free_modes_principal_axes",
            category=DeprecationWarning,
        )
        if not self.rigid_body_motion:
            warnings.warn(
                "No rigid body modes to transform because the structure is clamped"
            )
            return phi
        else:
            ...
            jj_for_vel = range(
                self.data.structure.num_dof.value, self.data.structure.num_dof.value + 3
            )
            jj_for_rot = range(
                self.data.structure.num_dof.value + 3,
                self.data.structure.num_dof.value + 6,
            )
            ...
            for node_glob in range(self.data.structure.num_node):
                ...
                if bc_here == 1:  # clamp (only rigid-body)
                    ...
                    dofs_here = 6
                    jj_tra = 6 * self.data.structure.vdof[node_glob] + np.array(
                        [0, 1, 2], dtype=int
                    )
                    jj_rot = 6 * self.data.structure.vdof[node_glob] + np.array(
                        [3, 4, 5], dtype=int
                    )
                # jj_tra=[jj  ,jj+1,jj+2]
                # jj_rot=[jj+3,jj+4,jj+5]
                    ...
                    raise NameError(
                        "Invalid boundary condition (%d) at node %d!"
                        % (bc_here, node_glob)
                    )
                ...
                psi = self.data.structure.timestep_info[self.data.ts].psi[
                    ee, node_loc, :
                ]
                ...
                K_vec[np.ix_(jj_rot, jj_for_rot)] += np.linalg.inv(
                    algebra.crv2tan(psi).T
                )
