...
class StepUvlm(BaseSolver):
    ...
    * :class:`~.generators.turbvelocityfield.TurbVelocityField`
    ...
    settings_description["convection_scheme"] = (
        "``0``: fixed wake, "
    ...
    settings_description["gamma_dot_filtering"] = (
        "Filtering parameter for the Welch filter for the Gamma_dot "
    ...
    settings_description["interp_method"] = (
        "Method of interpolation: linear(0), parabolic(1), splines(2), slerp around"
        " z(3), slerp around yaw_slerp(4)"
    )
    ...
    settings_default["centre_rot"] = [0.0, 0.0, 0.0]
    settings_description["centre_rot"] = (
        "Coordinates of the centre of rotation to perform slerp interpolation or"
    ...
    def initialise(self, data, custom_settings=None, restart=False):
        ...
        self.data.structure.add_unsteady_information(
            self.data.structure.dyn_dict, self.settings["n_time_steps"]
        )
    ...
    def run(self, **kwargs):
        ...
        if unsteady_contribution and not self.settings["quasi_steady"]:
            ...
            self.data.aero.compute_gamma_dot(
                dt, aero_tstep, self.data.aero.timestep_info[-3:]
            )
            ...
            uvlmlib.uvlm_calculate_unsteady_forces(
                aero_tstep,
                structure_tstep,
                self.settings,
                convect_wake=convect_wake,
