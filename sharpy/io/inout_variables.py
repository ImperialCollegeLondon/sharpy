...
class Variable:
    ...
    def __init__(self, name, inout, **kwargs):
        ...
        position = kwargs.get(
            "position", None
        )  # int for node, (i_surf, m, n, idx) for panel
    ...
    def get_variable_value(self, data, timestep_index=-1):
        ...
        if self.node is not None:
            ...
            try:
                ...
                variable = getattr(
                    data.structure.timestep_info[timestep_index], self.name
                )
            except AttributeError:
                try:
                    # First get the dict postproc_cell and the try to find the variable in it.
                    get_postproc_cell = getattr(
                        data.structure.timestep_info[timestep_index], "postproc_cell"
                    )
                    variable = get_postproc_cell[self.name]
                except (KeyError, AttributeError):
                    msg = (
                        "Node {} is neither in timestep_info nor in postproc_cell."
                        .format(self.node)
                    )
                    logger.error(msg)
            ...
            if len(variable.shape) == 1:
                try:
                    ...
                    msg = (
                        "Node {} and/or Index {} are out of index of variable {}, "
                        "which is of size ({})".format(
                            self.node, self.index, self.dref_name, variable.shape
                        )
                    )
                    logger.error(msg)
                    raise IndexError(msg)
                ...
                msg = (
                    f"Variable {self.name} is neither a node variable nor an element"
                    f" variable. The variable {self.name} is stored as a"
                    f" {variable.shape} array."
                )
                logger.error(msg)
                raise IndexError(msg)
            ...
            value = (
                len(data.structure.timestep_info[:timestep_index]) - 1
            )  # (-1) needed since first time step is idx 0
        elif self.panel is not None:
            variable = getattr(data.aero.timestep_info[timestep_index], self.name)[
                self.panel[0]
            ]  # surface index
            i_m = self.panel[1]
            i_n = self.panel[2]
            ...
            try:
                value = data.aero.timestep_info[
                    timestep_index
                ].control_surface_deflection[self.cs_index]
            except AttributeError:
                logger.error("Model not equipped with dynamic control surfaces")
                raise AttributeError
            except IndexError:
                logger.error(
                    "Requested index {} for control surface is out of range (size {})"
                    .format(
                        self.cs_index,
                        len(
            ...
            raise NotImplementedError(
                "Unable to get value for {} variable".format(self.name)
            )

        self.value = value
        logger.debug(
            "Getting value {} for variable {}".format(self.value, self.dref_name)
        )
        return value
    ...
    def set_variable_value(self, value):
        ...
        if self.inout == "in" or self.inout == "inout":
            ...
            logger.warning(
                "Trying to set the value of {} which is only an output variable".format(
                    self.dref_name
                )
            )
    ...
    def set_in_timestep(self, data):
        ...
        if self.node is not None:
            ...
            if self.name == "app_forces":
                ...
                try:
                    ...
                    logger.warning(
                        "Unable to set node {}, index {} of variable {}".format(
        ...
        if self.cs_index is not None:
            ...
            try:
                ...
                logger.warning(
                    "Unable to set control surface deflection {}. Check the order of "
...
class SetOfVariables:
    ...
    def get_value(self, data, timestep_index=-1):
        ...
        for out_idx in self.out_variables:
            self.variables[out_idx].get_variable_value(
                data, timestep_index=timestep_index
            )
    ...
    def set_value(self, values):
        ...
        for idx, value in values:
            ...
            logger.info(
                "Set the input variable {} to {:.4f}".format(
                    self.variables[idx].dref_name, self.variables[idx].value
                )
            )
        # save to file:
