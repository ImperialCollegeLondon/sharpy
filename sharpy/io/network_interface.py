...
class NetworkLoader:
    ...
    settings_description["input_network_settings"] = (
        "Settings for the input network."
    ...
    settings_description["output_network_settings"] = (
    ...
    def initialise(self, in_settings):
        ...
        if self.settings["byte_ordering"] == "little":
            ...
            raise KeyError(
                "Unknown byte ordering {}".format(self.settings["byte_ordering"])
            )

        logger_utils.load_logger_settings(
            log_name=self.settings["log_name"],
            file_level=self.settings["file_log_level"],
            console_level=self.settings["console_log_level"],
        )
        logger.info(
            "Initialising Network Interface. Local host name: {}".format(
                socket.gethostname()
            )
        )
    ...
    def get_networks(self, networks="inout"):
        ...
        if networks == "out" or networks == "inout":
            ...
            out_network.initialise(
                "w", in_settings=self.settings["output_network_settings"]
            )
        ...
        if networks == "in" or networks == "inout":
            ...
            in_network.initialise(
                "r", in_settings=self.settings["input_network_settings"]
            )
...
class Network:
    ...
    def set_client_list(self, list_of_clients):
        ...
        own_clients = (
            self.clients.copy()
        )  # make a copy of own clients prior to setting the common list
    ...
    def receive(self, msg_length=1024):
        ...
        logger.info(
            "Received a {}-byte long data packet from {}".format(
                len(r_msg), client_addr
            )
        )
...
class OutNetwork(Network):
    ...
    settings_description["send_on_demand"] = (
        "Waits for a signal demanding the output data. Else, sends to destination"
        " buffer"
    )
    ...
    settings_default[
        "destination_address"
    ] = list()  # add check to raise error if send_on_demand false and this is empty
    settings_description["destination_address"] = (
        "List of addresses to send output data. If ``send_on_demand`` is "
    ...
    def initialise(self, mode, in_settings):
        ...
        if (
            self.settings["send_on_demand"] is False
            and len(self.settings["destination_address"]) == 0
        ):
            logger.warning("No destination host address provided")

        clients = list(
            zip(
                self.settings["destination_address"], self.settings["destination_ports"]
            )
        )
...
class InNetwork(Network):
    ...
    def set_message_length(self, value):
        ...
        logger.debug(
            "Set input signal message size to {} bytes".format(self._in_message_length)
        )
