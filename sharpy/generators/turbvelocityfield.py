...
class TurbVelocityField(generator_interface.BaseGenerator):
    ...
    settings_description["store_field"] = (
        "If ``True``, the xdmf snapshots are stored in memory. Only two at a time for"
    ...
    def initialise(self, in_dict, restart=False):
        ...
        if "z" in self.settings["periodicity"]:
            raise ValueError(
                "Periodicitiy setting in TurbVelocityField cannot be z.\n A turbulent"
                " boundary layer is not periodic in the z direction!"
            )
    ...
    def interpolator_wrapper0(self, coords, i_dim=0):
        ...
        return (1.0 - self.coeff) * self._interpolator0[i_dim](
            coords
        ) + self.coeff * self._interpolator1[i_dim](coords)
    ...
    def read_xdmf(self, in_file):
        ...
        # mesh dimensions
        self.grid_data["dimensions"] = np.fromstring(
            tree.Domain.Topology.attrib["Dimensions"], sep=" ", count=3, dtype=int
        )

        # origin
        self.grid_data["origin"] = np.fromstring(
            tree.Domain.Geometry.DataItem[0].text,
            sep=" ",
        ...
        self.grid_data["dxdydz"] = np.fromstring(
            tree.Domain.Geometry.DataItem[1].text,
            sep=" ",
        ...
        self.grid_data["time"] = np.fromstring(
            tree.Domain.Grid.Time.DataItem.text, sep=" ", count=2, dtype=float
        )
        ...
        for i, i_grid in enumerate(tree.Domain.Grid.Grid):
            ...
            for i_attrib, attrib in enumerate(i_grid.Attribute):
                ...
                self.grid_data["grid"][i][attrib.attrib["Name"]][
                    "file"
        ...
        self.grid_data["initial_x_grid"] = (
            np.array(np.arange(0, self.grid_data["dimensions"][2]))
            * self.grid_data["dxdydz"][2]
        )
        # z in the file is -y for us in sharpy (y_sharpy = right)
        self.grid_data["initial_y_grid"] = (
            np.array(np.arange(0, self.grid_data["dimensions"][1]))
            * self.grid_data["dxdydz"][1]
        )
        # y in the file is z for us in sharpy (up)
        self.grid_data["initial_z_grid"] = (
            np.array(np.arange(0, self.grid_data["dimensions"][0]))
            * self.grid_data["dxdydz"][0]
        )
        ...
        if self.settings["centre_y"]:
            centre_z_offset = -0.5 * (
                self.grid_data["initial_z_grid"][-1]
                - self.grid_data["initial_z_grid"][0]
            )

        self.grid_data["initial_x_grid"] += (
            self.settings["offset"][0] + self.grid_data["origin"][0]
        )
        self.grid_data["initial_x_grid"] -= np.max(self.grid_data["initial_x_grid"])
        self.grid_data["initial_y_grid"] += (
            self.settings["offset"][1] + self.grid_data["origin"][1]
        )
        self.grid_data["initial_z_grid"] += (
            self.settings["offset"][2] + self.grid_data["origin"][2] + centre_z_offset
        )

        self.bbox = self.get_field_bbox(
            self.grid_data["initial_x_grid"],
            self.grid_data["initial_y_grid"],
            self.grid_data["initial_z_grid"],
            frame="G",
        ...
        if self.settings["print_info"]:
            ...
            cout.cout_wrap(
                " x = [" + str(self.bbox[0, 0]) + ", " + str(self.bbox[0, 1]) + "]", 1
            )
            cout.cout_wrap(
                " y = [" + str(self.bbox[1, 0]) + ", " + str(self.bbox[1, 1]) + "]", 1
            )
            cout.cout_wrap(
                " z = [" + str(self.bbox[2, 0]) + ", " + str(self.bbox[2, 1]) + "]", 1
            )
    ...
    def generate(self, params, uext):
        ...
        self.interpolate_zeta(zeta, for_pos, uext)
    ...
    def update_cache(self, t):
        ...
        if t < self._t0:
            raise ValueError(
                "Please make sure everything is ok. Your time is going backwards."
            )
    ...
    def time_2_timestep(self, t):
        return int(
            max(
                0, np.floor((t - self.grid_data["time"][0]) / self.grid_data["time"][1])
            )
        )
    ...
    def create_interpolator(self, data, x_grid, y_grid, z_grid, i_dim):
        interpolator = interpolate.RegularGridInterpolator(
            (x_grid, y_grid, z_grid), data, bounds_error=False, fill_value=0.0
        )
        return interpolator

    def interpolate_zeta(
        self, zeta, for_pos, u_ext, interpolator=None, offset=np.zeros((3))
    ):
        ...
        for isurf in range(len(zeta)):
            ...
            for i_m in range(n_m):
                for i_n in range(n_n):
                    coord = self.g_2_gstar(
                        self.apply_periodicity(
                            zeta[isurf][:, i_m, i_n] + for_pos[0:3] + offset
                        )
                    )
                    for i_dim in range(3):
                        try:
                            u_ext[isurf][i_dim, i_m, i_n] = self.interpolator[i_dim](
                                coord
                            )
                        except ValueError:
                            print(coord)
                            raise ValueError()
                    u_ext[isurf][:, i_m, i_n] = self.gstar_2_g(
                        u_ext[isurf][:, i_m, i_n]
                    )
        ...
        for i_dim in range(3):
            ...
            if i_cache == 0:
                if not self.settings["store_field"]:
                    ...
                    self.vel_holder0[i_dim] = np.memmap(
                        self.route + "/" + file_name,
                        # dtype='float64',
                        dtype=self.grid_data["grid"][i_grid][velocities[i_dim]][
                            "Precision"
                        ],
                        shape=(
                    ...
                    # load and store file
                    self.vel_holder0[i_dim] = np.fromfile(
                        open(self.route + "/" + file_name, "rb"),
                        dtype=self.grid_data["grid"][i_grid][velocities[i_dim]][
                            "Precision"
                        ],
                    ).reshape(
                ...
                if not self.settings["store_field"]:
                    # load file, but dont copy it
                    self.vel_holder1[i_dim] = np.memmap(
                        self.route + "/" + file_name,
                        # dtype='float64',
                        dtype=self.grid_data["grid"][i_grid][velocities[i_dim]][
                            "Precision"
                    ...
                    # load and store file
                    self.vel_holder1[i_dim] = np.fromfile(
                        open(self.route + "/" + file_name, "rb"),
                        dtype=self.grid_data["grid"][i_grid][velocities[i_dim]][
                            "Precision"
                        ],
