...
doc_settings_description = (
    "The ``GustVelocityField`` generator takes the following settings as a dictionary"
    " assignedto ``gust_parameters``."
)
...
class one_minus_cos(BaseGust):
    ...
    def gust_shape(self, x, y, z, time=0):
        ...
        vel[self.settings["gust_component"]] = (
            (1.0 - np.cos(2.0 * np.pi * x / gust_length)) * gust_intensity * 0.5
        )
        return vel
...
class continuous_sin(BaseGust):
    ...
    def gust_shape(self, x, y, z, time=0):
        ...
        vel[self.settings["gust_component"]] = (
            0.5 * gust_intensity * np.sin(2 * np.pi * x / gust_length)
        )
        return vel
...
class time_varying_global(BaseGust):
    ...
    settings_description["gust_component"] = (
        "Component of the gust velocity in the G-frame to be considered"
        " (x,y,z)->(0,1,2)."
    )
    ...
    def initialise_interpolation_functions(self):
        for idim in self.settings["gust_component"]:
            self.list_interpolated_velocity_field_functions.append(
                interp1d(
                    self.file_info[:, 0],
                    self.file_info[:, idim + 1],
                    bounds_error=False,
...
class time_varying(time_varying_global):
    ...
    def initialise_interpolation_functions(self):
        for idim in self.settings["gust_component"]:
            self.list_interpolated_velocity_field_functions.append(
                interp1d(
                    -self.file_info[::-1, 0] * self.u_inf,
                    self.file_info[::-1, idim + 1],
                    bounds_error=False,
                    fill_value="extrapolate",
                )
            )
        ...
        if np.abs(d) <= self.settings["span_with_gust"] / 2:
            vel = (
                ...
                * np.sin(
                    ...
                    * 2.0
                    * np.pi
                    / (self.settings["span"] / self.settings["periods_per_span"])
                )
            )
        else:
            vel = np.zeros((3,))
...
class GustVelocityField(generator_interface.BaseGenerator):
    ...
    settings_default["u_inf_direction"] = [1.0, 0.0, 0.0]
    ...
    settings_description[
        "relative_motion"
    ] = "If true, the gust is convected with u_inf"
    ...
    def initialise(self, in_dict, restart=False):
        ...
        if not (self.settings["gust_shape"] in self.implemented_gusts):
            raise AttributeError(
                "The gust shape " + self.settings["gust_shape"] + " is not implemented"
            )
            ...
            for i in range(zeta[i_surf].shape[1]):
                for j in range(zeta[i_surf].shape[2]):
                    ...
                    if self.settings["relative_motion"]:
                        uext[i_surf][:, i, j] += (
                            self.settings["u_inf"] * self.settings["u_inf_direction"]
                        )
                        total_offset_val -= self.settings["u_inf"] * t

                    total_offset = (
                        total_offset_val * self.settings["u_inf_direction"] + for_pos
                    )
