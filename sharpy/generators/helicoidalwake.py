    ...
    def generate(self, params):
        ...
        for isurf in range(nsurf):
            ...
            for i in range(M):
                ...
                rot = algebra.rotation_matrix_around_axis(
                    algebra.unit_vector(self.rotation_velocity), angle
                )
                for j in range(N):
                    # Define the helicoidal
                    aux_zeta_TE = (
                        zeta[isurf][:, -1, j]
                        - (self.h_ref - self.h_corr) * self.shear_direction
                    )
                    aux_zeta_TE = (
                        np.dot(rot, aux_zeta_TE)
                        + (self.h_ref - self.h_corr) * self.shear_direction
                    )
                    ...
                    zeta_star[isurf][:, i, j] = (
                        aux_zeta_TE
                        + self.u_inf
                        * self.u_inf_direction
                        * (h / self.h_ref) ** self.shear_exp
                        * delta_t
                    )
