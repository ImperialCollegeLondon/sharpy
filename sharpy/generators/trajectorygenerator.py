...
class TrajectoryGenerator(generator_interface.BaseGenerator):
    ...
    settings_description["shape"] = (
        "Shape of the ``z`` vs ``x`` function. ``quadratic`` or ``linear`` are"
        " supported"
    )
    ...
    settings_description["return_velocity"] = (
        "If ``True``, nodal velocities are given, if ``False``, coordinates are the"
        " output"
    )
    ...
    def generate(self, params):
        ...
        if self.in_dict["return_velocity"]:
            return (
                np.array([self.x_dot_vec[it], 0.0, self.y_dot_vec[it]])
                + self.in_dict["offset"]
            )
        else:
            return (
                np.array([self.x_vec[it], 0.0, self.y_vec[it]]) + self.in_dict["offset"]
            )
    ...
    def calculate_trajectory(self):
        ...
        if in_dict["shape"] == "linear":
            ...
            shape_polynomial[2] = (
                np.arctan(in_dict["angle_end"].value)
                - in_dict["coords_end"][1] / in_dict["coords_end"][0]
            ) / in_dict["coords_end"][0]
            shape_polynomial[1] = (
                np.arctan(in_dict["angle_end"].value)
                - 2.0 * shape_polynomial[2] * in_dict["coords_end"][0]
            )
            shape_polynomial[0] = 0.0
            curve_length = quadratic_curve_length(
                shape_polynomial, in_dict["coords_end"]
            )
        ...
        if in_dict["acceleration"] == "constant":
            acceleration_position_coefficients = constant_acceleration_position_coeffs(
                curve_length, np.abs(in_dict["veloc_end"].value)
            )
            travel_time = constant_acceleration_travel_time(
                curve_length, np.abs(in_dict["veloc_end"].value)
            )
        elif in_dict["acceleration"] == "linear":
            acceleration_position_coefficients = linear_acceleration_position_coeffs(
                curve_length, np.abs(in_dict["veloc_end"].value)
            )
            travel_time = linear_acceleration_travel_time(
                curve_length, np.abs(in_dict["veloc_end"].value)
            )
        ...
        n_steps_offset = int(
            round(self.in_dict["time_offset"].value / in_dict["dt"].value)
        )
        self.n_steps = n_steps_offset + n_steps_travel
        self.time_vec = np.zeros((self.n_steps,))
        self.time_vec[n_steps_offset:] = np.linspace(
            0.0, n_steps_travel * in_dict["dt"].value, n_steps_travel
        )
        ...
        for i_sample in range(n_samples):
            if in_dict["shape"] == "linear":
                sampled_s_vec[i_sample] = np.abs(
                    linear_curve_length(
                        shape_polynomial, np.array([sampled_x_vec[i_sample], 0.0])
                    )
                )
            elif in_dict["shape"] == "quadratic":
                sampled_s_vec[i_sample] = np.abs(
                    quadratic_curve_length(
                        shape_polynomial, np.array([sampled_x_vec[i_sample], 0.0])
                    )
                )
        x_of_s_interp = sc.interpolate.interp1d(
            sampled_s_vec, sampled_x_vec, kind="quadratic", fill_value="extrapolate"
        )
...
def linear_acceleration_position_coeffs(s_e, s_dot_e):
    coeff = ((6.0 * s_e) ** (2.0 / 3.0) / (2.0 * s_dot_e)) ** (-3.0)
    coeffs = (0.0, 0.0, 0.0, (1.0 / 6.0) * coeff)
    return coeffs
