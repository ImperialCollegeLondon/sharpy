...
class FloatingForces(generator_interface.BaseGenerator):
    ...
    def generate(self, params):
        ...
        for imoor in range(self.n_mooring_lines):
            ...
            struct_tstep.applied_forces[self.mooring_node, 0:3] += np.dot(cbg, force_cl[0:3])
            struct_tstep.applied_forces[self.mooring_node, 3:6] += np.dot(cbg, force_cl[3:6])

        # Yaw moment generated by the mooring system
        yaw = algebra.quat2euler(struct_tstpe.quat)[0]
        struct_tstep.applied_forces[self.mooring_node, 3:6] += np.dot(cbg,
                                                                      self.settings['mooring_yaw_spring_stif']*yaw)

        # Hydrodynamic model
        q = np.zeros((6,), dtype=int)
        q[0:3] = (np.dot(cga, struct_tstep.pos[self.cob_node, :]) -
                  np.dot(data.structure.ini_info.cga, data.structure.ini_info.pos[self.cob_node, :]) +
