...
class StraightWake(generator_interface.BaseGenerator):
    ...
    settings_default["u_inf"] = 1.0  # None
    ...
    def initialise(self, data, in_dict=None, restart=False):
        ...
        if len(self.in_dict.keys()) == 0:
            cout.cout_wrap(
                ...
                "     In future releases you will need to define a"
                " 'wake_shape_generator' in ``AerogridLoader''.                   "
            ...
            if "dt" in aero_solver_settings.keys():
                ...
                dt = 1.0 / aero_solver_settings["velocity_field_input"]["u_inf"]
            self.in_dict = {
                "u_inf": aero_solver_settings["velocity_field_input"]["u_inf"],
                "u_inf_direction": aero_solver_settings["velocity_field_input"][
                    "u_inf_direction"
                ],
                "dt": dt,
            }
    ...
    def generate(self, params):
        ...
        for isurf in range(nsurf):
            r_surf, ndx1_surf, dx1_surf, dxmax_surf = self.get_all_surface_parameters(
                isurf
            )
            ...
            for j in range(N):
                ...
                for i in range(1, M):
                    ...
                    zeta_star[isurf][:, i, j] = (
                        zeta_star[isurf][:, i - 1, j] + deltax * self.u_inf_direction
                    )
            gamma[isurf] *= 0.0
            gamma_star[isurf] *= 0.0

        for isurf in range(nsurf):
            M, N = zeta_star[isurf][0, :, :].shape
            dist_to_orig[isurf][0] = 0.0
            for j in range(0, N):
                for i in range(1, M):
                    dist_to_orig[isurf][i, j] = dist_to_orig[isurf][
                        i - 1, j
                    ] + np.linalg.norm(
                        zeta_star[isurf][:, i, j] - zeta_star[isurf][:, i - 1, j]
                    )
