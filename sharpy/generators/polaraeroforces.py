...
from sharpy.aero.utils.utils import (
...
class PolarCorrection(generator_interface.BaseGenerator):
    ...
    settings_description["cd_from_cl"] = (
        "Interpolate the C_D for the given C_L, as opposed to getting the C_D from "
        "the section AoA."
    )

    settings_types["moment_from_polar"] = "bool"
    settings_default["moment_from_polar"] = False
    settings_description["moment_from_polar"] = (
        "If ``correct_lift`` is selected, it will compute the pitching moment "
    ...
    settings_description["aoa_cl0"] = (
        "Angle of attack for which zero lift is achieved specified in deg for each"
        " airfoil."
    )
    ...
    def generate(self, **params):
        ...
        pos_g = np.array(
            [
                cga.dot(structural_kstep.pos[inode]) + np.array([0, 0, 0])
            ...
            if aero_dict["aero_node"][inode]:
                ...
                if isurf not in self.settings["skip_surfaces"]:
                    ...
                    cab = algebra.crv2rotation(
                        structural_kstep.psi[ielem, inode_in_elem, :]
                    )
                    cgb = np.dot(cga, cab)

                    if not self.cd_from_cl:
                        airfoil = str(
                            aero_dict["airfoil_distribution"][ielem, inode_in_elem]
                        )
                        aoa_0cl = self.list_aoa_cl0[int(airfoil)]

                    # computing surface area of panels contributing to force
                    dir_span, span, dir_chord, chord = span_chord(
                        i_n, aero_kstep.zeta[isurf]
                    )
                    area = span * chord
                    area = self.correct_surface_area(
                        inode, aerogrid.struct2aero_mapping, aero_kstep.zeta, area
                    )

                    # Define the relative velocity and its direction
                    urel, dir_urel = magnitude_and_direction_of_relative_velocity(
                    ...
                    c_bs = local_stability_axes(
                        cgb.T.dot(dir_urel), cgb.T.dot(dir_chord)
                    )
                    ...
                    forces_s[0] += (
                        cd * coef
                    )  # add viscous drag to induced drag from UVLM
                    forces_s[2] = cl * coef
                    ...
                    panel_shift = 0.25 * (
                        aero_kstep.zeta[isurf][:, 1, i_n]
                        - aero_kstep.zeta[isurf][:, 0, i_n]
                    )
                    ref_point = (
                        aero_kstep.zeta[isurf][:, 0, i_n]
                        + 0.25 * chord * dir_chord
                        - panel_shift
                    )
                    ...
                    moment_polar_drag = algebra.cross3(
                        c_bs.T.dot(arm), cd * dir_urel * coef
                    )  # in S frame
                    ...
                    if correct_lift and moment_from_polar:
                        ...
                        moment_polar_lift = algebra.cross3(
                            c_bs.T.dot(arm), forces_s[2] * np.array([0, 0, 1])
                        )
    ...
    def correct_surface_area(self, inode, struct2aero_mapping, zeta_ts, area):
        ...
        if self.flag_shared_node_by_surfaces[inode]:
            ...
            for isurf in range(1, n_surfaces_shared_by_node):
                ...
                _, span_shared_surf, _, chord_shared_surf = span_chord(
                    i_n_shared_surf, zeta_ts[shared_surf]
                )
    ...
    def write_induced_aoa_of_each_node(self, ts, list_aoa_induced):
        ...
        np.savetxt(
            self.folder + "/aoa_induced_ts_{}.txt".format(ts),
    ...
    def compute_aoa_cl0_from_airfoil_data(self, aerogrid):
        ...
        for i, airfoil in enumerate(aerogrid.aero_dict["airfoils"]):
            ...
            self.list_aoa_cl0[i] = get_aoacl0_from_camber(
                airfoil_coords[:, 0], airfoil_coords[:, 1]
            )
...
class EfficiencyCorrection(generator_interface.BaseGenerator):
    ...
    def generate(self, **params):
        ...
        for inode in range(n_node):
            ...
            new_struct_forces[inode, :] = struct_forces[inode, :].copy()
            new_struct_forces[inode, 0:3] *= force_efficiency[
                i_elem, i_local_node, 0, :
            ]  # element wise multiplication
            new_struct_forces[inode, 0:3] += force_efficiency[
                i_elem, i_local_node, 1, :
            ]
            new_struct_forces[inode, 3:6] *= moment_efficiency[
                i_elem, i_local_node, 0, :
            ]
            new_struct_forces[inode, 3:6] += moment_efficiency[
                i_elem, i_local_node, 1, :
