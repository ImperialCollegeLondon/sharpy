...
class VectorVariable:
    ...
    def __repr__(self):
        return (
            "({:s}: {:s}, size: {:g}, index: {:g}, starting at: {:g}, finishing at:"
            " {:g})".format(
                type(self).__name__,
                self.name,
                self.size,
...
class OutputVariable(VectorVariable):
    ...
    def cols_loc(self, value=None):
        if self._cols_loc is None:
            self._cols_loc = np.arange(
                self.first_position, self.end_position, dtype=int
            )  # Original location, should not update
...
class LinearVector:
    ...
    def remove(self, *variable_name_list):
        for variable_name in variable_name_list:
            list_of_variable_names = [
                variable.name for variable in self.vector_variables
            ]
            try:
                remove_variable_index = list_of_variable_names.index(variable_name)
            except ValueError:
                raise ValueError(
                    "Trying to remove non-existent {:s} variable".format(variable_name)
                )
            else:
    ...
    def add(self, vector_variable, **kwargs):
        if isinstance(vector_variable, self.variable_class):
            ...
            raise TypeError(
                "Only can add a variable from either a {:s} object or with name and"
                " kwargs".format(self.variable_class.__name__)
            )
    ...
    def update_indices(self):
        ...
        index_variable_dict = {
            key_index: self.vector_variables[i]
            for i, key_index in enumerate(list_of_indices)
        }
        updated_list = []
    ...
    def update_locations(self):
        for i_var in range(self.num_variables):
            if i_var == 0:
                ...
                self.vector_variables[i_var].first_position = self.vector_variables[
                    i_var - 1
                ].end_position
    ...
    def transform(cls, linear_vector, to_type):
        if not (
            to_type is InputVariable
            or to_type is StateVariable
            or to_type is OutputVariable
        ):
            raise TypeError(
                "Argument should be the class to which the linear vector should be"
                " transformed"
            )
        ...
        for variable in linear_vector.copy():
            list_of_variables.append(
                to_type(name=variable.name, size=variable.size, index=variable.index)
            )
        ...
        for ith, out_variable in enumerate(output_vector):
            try:
                ...
                if not out_variable.rows_loc[-1] == in_variable.cols_loc[-1]:
                    ...
                    err_log += (
                        "Variable {:s}Output rows not coincident with input columns for"
                        " variable\n".format(out_variable.name)
                    )
    ...
    def check_same_vectors(vec1, vec2):
        ...
        assert (
            vec1.num_variables == vec2.num_variables
        ), "Number of variables is not equal"
        ...
        for i_var in range(vec1.num_variables):
            ...
            assert (
                vec1[i_var] is not vec2[i_var]
            ), "Variables in LinearVector reference the same object in memory. Careful!"
    ...
    def load_from_h5_file(cls, variable_type, variables_data):
        ...
        try:
            ...
            raise KeyError(
                f"Unknown variable type {variable_type}. Must be either InputVariable,"
                " OutputVariable or StateVariable"
            )
        for ith_variable in range(n_variables):
            # name = variables_data['names'][ith_variable].astype('U13')  # decode to unicode
            name = str(
                variables_data["names"][ith_variable], "utf8"
            )  # decode to unicode
            list_of_variables.append(
                var_class(
                    name,
