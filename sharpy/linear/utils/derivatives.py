...
class Derivatives:
    ...
    def __init__(self, reference_dimensions, static_state, target_system=None):
        self.target_system = (
            target_system
        )  # type: str # name of target system (aerodynamic/aeroelastic)
        self.transfer_function = (
            None
        )  # type: np.array # matrix of steady-state TF for target system

        self.static_state = static_state  # type: tuple # [fx, fy, fz] at ref state
        self.reference_dimensions = (
            reference_dimensions
        )  # type: dict # name: ref_dimension_value dictionary

        self.separator = "\n" + 80 * "#" + "\n"

        self.dict_of_derivatives = (
            {}
        )  # type: dict # {name:DerivativeSet} Each of the derivative sets DerivativeSet
        ...
        self.coefficients = {
            "force": self.dynamic_pressure * s_ref,
            ...
            "moment_lon_angular_vel": self.dynamic_pressure
            * s_ref
            * c_ref
            * c_ref
            / u_inf,
        ...
        self.steady_coefficients = (
            np.array(self.static_state) / self.coefficients["force"]
        )
    ...
    def initialise_derivatives(
        self, state_space, steady_forces, quat, v0, phi=None, cg=None, tpa=None
    ):
        """
        ...
        if cls.modal:
            vel_inputs_variables = state_space.input_variables.get_variable_from_name(
                "q_dot"
            )
            output_indices = state_space.output_variables.get_variable_from_name(
                "Q"
            ).rows_loc[:6]
            cls.steady_forces = cls.inv_phi_forces.dot(
                cls.steady_forces[output_indices]
            )
        else:
            vel_inputs_variables = state_space.input_variables.get_variable_from_name(
                "beta"
        ...
        try:
            cs_input_variables = state_space.input_variables.get_variable_from_name(
                "control_surface_deflection"
            )
            dot_cs_input_variables = state_space.input_variables.get_variable_from_name(
                "dot_control_surface_deflection"
            )
        except ValueError:
    ...
    def save(self, output_route):
        with h5py.File(
            output_route + "/" + self.filename.replace(".txt", ".h5"), "w"
        ) as f:
        ...
        with open(folder + "/" + filename, mode="w") as outfile:
            ...
            if self.cg is not None:
                ...
                if self.tpa is not None:
                    ...
                    for i in range(3):
                        outfile.write(
                            "\t{:s}_ppal in A = [{:.4f}, {:.4f}, {:.4f}]\t\t\t\n"
                            .format(lab[i], *self.tpa.dot(np.eye(3)[:, i]))
                        )
    ...
    def new_derivative(
        self, frame_of_reference, derivative_calculation=None, name=None
    ):
        """
...
class DerivativeSet:
    ...
    def __init__(
        ...
        self.transfer_function = (
            transfer_function
        )  # type: np.array # steady-state TF for the specific out/in
        ...
        self.frame_of_reference = (
            frame_of_reference
        )  # type: str # name of the FoR (stability or body axes)
    ...
    def angle_derivatives(self):
        ...
        if self.modal:
            delta_nodal_vel = np.linalg.inv(self.phi[:3, :3]).dot(
                cga.T.dot(algebra.der_Peuler_by_v(euler0 * 0, v0))
            )
            delta_nodal_forces = self.inv_phi_forces.dot(
                self.transfer_function[:6, :3].real.dot(delta_nodal_vel)
            )
        else:
            delta_nodal_vel = cga.T.dot(algebra.der_Peuler_by_v(euler0 * 0, v0))
            delta_nodal_forces = self.transfer_function[:6, :3].real.dot(
                delta_nodal_vel
            )
    ...
    def angle_derivatives_tb(self):
        ...
        if self.modal:
            delta_nodal_forces = self.inv_phi_forces.dot(
                self.transfer_function[:6, 6:9].real
            )
        else:
    ...
    def body_derivatives(self):
        ...
        if self.modal:
            body_derivatives = self.inv_phi_forces.dot(body_derivatives).dot(
                self.inv_phi_vel
            )
    ...
    def control_surface_derivatives(self):
        ...
        assert body_derivatives.shape == (
            6,
            2 * self.n_control_surfaces,
        ), "Incorrect TF shape"
