...
class LinearBeam(BaseElement):
    ...
    settings_description["remove_dofs"] = (
        "Remove desired degrees of freedom (flexible DOFs, "
    ...
    settings_description["remove_rigid_states"] = (
        "(For Stability Derivatives) - Remove RIGID STATES from SS leaving"
    ...
    def initialise(self, data, custom_settings=None):
        ...
        self.settings["rigid_modes_ppal_axes"] = data.settings["Modal"][
            "rigid_modes_ppal_axes"
        ]  # use the same value as in Modal solver
        ...
        self.linearisation_vectors[
            "forces_struct"
        ] = self.tsstruct0.steady_applied_forces.reshape(
            -1, order="C"
        )  # B frame
    ...
    def assemble(self, t_ref=None):
        ...
        if (
            self.settings["modal_projection"]
            and self.settings["remove_sym_modes"]
            and self.clamped
        ):
    ...
    def remove_rigid_states(self):
        ...
        if self.sys.modal:
            ...
            self.ss.state_variables.modify(
                "q", size=self.ss.state_variables[0].size - num_rig_dof
            )
            self.ss.state_variables.update_locations()

            retain_state = np.zeros((self.ss.states - 9, self.ss.states))
            retain_state[
                : self.ss.state_variables[0].size, : self.ss.state_variables[0].size
            ] = np.eye(self.ss.state_variables[0].size)
            retain_state[
                self.ss.state_variables[0].size :, self.ss.state_variables[0].size + 9 :
            ] = np.eye(self.ss.state_variables[0].size)
            ...
            self.ss.state_variables.modify(
                "q_dot", size=self.ss.state_variables[1].size - 9
            )
            self.ss.state_variables.update_locations()
            ...
            retain_state[
                eta_size : 2 * eta_size, eta_size + 9 : 2 * eta_size + 9
            ] = np.eye(eta_size)
    ...
    def remove_symmetric_modes(self):
        ...
        for i in range(total_modes // 2):
            ...
            jo = 2 * i + 1
            modes_sym[:, je] = (
                1.0 / np.sqrt(2) * (self.sys.U[:, je] + self.sys.U[:, jo])
            )
            modes_sym[:, jo] = (
                1.0 / np.sqrt(2) * (self.sys.U[:, je] - self.sys.U[:, jo])
            )
        ...
        ind_w1 = [
            6 * i + 2 for i in range(self.sys.structure.num_node // 2)
        ]  # Wing 1 nodes are in the first half rows
        ind_w1_x = [
            6 * i for i in range(self.sys.structure.num_node // 2)
        ]  # Wing 1 nodes are in the first half rows
        ind_w1_y = [
        ...
        ind_w2 = [
            6 * i + 2
            for i in range(
                self.sys.structure.num_node // 2, self.sys.structure.num_node - 1
            )
        ...
        self.sys.U = modalutils.mode_sign_convention(
            self.sys.structure.boundary_conditions,
            self.sys.U,
    ...
    def unpack_ss_vector(self, x_n, u_n, struct_tstep):
        ...
        if u_n is not None:
            ...
            for i_node in vdof[vdof >= 0]:
                ...
                steady_applied_forces[i_node + 1, :3] = cba.dot(
                    u_n[6 * i_node : 6 * i_node + 3]
                )
                steady_applied_forces[i_node + 1, 3:] = cba.dot(
                    u_n[6 * i_node + 3 : 6 * i_node + 6]
                )
            i_elem = self.sys.structure.node_master_elem[0, 0]
            i_local_node = self.sys.structure.node_master_elem[0, 1]
            cba = cba_m(struct_tstep.psi[i_elem, i_local_node])
            steady_applied_forces[0, :3] = cba.dot(u_n[-10:-7]) - np.sum(
                steady_applied_forces[1:, :3], 0
            )
            steady_applied_forces[0, 3:] = cba.dot(u_n[-7:-4]) - np.sum(
                steady_applied_forces[1:, 3:], 0
            )
        ...
        try:
            ...
            C_grav[
                :-rig_dof, -rig_dof:
            ] = Csr  # TODO: sort out changing q vector with euler
        ...
        current_time_step.total_gravity_forces = (
            total_gravity_forces + struct_tstep.total_gravity_forces
        )
        current_time_step.unsteady_applied_forces = (
            unsteady_applied_forces + struct_tstep.unsteady_applied_forces
        )
        new_quat = quat + struct_tstep.quat
        current_time_step.quat = new_quat / np.linalg.norm(new_quat)
        current_time_step.steady_applied_forces = (
            steady_applied_forces + struct_tstep.steady_applied_forces
        )
        ...
        for i_elem in range(self.tsstruct0.num_elem):
            for i_node in range(self.tsstruct0.num_node_elem):
                psi[i_elem, i_node, :] = np.linalg.inv(
                    algebra.crv2tan(self.tsstruct0.psi[i_elem, i_node]).T
                ).dot(eta[i_node + 3 : i_node + 6])
    ...
    def recover_accelerations(self, full_ss):
        ...
        input_variables = LinearVector.transform(
            full_ss.output_variables, to_type=InputVariable
        )

        output_variables = full_ss.output_variables.copy()
        acceleration_variables = []
        for var in self.ss.output_variables[
            self.ss.output_variables.num_variables // 2 :
        ]:
            new_var = var.copy()
            new_var.name += "_dot"
        ...
        acc_gain[-self.ss.states // 2 :, : self.ss.inputs] = self.ss.B[
            self.ss.states // 2 :
        ]
        acc_gain[-self.ss.states // 2 :, self.ss.inputs :] = self.ss.A[
            self.ss.states // 2 :, :
        ]

        acceleration_recovery = libss.Gain(
            acc_gain, input_vars=input_variables, output_vars=output_variables
        )

        if self.sys.modal:
            output_variables = []
            for var in self.sys.Kout.output_variables[
                self.sys.Kout.output_variables.num_variables // 2 :
            ]:
                new_var = var.copy()
                new_var.name += "_dot"
                output_variables.append(new_var)
            modal_gain = libss.Gain(
                self.sys.U,
                input_vars=LinearVector.transform(
                    LinearVector(acceleration_variables), to_type=InputVariable
                ),
