...
class LinAeroEla:
    ...
    def __init__(
        self, data, custom_settings_linear=None, uvlm_block=False, chosen_ts=None
    ):
        self.data = data
        ...
        if uvlm_block:
            self.linuvlm = linuvlm.DynamicBlock(
                ...
                for_vel=np.hstack(
                    (cga.dot(self.tsstr.for_vel[:3]), cga.dot(self.tsstr.for_vel[3:]))
                ),
            )
        else:
    ...
    def assemble_ss(self, beam_num_modes=None, wake_prop_settings=None):
        ...
        if self.rigid_body_motions:
            ...
            Kas = np.block(
                [
                    [self.Kdisp[:, : -self.num_dof_rig], Zblock],
                    [
    ...
    def get_gebm2uvlm_gains(self):
        ...
        Kdisp_vel = np.zeros(
            (3 * self.linuvlm.Kzeta, self.num_dof_str)
        ...
        jj_for_tra = range(
            self.num_dof_str - self.num_dof_rig, self.num_dof_str - self.num_dof_rig + 3
        )
        jj_for_rot = range(
            self.num_dof_str - self.num_dof_rig + 3,
            self.num_dof_str - self.num_dof_rig + 6,
        )
...
if __name__ == "__main__":
    ...
    M, cref = 4.0, 1.0
    Uinf = 25.0
    dict_config["LinearUvlm"] = {"dt": cref / M / Uinf, "integr_order": 2, "Uref": 1.0}

    Sol = AeroElaDyn(
        tsaero=hd.tsaero00000,
        tsstr=hd.tsstr00000,
        aero2str_mapping=hd.aero2str,
        settings=dict_config,
    )
