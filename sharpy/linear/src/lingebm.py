...
class FlexDynamic:
    ...
    def __init__(self, tsinfo, structure=None, custom_settings=dict()):
        ...
        self.use_principal_axes = self.settings.get(
            "rigid_modes_ppal_axes", False
        )  # this setting is inherited from the setting in Modal solver
        ...
        self.scaled_reference_matrices = (
            dict()
        )  # keep reference values prior to time scaling
    ...
    def sort_repeated_evecs(self, evecs, evals):
        num_rbm = np.sum(evals.__abs__() == 0.0)
    ...
    def euler_propagation_equations(self, tsstr):
        ...
        if self.Mstr.shape[0] == num_flex_dof + 10:
            ...
            warnings.warn(
                "Euler parametrisation not implemented - Either rigid body modes are"
                " not being used or this method has already been called."
            )
    ...
    def linearise_gravity_forces(self, tsstr=None):
        ...
        if self.use_euler:
            ...
            # Pga = Cga.T
            # Pag = Pga.T
            Cag = algebra.quat2rotation(
                tsstr.quat
            )  # Rotation matrix FoR G rotated by quat
            Pag = Cag.T
            Pga = Pag.T
        ...
        if self.settings["print_info"]:
            ...
            cout.cout_wrap(
                "\tX_CG A -> %.2f %.2f %.2f" % (Xcg_A[0], Xcg_A[1], Xcg_A[2]), 1
            )
        ...
        for i_node in range(num_node):
            ...
            bc_at_node = self.structure.boundary_conditions[
                i_node
            ...
            if bc_at_node == 1:  # clamp (only rigid-body)
                ...
                dofs_at_node = 6
                jj_tra = 6 * self.structure.vdof[i_node] + np.array(
                    [0, 1, 2], dtype=int
                )  # Translations
                jj_rot = 6 * self.structure.vdof[i_node] + np.array(
                    [3, 4, 5], dtype=int
                )  # Rotations
            else:
                raise NameError(
                    "Invalid boundary condition (%d) at node %d!" % (bc_at_node, i_node)
                )
            ...
            if bc_at_node != 1:
                ...
                Xcg_B = Cba.dot(
                    -np.array([Mss_node[2, 4], Mss_node[0, 5], Mss_node[1, 3]])
                    / Mss_node[0, 0]
                )
                ...
                if self.settings["print_info"]:
                    cout.cout_wrap(
                        "Node %2d \t-> B %.3f %.3f %.3f"
                        % (i_node, Xcg_B[0], Xcg_B[1], Xcg_B[2]),
                        2,
                    )
                    cout.cout_wrap(
                        "\t\t\t-> A %.3f %.3f %.3f"
                        % (Xcg_A_n[0], Xcg_A_n[1], Xcg_A_n[2]),
                        2,
                    )
                    cout.cout_wrap(
                        "\t\t\t-> G %.3f %.3f %.3f"
                        % (Xcg_G_n[0], Xcg_G_n[1], Xcg_G_n[2]),
                        2,
                    )
            ...
            if self.use_euler:
                if bc_at_node != 1:
                    ...
                    Kss_grav[np.ix_(jj_rot, jj_rot)] -= Tan.dot(
                        Xcg_Bskew.dot(algebra.der_Ccrv_by_v(psi, fgravA))
                    )
                    Kss_grav[np.ix_(jj_rot, jj_rot)] -= algebra.der_TanT_by_xv(
                        psi, Xcg_Bskew.dot(Cbg.dot(fgravG))
                    )

                    # Nodal forces due to gravity -> linearisation terms wrt to delta_euler
                    Csr_grav[jj_tra, -3:] -= algebra.der_Peuler_by_v(
                        tsstr.euler, fgravG
                    )

                    # Nodal moments due to gravity -> linearisation terms wrt to delta_euler
                    Csr_grav[jj_rot, -3:] -= Tan.dot(
                        Xcg_Bskew.dot(
                            Cba.dot(algebra.der_Peuler_by_v(tsstr.euler, fgravG))
                        )
                    )
                    ...
                    Krs_grav[3:6, jj_rot] += np.dot(
                        algebra.skew(fgravA), algebra.der_Ccrv_by_v(psi, Xcg_B)
                    )
                ...
                if bc_at_node != 1:
                    ...
                    # Nodal forces due to gravity -> linearisation terms wrt to delta_euler
                    Csr_grav[jj_tra, -4:] -= algebra.der_CquatT_by_v(
                        tsstr.quat, fgravG
                    )  # ok
                    # Crr_grav[:3, -4:] -= algebra.der_CquatT_by_v(tsstr.quat, fgravG)  # not ok - see below

                    # Nodal moments due to gravity -> linearisation terms wrt to delta_euler
                    Csr_grav[jj_rot, -4:] -= Tan.dot(
                        Xcg_Bskew.dot(
                            Cba.dot(algebra.der_CquatT_by_v(tsstr.quat, fgravG))
                        )
                    )
        ...
        if self.use_euler:
            ...
            Crr_grav[3:6, -3:] -= algebra.skew(Xcg_A).dot(
                algebra.der_Peuler_by_v(tsstr.euler, FgravG)
            )
        else:
            # Total gravity forces acting at the A frame
            ...
            # Total moments due to gravity in A frame
            Crr_grav[3:6, -4:] -= algebra.skew(Xcg_A).dot(
                algebra.der_CquatT_by_v(tsstr.quat, FgravG)
            )
        ...
        if (
            self.Kstr[:flex_dof, :flex_dof].shape != self.Kstr.shape
        ):  # If the beam is free, update rigid terms as well
    ...
    def linearise_applied_forces(self, tsstr=None):
        ...
        stiff_flex = np.zeros(
            (flex_dof, flex_dof), dtype=float
        )  # flex-flex partition of K
        stiff_rig = np.zeros(
            (rig_dof, flex_dof), dtype=float
        )  # rig-flex partition of K
        ...
        for i_node in range(num_node):
            ...
            if bc_at_node == 1:  # clamp (only rigid-body)
                ...
                raise NameError(
                    "Invalid boundary condition ({}) at node {}".format(
                        bc_at_node, i_node
                    )
                )
            ...
            if bc_at_node != 1:
                ...
                stiff_rig[:3, jj_rot] -= algebra.der_Ccrv_by_v(
                    psi, fext_b
                )  # Rigid body contribution

                # Externally applied moments in B frame
                stiff_flex[np.ix_(jj_rot, jj_rot)] -= algebra.der_TanT_by_xv(
                    psi, mext_b
                )
                ...
                stiff_rig[3:6, jj_rot] -= algebra.skew(Ra).dot(
                    algebra.der_Ccrv_by_v(psi, fext_b)
                )  # delta psi term
            ...
            if bc_at_node == 1:
                ...
                jj_rot = 6 * self.structure.vdof[closest_node] + np.array(
                    [3, 4, 5], dtype=int
                )
        ...
        if dlti:  # ---------------------------------- assemble discrete time
            if self.discr_method in ["zoh", "bilinear"]:
                ...
                if modal:  # Modal projection
                    if self.proj_modes == "undamped":
                        ...
                        Ass, Bss, Css, Dss = newmark_ss(
                            np.linalg.inv(
                                np.dot(
                                    self.U[:, :Nmodes].T,
                                    np.dot(self.Mstr, self.U[:, :Nmodes]),
                                )
                            ),
                            Ccut,
                            np.dot(
                                self.U[:, :Nmodes].T,
                                np.dot(self.Kstr, self.U[:, :Nmodes]),
                            ),
                            self.dt,
                            self.newmark_damp,
                        )
                        ...
                        self.Kin.input_variables = LinearVector(
                            [
                                InputVariable(
                                    "forces_n", size=self.Mstr.shape[0], index=0
                                )
                            ]
                        )
                        self.Kin.output_variables = LinearVector(
                            [OutputVariable("Q", size=Nmodes, index=0)]
                        )

                        self.Kout = libss.Gain(sc.linalg.block_diag(*[Phi, Phi]))
                        self.Kout.input_variables = LinearVector(
                            [
                                InputVariable("q", size=Nmodes, index=0),
                                InputVariable("q_dot", size=Nmodes, index=1),
                            ]
                        )
                        output_variables = LinearVector(
                            [
                                OutputVariable("eta", size=self.num_dof_flex, index=0),
                                OutputVariable(
                                    "eta_dot", size=self.num_dof_flex, index=1
                                ),
                            ]
                        )
                        if not self.clamped:
                            output_variables.add(
                                "beta_bar", size=self.num_dof_rig, index=0.5
                            )
                    ...
                    state_variables = output_variables.transform(
                        output_variables, to_type=StateVariable
                    )
                    ...
                    Ass, Bss, Css, Dss = newmark_ss(
                        self.Minv, self.Cstr, self.Kstr, self.dt, self.newmark_damp
                    )
                    self.Kin = None
                    self.Kout = None
                    self.SSdisc = libss.StateSpace(Ass, Bss, Css, Dss, dt=self.dt)
                    ...
                    input_variables = LinearVector(
                        [InputVariable("forces_n", size=self.Mstr.shape[0], index=0)]
                    )
                    ...
                    self.SSdisc.output_variables = output_variables
                    self.SSdisc.input_variables = input_variables
                    self.SSdisc.state_variables = LinearVector.transform(
                        output_variables, to_type=StateVariable
                    )
            else:
                raise NameError(
            ...
            if modal:  # Modal projection
                ...
                if self.proj_modes == "undamped":
                    ...
                    self.Kin = np.block(
                        [[self.Kin_damp[iivec, :].real], [self.Kin_damp[iivec, :].imag]]
                    )
                    self.Kout = np.block([2.0 * U.real, (-2.0) * U.imag])
                ...
                Ass[range(num_dof), range(num_dof, 2 * num_dof)] = 1.0
                ...
                self.SScont.output_variables = output_variables
                self.SScont.input_variables = input_variables
                self.SScont.state_variables = LinearVector.transform(
                    output_variables, to_type=StateVariable
                )
    ...
    def tune_newmark_damp(self, amplification_factor=0.999):
        ...
        assert (
            self.discr_method == "newmark" and self.dlti
        ...
        def get_res(newmark_damp_log10):
            self.newmark_damp = 10.0 ** (newmark_damp_log10)
            self.assemble()
        ...
        self.newmark_damp = 10.0**exp_opt
    ...
    def update_modal(self):
        ...
        if self.proj_modes == "undamped":
            ...
            eigenvalues, eigenvectors = np.linalg.eig(
                np.linalg.solve(self.Mstr, self.Kstr)
            )
            ...
            phi = modalutils.mode_sign_convention(
                self.structure.boundary_conditions,
                phi,
                not self.clamped,
                self.use_euler,
            )
            ...
            if not self.clamped and self.use_principal_axes:
                phi = modalutils.free_modes_principal_axes(
                    phi, self.Mstr, use_euler=self.use_euler
                )
            ...
            raise NotImplementedError(
                "Projection update for damped systems not yet implemented "
            )
    ...
    def update_truncated_modes(self, nmodes):
        ...
        assert (
            nmodes <= self.Nmodes
        ), "Unable to truncate to %g modes since only %g are available" % (
            nmodes,
            self.Nmodes,
        )
    ...
    def scale_system_normalised_time(self, time_ref):
        ...
        if self.scaled_reference_matrices:
            raise UserWarning(
                "System already time scaled. System may just need an update."
        ...
        if self.num_rig_dof != 0:
            warnings.warn(
                "Time normalisation not yet implemented with rigid body motion."
            )
    ...
    def update_matrices_time_scale(self, time_ref):
        try:
            cout.cout_wrap(
                "Updating C and K matrices and natural frequencies with new normalised"
                " time...",
                1,
            )
        except ValueError:
        ...
        try:
            ...
            raise KeyError(
                "The scaled reference matrices have not been set, most likely because"
                " you are trying to rescale a dimensional system. Make sure your system"
                " is normalised."
            )
...
def newmark_ss(Minv, C, K, dt, num_damp=1e-4):
    ...
    # build StateSpace
    Ass0 = np.block(
        [[Imat - a0 * MinvK, dt * Imat - a0 * MinvC], [-b0 * MinvK, Imat - b0 * MinvC]]
    )
    Ass1 = np.block([[Imat + a1 * MinvK, a1 * MinvC], [b1 * MinvK, Imat + b1 * MinvC]])
