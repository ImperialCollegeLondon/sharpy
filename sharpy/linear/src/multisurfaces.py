...
class MultiAeroGridSurfaces:
    ...
    def get_ind_velocities_at_target_collocation_points(self, target):
        ...
        for ss_in in range(self.n_surf):
            ...
            target.u_ind_coll += Surf_in.get_induced_velocity_over_surface(
                target, target="collocation", Project=False
            )
    ...
    def get_normal_ind_velocities_at_collocation_points(self):
        ...
        for ss_out in range(self.n_surf):
            ...
            if hasattr(Surf_out, "u_ind_coll"):
                Surf_out.u_ind_coll_norm = Surf_out.project_coll_to_normal(
                    Surf_out.u_ind_coll
                )
                ...
                for ss_in in range(self.n_surf):
                    ...
                    Surf_out.u_ind_coll_norm += (
                        Surf_in.get_induced_velocity_over_surface(
    ...
    def get_ind_velocities_at_segments(self, overwrite=False):
        ...
        for ss_out in range(self.n_surf):
            ...
            for ss_in in range(self.n_surf):
                # Buond
                Surf_in = self.Surfs[ss_in]
                Surf_out.u_ind_seg += Surf_in.get_induced_velocity_over_surface(
    ...
    def verify_non_penetration(self, print_info=False):
        ...
        for surf in self.Surfs:
            ...
            ErMax = np.max(np.abs(surf.u_ind_coll_norm + surf.u_input_coll_norm))
            if print_info:
                print("Surface %.2d max abs error: %.3e" % (ss, ErMax))

            assert ErMax < 1e-12 * np.max(
                np.abs(self.Surfs[0].u_ext)
    ...
    def verify_aic_coll(self, print_info=False):
        ...
        for ss_out in range(self.n_surf):
            ...
            Surf_out.u_ind_coll_norm = Surf_out.u_ind_coll_norm.reshape(
                (Surf_out.maps.M, Surf_out.maps.N)
            )
