...
def block_dot(A, B):
    ...
    for arow, brow in zip(A, B):
        assert len(brow) == cB, "B rows do not contain the same number of column blocks"
        assert len(arow) == cA, "A rows do not contain the same number of column blocks"
                    ...
                    for tout, strout in zip([tsum, tdiff], ["(+)", "(-)"]):
                        if tout not in SupportedTypes:
                            if tout in WarningTypes:
                                warnings.warn(
                                    "Undesired type (%s) resulting from %s operations"
                                    " between %s and %s types"
                                    % (tout, strout, type(aa), type(bb))
                                )
                            else:
                                raise NameError(
                                    "Unexpected type (%s) resulting from %s operations"
                                    " between %s and %s types"
                                    % (tout, strout, type(aa), type(bb))
                                )
