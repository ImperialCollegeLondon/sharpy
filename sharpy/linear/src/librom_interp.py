...
def transfer_function(SS_list, wv):
    ...
    Inputs:
    - list of state-space models instances of libss.ss class
    - wv: list of interpolatory weights
...
def FLB_transfer_function(SS_list, wv, U_list, VT_list, hsv_list=None, M_list=None):
    ...
    - U_list, VT_list, hsv_list: small size, thin SVD factors of Gramians square 
    roots of each state-space model. 
        - If hsv_list is none, it is assumed that
            U_list = [ U_i sqrt(hsv_i) ]
            VT_list = [ sqrt(hsv_i) V_i.T ]
          where U_i and V_i.T are square matrices and hsv is an array.

    - M_list: for fast on-line evaluation. Small size product of Gramians
...
class InterpROM():
    ...
    where A is the full-states matric, also need to be provided. This will allow 
