...
def solve_linear(Ref, Pert, solve_beam=True):
    ...
    if solve_beam:
        ...
        usta_uinf = np.concatenate([0.0 * dzeta, 0.0 * dzeta_dot, du_ext])
        ...
        mtot_beam = (
            Ref.mtot + np.dot(Ref.Kmtot, ysta) + np.dot(Ref.Kmtot_disp, dzeta_exp)
        )
    else:
...
def extract_from_data(
    data, assemble=True, zeta_pole=np.zeros((3,)), build_Asteady_inv=False
):
    """
    ...
    zeta_dot = np.concatenate(
        [tsaero.zeta_dot[ss].reshape(-1, order="C") for ss in range(tsaero.n_surf)]
    )
    uext = np.concatenate(
    ...
    if assemble is True:
        ...
        Kas = np.block(
            [
                [Sol.Kdisp, np.zeros((3 * uvlm.Kzeta, gebm.num_dof + 10))],
