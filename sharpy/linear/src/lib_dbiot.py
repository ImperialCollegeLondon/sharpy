...
def eval_seg_exp(ZetaP, ZetaA, ZetaB, vortex_radius, gamma_seg=1.0):
    ...
    DerA = np.zeros((3, 3))
    DerB = np.zeros((3, 3))
    eval_seg_exp_loop(DerP, DerA, DerB, ZetaP, ZetaA, ZetaB, gamma_seg, vortex_radius)
    return DerP, DerA, DerB


def eval_seg_exp_loop(DerP, DerA, DerB, ZetaP, ZetaA, ZetaB, gamma_seg, vortex_radius):
    """
    ...
    ra_vcr_x, ra_vcr_y, ra_vcr_z = 2.0 * algebra.cross3(RA, Vcr)
    rb_vcr_x, rb_vcr_y, rb_vcr_z = 2.0 * algebra.cross3(RB, Vcr)
    ...
    dQ_dRA = np.array(
        [
            [
                -vdot_prod * rb_vcr_x * vcr_x
                + vcr_sca_x
                * (rab_x * (ra2 - ra2_x) - ra_x * ra_y * rab_y - ra_x * ra_z * rab_z),
                -T2 * rb_z
                - vdot_prod * rb_vcr_x * vcr_y
                + vcr_sca_y
                * (rab_x * (ra2 - ra2_x) - ra_x * ra_y * rab_y - ra_x * ra_z * rab_z),
                T2 * rb_y
                - vdot_prod * rb_vcr_x * vcr_z
                + vcr_sca_z
                * (rab_x * (ra2 - ra2_x) - ra_x * ra_y * rab_y - ra_x * ra_z * rab_z),
            ],
            [
                T2 * rb_z
                - vdot_prod * rb_vcr_y * vcr_x
                + vcr_sca_x
                * (rab_y * (ra2 - ra2_y) - ra_x * ra_y * rab_x - ra_y * ra_z * rab_z),
                -vdot_prod * rb_vcr_y * vcr_y
                + vcr_sca_y
                * (rab_y * (ra2 - ra2_y) - ra_x * ra_y * rab_x - ra_y * ra_z * rab_z),
                -T2 * rb_x
                - vdot_prod * rb_vcr_y * vcr_z
                + vcr_sca_z
                * (rab_y * (ra2 - ra2_y) - ra_x * ra_y * rab_x - ra_y * ra_z * rab_z),
            ],
            [
                -T2 * rb_y
                - vdot_prod * rb_vcr_z * vcr_x
                + vcr_sca_x
                * (rab_z * (ra2 - ra2_z) - ra_x * ra_z * rab_x - ra_y * ra_z * rab_y),
                T2 * rb_x
                - vdot_prod * rb_vcr_z * vcr_y
                + vcr_sca_y
                * (rab_z * (ra2 - ra2_z) - ra_x * ra_z * rab_x - ra_y * ra_z * rab_y),
                -vdot_prod * rb_vcr_z * vcr_z
                + vcr_sca_z
                * (rab_z * (ra2 - ra2_z) - ra_x * ra_z * rab_x - ra_y * ra_z * rab_y),
            ],
        ]
    ...
    dQ_dRB = np.array(
        [
            [
                vdot_prod * ra_vcr_x * vcr_x
                + vcr_scb_x
                * (rab_x * (-rb2 + rb2_x) + rab_y * rb_x * rb_y + rab_z * rb_x * rb_z),
                T2 * ra_z
                + vdot_prod * ra_vcr_x * vcr_y
                + vcr_scb_y
                * (rab_x * (-rb2 + rb2_x) + rab_y * rb_x * rb_y + rab_z * rb_x * rb_z),
                -T2 * ra_y
                + vdot_prod * ra_vcr_x * vcr_z
                + vcr_scb_z
                * (rab_x * (-rb2 + rb2_x) + rab_y * rb_x * rb_y + rab_z * rb_x * rb_z),
            ],
            [
                -T2 * ra_z
                + vdot_prod * ra_vcr_y * vcr_x
                + vcr_scb_x
                * (rab_x * rb_x * rb_y + rab_y * (-rb2 + rb2_y) + rab_z * rb_y * rb_z),
                vdot_prod * ra_vcr_y * vcr_y
                + vcr_scb_y
                * (rab_x * rb_x * rb_y + rab_y * (-rb2 + rb2_y) + rab_z * rb_y * rb_z),
                T2 * ra_x
                + vdot_prod * ra_vcr_y * vcr_z
                + vcr_scb_z
                * (rab_x * rb_x * rb_y + rab_y * (-rb2 + rb2_y) + rab_z * rb_y * rb_z),
            ],
            [
                T2 * ra_y
                + vdot_prod * ra_vcr_z * vcr_x
                + vcr_scb_x
                * (rab_x * rb_x * rb_z + rab_y * rb_y * rb_z + rab_z * (-rb2 + rb2_z)),
                -T2 * ra_x
                + vdot_prod * ra_vcr_z * vcr_y
                + vcr_scb_y
                * (rab_x * rb_x * rb_z + rab_y * rb_y * rb_z + rab_z * (-rb2 + rb2_z)),
                vdot_prod * ra_vcr_z * vcr_z
                + vcr_scb_z
                * (rab_x * rb_x * rb_z + rab_y * rb_y * rb_z + rab_z * (-rb2 + rb2_z)),
            ],
        ]
...
def eval_seg_comp(ZetaP, ZetaA, ZetaB, vortex_radius, gamma_seg=1.0):
    ...
    DerA = np.zeros((3, 3))
    DerB = np.zeros((3, 3))
    eval_seg_comp_loop(DerP, DerA, DerB, ZetaP, ZetaA, ZetaB, gamma_seg, vortex_radius)
    return DerP, DerA, DerB
...
def eval_seg_comp_loop(DerP, DerA, DerB, ZetaP, ZetaA, ZetaB, gamma_seg, vortex_radius):
    ...
    dQ_dRA = Dvcross_by_skew3d(Dvcross, -RB) + np.dot(
        Ddiff, der_runit(RA, rainv, -(rainv**3))
    )
    dQ_dRB = Dvcross_by_skew3d(Dvcross, RA) - np.dot(
        Ddiff, der_runit(RB, rbinv, -(rbinv**3))
    )
...
def eval_panel_fast(zetaP, ZetaPanel, vortex_radius, gamma_pan=1.0):
    ...
    r1inv_list = [1.0 / r1_list[ii] for ii in svec]
    ...
    for aa, bb in LoopPanel:
        ...
        dQ_dRA = Dvcross_by_skew3d(Dvcross, -R_list[bb]) + np.dot(
            Ddiff, Der_runit_list[aa]
        )
        dQ_dRB = Dvcross_by_skew3d(Dvcross, R_list[aa]) - np.dot(
            Ddiff, Der_runit_list[bb]
        )
...
def eval_panel_fast_coll(zetaP, ZetaPanel, vortex_radius, gamma_pan=1.0):
    ...
    for aa, bb in LoopPanel:
        ...
        DerP += Dvcross_by_skew3d(Dvcross, RAB) + +np.dot(
            Ddiff, Der_runit_list[aa] - Der_runit_list[bb]
        )
...
if __name__ == "__main__":
    ...
    DPcpp, DVcpp = eval_panel_cpp(
        zetaP, ZetaPanel, 1e-6, gamma_pan=gamma
    )  # vortex_radius
    DPexp, DVexp = eval_panel_exp(zetaP, ZetaPanel, gamma_pan=gamma)
    DPcomp, DVcomp = eval_panel_comp(zetaP, ZetaPanel, gamma_pan=gamma)
    DPfast, DVfast = eval_panel_fast(zetaP, ZetaPanel, vortex_radius, gamma_pan=gamma)
    DPfast_coll = eval_panel_fast_coll(zetaP, ZetaPanel, vortex_radius, gamma_pan=gamma)
    ...
    def run_eval_panel_cpp():
        for ii in range(10000):
            eval_panel_cpp(zetaP, ZetaPanel, 1e-6, gamma_pan=3.0)  # vortex_radius

    def run_eval_panel_fast(vortex_radius=vortex_radius_def):
        for ii in range(10000):
            eval_panel_fast(zetaP, ZetaPanel, vortex_radius, gamma_pan=3.0)

    def run_eval_panel_fast_coll(vortex_radius=vortex_radius_def):
        for ii in range(10000):
            eval_panel_fast_coll(zetaP, ZetaPanel, vortex_radius, gamma_pan=3.0)

    def run_eval_panel_comp():
        for ii in range(10000):
            eval_panel_comp(zetaP, ZetaPanel, gamma_pan=3.0)

    def run_eval_panel_exp():
        for ii in range(10000):
            eval_panel_exp(zetaP, ZetaPanel, gamma_pan=3.0)
