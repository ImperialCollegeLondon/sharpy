...
def generate_documentation():
    ...
    ignore_modules = yaml.load(
        open(sharpydir.SharpyDir + "/docs/docignore.yml", "r"), Loader=yaml.Loader
    )
    ...
    main_msg = (
        ...
        " still a work in progress and therefore, most functions/classes with which"
        " there is not much user interaction are not fully documented. We would"
        " appreciate any help by means of you contributing to our growing"
        " documentation!\n\n\nIf you feel that a function/class is not well documented"
        " and, hence, you cannot use it, feel free to raise an issue so that we can"
        " improve it.\n\n"
    )
...
def write_file(file):
    ...
    try:
        output_documentation_module_page(source, outfile)
    ...
    if docstring is not None:
        ...
        if title == "":
            try:
                ...
                raise exceptions.DocumentationError(
                    "Module %s has been given no title in neither the 1st or 2nd "
