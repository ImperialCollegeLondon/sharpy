...
class AeroTimeStepInfo(object):
    ...
    def __init__(self, dimensions, dimensions_star):
        ...
        for i_surf in range(self.n_surf):
            self.zeta.append(
                np.zeros(
                    (3, dimensions[i_surf, 0] + 1, dimensions[i_surf, 1] + 1),
                    dtype=ct.c_double,
                )
            )
        self.zeta_dot = []
        for i_surf in range(self.n_surf):
            self.zeta_dot.append(
                np.zeros(
                    (3, dimensions[i_surf, 0] + 1, dimensions[i_surf, 1] + 1),
                    dtype=ct.c_double,
                )
        ...
        for i_surf in range(self.n_surf):
            self.normals.append(
                np.zeros(
                    (3, dimensions[i_surf, 0], dimensions[i_surf, 1]), dtype=ct.c_double
                )
            )
        ...
        for i_surf in range(self.n_surf):
            self.forces.append(
                np.zeros(
                    (6, dimensions[i_surf, 0] + 1, dimensions[i_surf, 1] + 1),
                    dtype=ct.c_double,
                )
            )
        # panel forces
        ...
        self.zeta_star = []
        for i_surf in range(self.n_surf):
            self.zeta_star.append(
                np.zeros(
                    (3, dimensions_star[i_surf, 0] + 1, dimensions_star[i_surf, 1] + 1),
                    dtype=ct.c_double,
                )
            )
        ...
        for i_surf in range(self.n_surf):
            self.gamma.append(
                np.zeros(
                    (dimensions[i_surf, 0], dimensions[i_surf, 1]), dtype=ct.c_double
                )
            )

        self.gamma_star = []
        for i_surf in range(self.n_surf):
            self.gamma_star.append(
                np.zeros(
                    (dimensions_star[i_surf, 0], dimensions_star[i_surf, 1]),
                    dtype=ct.c_double,
                )
            )
        ...
        self.dist_to_orig = []
        for i_surf in range(self.n_surf):
            self.dist_to_orig.append(
                np.zeros(
                    (dimensions_star[i_surf, 0] + 1, dimensions_star[i_surf, 1] + 1),
                    dtype=ct.c_double,
                )
            )
    ...
    def copy(self):
        ...
        for i_surf in range(copied.n_surf):
            copied.zeta[i_surf] = self.zeta[i_surf].astype(
                dtype=ct.c_double, copy=True, order="C"
            )

        for i_surf in range(copied.n_surf):
            copied.zeta_dot[i_surf] = self.zeta_dot[i_surf].astype(
                dtype=ct.c_double, copy=True, order="C"
            )

        # panel normals
        for i_surf in range(copied.n_surf):
            copied.normals[i_surf] = self.normals[i_surf].astype(
                dtype=ct.c_double, copy=True, order="C"
            )

        # panel forces
        for i_surf in range(copied.n_surf):
            copied.forces[i_surf] = self.forces[i_surf].astype(
                dtype=ct.c_double, copy=True, order="C"
            )

        # panel forces
        for i_surf in range(copied.n_surf):
            copied.dynamic_forces[i_surf] = self.dynamic_forces[i_surf].astype(
                dtype=ct.c_double, copy=True, order="C"
            )

        # generate placeholder for aero grid zeta_star coordinates
        for i_surf in range(copied.n_surf):
            copied.zeta_star[i_surf] = self.zeta_star[i_surf].astype(
                dtype=ct.c_double, copy=True, order="C"
            )

        # placeholder for external velocity
        for i_surf in range(copied.n_surf):
            copied.u_ext[i_surf] = self.u_ext[i_surf].astype(
                dtype=ct.c_double, copy=True, order="C"
            )

        for i_surf in range(copied.n_surf):
            copied.u_ext_star[i_surf] = self.u_ext_star[i_surf].astype(
                dtype=ct.c_double, copy=True, order="C"
            )

        # allocate gamma and gamma star matrices
        for i_surf in range(copied.n_surf):
            copied.gamma[i_surf] = self.gamma[i_surf].astype(
                dtype=ct.c_double, copy=True, order="C"
            )

        for i_surf in range(copied.n_surf):
            copied.gamma_dot[i_surf] = self.gamma_dot[i_surf].astype(
                dtype=ct.c_double, copy=True, order="C"
            )

        for i_surf in range(copied.n_surf):
            copied.gamma_star[i_surf] = self.gamma_star[i_surf].astype(
                dtype=ct.c_double, copy=True, order="C"
            )

        for i_surf in range(copied.n_surf):
            copied.dist_to_orig[i_surf] = self.dist_to_orig[i_surf].astype(
                dtype=ct.c_double, copy=True, order="C"
            )

        # total forces
        copied.inertial_steady_forces = self.inertial_steady_forces.astype(
            dtype=ct.c_double, copy=True, order="C"
        )
        copied.body_steady_forces = self.body_steady_forces.astype(
            dtype=ct.c_double, copy=True, order="C"
        )
        copied.inertial_unsteady_forces = self.inertial_unsteady_forces.astype(
            dtype=ct.c_double, copy=True, order="C"
        )
        copied.body_unsteady_forces = self.body_unsteady_forces.astype(
            dtype=ct.c_double, copy=True, order="C"
        )
        ...
        copied.control_surface_deflection = self.control_surface_deflection.astype(
            dtype=ct.c_double, copy=True
        )
    ...
    def generate_ctypes_pointers(self):
        ...
        self.ct_dimensions_star = self.dimensions_star.astype(
            dtype=ct.c_uint, copy=True
        )
        ...
        for i_surf in range(self.n_surf):
            for i_dim in range(NDIM):
                self.ct_zeta_dot_list.append(
                    self.zeta_dot[i_surf][i_dim, :, :].reshape(-1)
                )

        self.ct_zeta_star_list = []
        for i_surf in range(self.n_surf):
            for i_dim in range(NDIM):
                self.ct_zeta_star_list.append(
                    self.zeta_star[i_surf][i_dim, :, :].reshape(-1)
                )
        ...
        for i_surf in range(self.n_surf):
            for i_dim in range(NDIM):
                self.ct_u_ext_star_list.append(
                    self.u_ext_star[i_surf][i_dim, :, :].reshape(-1)
                )
        ...
        for i_surf in range(self.n_surf):
            for i_dim in range(NDIM):
                self.ct_normals_list.append(
                    self.normals[i_surf][i_dim, :, :].reshape(-1)
                )
        ...
        for i_surf in range(self.n_surf):
            for i_dim in range(NDIM * 2):
                self.ct_dynamic_forces_list.append(
                    self.dynamic_forces[i_surf][i_dim, :, :].reshape(-1)
                )

        self.ct_dist_to_orig_list = []
        for i_surf in range(self.n_surf):
            self.ct_dist_to_orig_list.append(
                self.dist_to_orig[i_surf][:, :].reshape(-1)
            )
        ...
        if with_incidence_angle:
            ...
            for i_surf in range(self.n_surf):
                self.ct_incidence_list.append(
                    self.postproc_cell["incidence_angle"][i_surf][:, :].reshape(-1)
                )
        ...
        self.ct_p_dynamic_forces = (
            ct.POINTER(ct.c_double) * len(self.ct_dynamic_forces_list)
        )(*[np.ctypeslib.as_ctypes(array) for array in self.ct_dynamic_forces_list])
        self.ct_p_dist_to_orig = (
            ct.POINTER(ct.c_double) * len(self.ct_dist_to_orig_list)
        )(*[np.ctypeslib.as_ctypes(array) for array in self.ct_dist_to_orig_list])

        if with_incidence_angle:
            self.postproc_cell["incidence_angle_ct_pointer"] = (
                ct.POINTER(ct.c_double) * len(self.ct_incidence_list)
...
class StructTimeStepInfo(object):
    ...
    def __init__(self, num_node, num_elem, num_node_elem=3, num_dof=None, num_bodies=1):
        ...
        # placeholder for CRV
        self.psi = np.zeros(
            (self.num_elem, num_node_elem, 3), dtype=ct.c_double, order="F"
        )
        self.psi_dot = np.zeros(
            (self.num_elem, num_node_elem, 3), dtype=ct.c_double, order="F"
        )
        self.psi_ddot = np.zeros(
            (self.num_elem, num_node_elem, 3), dtype=ct.c_double, order="F"
        )

        # FoR data
        self.quat = np.array([1.0, 0, 0, 0], dtype=ct.c_double, order="F")
        ...
        self.steady_applied_forces = np.zeros(
            (self.num_node, 6), dtype=ct.c_double, order="F"
        )
        self.unsteady_applied_forces = np.zeros(
            (self.num_node, 6), dtype=ct.c_double, order="F"
        )
        self.runtime_steady_forces = np.zeros(
            (self.num_node, 6), dtype=ct.c_double, order="F"
        )
        self.runtime_unsteady_forces = np.zeros(
            (self.num_node, 6), dtype=ct.c_double, order="F"
        )
        ...
        # Multibody
        self.psi_local = np.zeros(
            (self.num_elem, num_node_elem, 3), dtype=ct.c_double, order="F"
        )
        self.psi_dot_local = np.zeros(
            (self.num_elem, num_node_elem, 3), dtype=ct.c_double, order="F"
        )
        ...
        self.forces_constraints_nodes = np.zeros(
            (self.num_node, 6), dtype=ct.c_double, order="F"
        )
        self.forces_constraints_FoR = np.zeros(
            (num_bodies, 10), dtype=ct.c_double, order="F"
        )
        self.mb_dict = None
    ...
    def copy(self):
        ...
        """
        copied = StructTimeStepInfo(
            self.num_node,
            self.num_elem,
            self.num_node_elem,
            ct.c_int(len(self.q) - 10),
        ...
        copied.steady_applied_forces = self.steady_applied_forces.astype(
            dtype=ct.c_double, order="F", copy=True
        )
        copied.unsteady_applied_forces = self.unsteady_applied_forces.astype(
            dtype=ct.c_double, order="F", copy=True
        )
        copied.runtime_steady_forces = self.runtime_steady_forces.astype(
            dtype=ct.c_double, order="F", copy=True
        )
        copied.runtime_unsteady_forces = self.runtime_unsteady_forces.astype(
            dtype=ct.c_double, order="F", copy=True
        )
        copied.gravity_forces = self.gravity_forces.astype(
            dtype=ct.c_double, order="F", copy=True
        )
        copied.total_gravity_forces = self.total_gravity_forces.astype(
            dtype=ct.c_double, order="F", copy=True
        )
        copied.total_forces = self.total_forces.astype(
            dtype=ct.c_double, order="F", copy=True
        )
        ...
        copied.psi_local = self.psi_local.astype(
            dtype=ct.c_double, order="F", copy=True
        )
        copied.psi_dot_local = self.psi_dot_local.astype(
            dtype=ct.c_double, order="F", copy=True
        )
        copied.mb_FoR_pos = self.mb_FoR_pos.astype(
            dtype=ct.c_double, order="F", copy=True
        )
        copied.mb_FoR_vel = self.mb_FoR_vel.astype(
            dtype=ct.c_double, order="F", copy=True
        )
        copied.mb_FoR_acc = self.mb_FoR_acc.astype(
            dtype=ct.c_double, order="F", copy=True
        )
        copied.mb_quat = self.mb_quat.astype(dtype=ct.c_double, order="F", copy=True)
        copied.mb_dquatdt = self.mb_dquatdt.astype(
            dtype=ct.c_double, order="F", copy=True
        )
        copied.forces_constraints_nodes = self.forces_constraints_nodes.astype(
            dtype=ct.c_double, order="F", copy=True
        )
        copied.forces_constraints_FoR = self.forces_constraints_FoR.astype(
            dtype=ct.c_double, order="F", copy=True
        )
    ...
    def get_body(self, beam, num_dof_ibody, ibody):
        ...
        ibody_StructTimeStepInfo.pos_ddot = self.pos_ddot[ibody_nodes, :].astype(
            dtype=ct.c_double, order="F", copy=True
        )
        ...
        ibody_StructTimeStepInfo.psi_local = self.psi_local[ibody_elems, :, :].astype(
            dtype=ct.c_double, order="F", copy=True
        )
        ibody_StructTimeStepInfo.psi_dot = self.psi_dot[ibody_elems, :, :].astype(
            dtype=ct.c_double, order="F", copy=True
        )
        ibody_StructTimeStepInfo.psi_dot_local = self.psi_dot_local[
            ibody_elems, :, :
        ].astype(dtype=ct.c_double, order="F", copy=True)
        ibody_StructTimeStepInfo.psi_ddot = self.psi_ddot[ibody_elems, :, :].astype(
            dtype=ct.c_double, order="F", copy=True
        )
        ...
        ibody_StructTimeStepInfo.runtime_steady_forces = self.runtime_steady_forces[
            ibody_nodes, :
        ].astype(dtype=ct.c_double, order="F", copy=True)
        ibody_StructTimeStepInfo.runtime_unsteady_forces = self.runtime_unsteady_forces[
            ibody_nodes, :
        ].astype(dtype=ct.c_double, order="F", copy=True)
        ibody_StructTimeStepInfo.gravity_forces = self.gravity_forces[
            ibody_nodes, :
        ].astype(dtype=ct.c_double, order="F", copy=True)
        ibody_StructTimeStepInfo.total_gravity_forces = (
            self.total_gravity_forces.astype(dtype=ct.c_double, order="F", copy=True)
        )

        ibody_StructTimeStepInfo.q[0 : num_dof_ibody.value] = self.q[
            ibody_first_dof : ibody_first_dof + num_dof_ibody.value
        ].astype(dtype=ct.c_double, order="F", copy=True)
        ...
        ibody_StructTimeStepInfo.dqdt[-10:-4] = ibody_StructTimeStepInfo.for_vel.astype(
            dtype=ct.c_double, order="F", copy=True
        )
        ibody_StructTimeStepInfo.dqddt[
            -10:-4
        ] = ibody_StructTimeStepInfo.for_acc.astype(
            dtype=ct.c_double, order="F", copy=True
        )
        ibody_StructTimeStepInfo.dqdt[-4:] = self.quat.astype(
            dtype=ct.c_double, order="F", copy=True
        )
        ibody_StructTimeStepInfo.dqddt[-4:] = self.mb_dquatdt[ibody, :].astype(
            dtype=ct.c_double, order="F", copy=True
        )
        ibody_StructTimeStepInfo.mb_dquatdt[ibody, :] = self.mb_dquatdt[
            ibody, :
        ].astype(dtype=ct.c_double, order="F", copy=True)
    ...
    def compute_psi_local_AFoR(self, for0_pos, for0_vel, quat0):
        ...
        for ielem in range(self.psi.shape[0]):
            for inode in range(3):
                self.psi_local[ielem, inode, :] = algebra.rotation2crv(
                    np.dot(Csm, algebra.crv2rotation(self.psi[ielem, inode, :]))
                )
    ...
    def change_to_local_AFoR(self, for0_pos, for0_vel, quat0):
        ...
        for inode in range(self.pos.shape[0]):
            ...
            self.pos[inode, :] = np.dot(Csm, self.pos[inode, :]) + np.dot(
                CAslaveG, for0_pos[0:3] - self.for_pos[0:3]
            )
            self.pos_dot[inode, :] = (
                np.dot(Csm, vel_master)
                - self.for_vel[0:3]
                - algebra.cross3(self.for_vel[3:6], self.pos[inode, :])
            )

            self.gravity_forces[inode, 0:3] = np.dot(
                Csm, self.gravity_forces[inode, 0:3]
            )
            self.gravity_forces[inode, 3:6] = np.dot(
                Csm, self.gravity_forces[inode, 3:6]
            )
        ...
        for ielem in range(self.psi.shape[0]):
            for inode in range(3):
                ...
                self.psi_dot[ielem, inode, :] = self.psi_dot_local[
                    ielem, inode, :
                ].copy()
    ...
    def change_to_global_AFoR(self, for0_pos, for0_vel, quat0):
        ...
        for inode in range(self.pos.shape[0]):
            ...
            self.pos[inode, :] = np.dot(Cms, self.pos[inode, :]) + np.dot(
                CAmasterG, self.for_pos[0:3] - for0_pos[0:3]
            )
            self.pos_dot[inode, :] = (
                np.dot(Cms, vel_slave)
                - for0_vel[0:3]
                - algebra.cross3(for0_vel[3:6], self.pos[inode, :])
            )

            self.gravity_forces[inode, 0:3] = np.dot(
                Cms, self.gravity_forces[inode, 0:3]
            )
            self.gravity_forces[inode, 3:6] = np.dot(
                Cms, self.gravity_forces[inode, 3:6]
            )
        ...
        for ielem in range(self.psi.shape[0]):
            for inode in range(3):
                ...
                self.psi_local[ielem, inode, :] = self.psi[
                    ielem, inode, :
                ].copy()  # Copy here the result from the structural computation
                self.psi_dot_local[ielem, inode, :] = self.psi_dot[
                    ielem, inode, :
                ].copy()  # Copy here the result from the structural computation

                # psi_slave = self.psi[ielem, inode, :] + np.zeros((3,),)
                self.psi[ielem, inode, :] = algebra.rotation2crv(
                    np.dot(Cms, algebra.crv2rotation(self.psi[ielem, inode, :]))
                )

                # Convert psi_dot_local to psi_dot to be used by the rest of the code
                psi_master = self.psi[ielem, inode, :] + np.zeros(
                    (3,),
                )
                cbam = algebra.crv2rotation(psi_master).T
                ...
                self.psi_dot[ielem, inode, :] = np.dot(
                    inv_tm,
                    (
                        np.dot(ts, self.psi_dot_local[ielem, inode, :])
                        + np.dot(cbas, self.for_vel[3:6])
                        - np.dot(cbam, for0_vel[3:6])
                    ),
                )
    ...
    def nodal_type_b_for_2_a_for(
        ...
        for ft in force_type:
            ...
            forces_output.append(
                self.nodal_b_for_2_a_for(fb, beam, filter=filter, ibody=ibody)
            )

        return forces_output

    def extract_resultants(
        self, beam, force_type=["steady", "unsteady", "grav"], ibody=None
    ):
        forces_output = []
        for ft in force_type:
            totals = np.zeros((6))
            if ft == "steady":
                fa = self.nodal_type_b_for_2_a_for(
                    beam, force_type=["steady"], ibody=ibody
                )[0]
            elif ft == "grav":
                fa = self.gravity_forces.copy()
            elif ft == "unsteady":
                fa = self.nodal_type_b_for_2_a_for(
                    beam, force_type=["unsteady"], ibody=ibody
                )[0]

            for i_node in range(beam.num_node):
                totals += fa[i_node, :]
                totals[3:6] += algebra.cross3(self.pos[i_node, :], fa[i_node, 0:3])
