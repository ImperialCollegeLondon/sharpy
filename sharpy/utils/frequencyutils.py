...
def find_limits(wv, **kwargs):
    ...
    """
    freq_upper_limit = kwargs.get('vmax', wv[-1])
    freq_lower_limit = kwargs.get('vmin', wv[0])

    if freq_upper_limit > wv[-1]:
        freq_upper_limit = wv[-1]

    if freq_lower_limit < wv[0]:
        freq_lower_limit = wv[0]
