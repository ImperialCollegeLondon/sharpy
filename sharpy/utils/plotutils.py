    ...
    if not plotly:
        ...
        if aero_tstep is not None:
            for isurf in range(aero_tstep.n_surf):
                ...
                # Wake grid
                ax.plot_wireframe(
                    aero_tstep.zeta_star[isurf][0, : (Mstar - minus_mstar), :],
        ...
        if aero_tstep is not None:
            for isurf in range(aero_tstep.n_surf):
                ...
                for i_m in range(Mstar + 1 - minus_mstar):
                    fig.add_trace(
                        go.Scatter3d(
                            x=aero_tstep.zeta_star[isurf][0, i_m, :],
                ...
                for i_n in range(Nstar + 1):
                    fig.add_trace(
                        go.Scatter3d(
                            x=aero_tstep.zeta_star[isurf][
                                0, : (Mstar + 1 - minus_mstar), i_n
                            ],
                            y=aero_tstep.zeta_star[isurf][
                                1, : (Mstar + 1 - minus_mstar), i_n
                            ],
                            z=aero_tstep.zeta_star[isurf][
                                2, : (Mstar + 1 - minus_mstar), i_n
                            ],
                            mode="lines",
                            line={"color": "grey"},
                            showlegend=False,
