...
def output_documentation(route=None):
    ...
    for k, v in dict_of_solvers.items():
        ...
        try:
            ...
            print(
                'The solver {} does not have a classification. Dumping it into "Other"'
                .format(k)
            )
            solver_folder = "other"
        ...
        if solver.__doc__ is not None:
            print(
                "\tCreating %s"
                % (base_route + "/" + folder + "/" + solver_folder + "/" + filename)
            )
            autodoc_string = ""
            autodoc_string = (
                "\n\n.. autoclass:: "
                + route_to_solver_python
                + k.lower()
                + "."
                + k
                + "\n\t:members:"
            ...
            with open(
                base_route + "/" + folder + "/" + solver_folder + "/" + filename, "w"
            ) as out_file:
    ...
    for solver_type in solver_types:
        ...
        with open(route_solvers + "/" + filename, "w") as out_file:
            ...
            for k in dict_of_solvers.keys():
                ...
                try:
                    if (
                        created_solvers[k].solver_classification.lower() == solver_type
                        and created_solvers[k].__doc__ is not None
                    ):
