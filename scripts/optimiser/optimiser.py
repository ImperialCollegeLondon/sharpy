...
def parse_inputs():
    parser = argparse.ArgumentParser(
        description="""The optimiser.py script is an example of the use of SHARPy as a
...
def optimiser(in_dict, previous_x, previous_y):
    ...
    try:
        os.mkdir(
            (settings_dict["cases_folder"] + "/" + case_dict["name"] + "/").replace(
                "//", "/"
            )
        )
        print("Folder made")
    ...
    case_route = (
        settings_dict["cases_folder"] + "/" + case_dict["name"] + "/"
    ).replace("//", "/")
    ...
    output_route = (
        in_dict["case"]["output_folder"] + "/" + in_dict["case"]["name"] + "/"
    )
    ...
    for k, v in in_dict["optimiser"]["parameters_initial"].items():
        bounds.append(
            {
                "name": in_dict["optimiser"]["parameters"][k],
                "type": "continuous",
    ...
    try:
        ...
        constraints.append({"name": "angle", "constraint": constraint_string})
    except KeyError:
        pass
    ...
    opt = GPyOpt.methods.BayesianOptimization(
        ...
        acquisition_type="EI",
        normalize_y=False,
        initial_design_numdata=in_dict["optimiser"]["numerics"][
    ...
    opt.run_optimization(
        in_dict["optimiser"]["numerics"]["n_iter"],
    ...
    if np.linalg.norm(opt.x_opt - local_x) < 1e-1:
        ...
        print(
            "Improvement over the previous solution with the local min.: ",
            -(local_cost - opt.fx_opt) / opt.fx_opt * 100,
            "%",
        )
        print(
            "The RBF estimation of the cost was off by: ",
...
def evaluate(x_dict, yaml_dict):
    ...
    files, case_name = set_case(
        case_name, yaml_dict["base"], x_dict, yaml_dict["settings"], yaml_dict["case"]
    )
    data = run_case(files)
    ...
    if data is not None:
        ...
        if yaml_dict["settings"]["save_data"]:
            try:
                os.mkdir(
                    yaml_dict["settings"]["cases_folder"]
                    + "/"
                    + yaml_dict["case"]["name"]
                    + "/"
                )
...
def cost_function(data, x_dict, cost_dict, insight=False):
    """
    ...
    try:
        ...
        clearance_cost = cost_sigmoid(clearance, **cost_dict["ground_clearance"])
...
def loads_cost(data, cost_loads_dict):
    index2load = {0: "Torsion", 1: "OOP", 2: "IP"}
    try:
        loads_array = np.loadtxt(
            cost_loads_dict["reference_loads"], skiprows=1, delimiter=","
        )
    except OSError:
        try:
            ...
            warnings.warn(
                "Not found reference_loads file, anywhere. Filling up with ones instead"
            )
            loads_array = np.ones((data.structure.ini_info.psi.shape[0], 4))
    ...
    separate_cost = np.max(loads_history, axis=0) - 1.0
...
def cost_sigmoid(z, z_min, z_0, x_offset=0.5, offset=0.0, scale=1.0):
    # I need the input to f to be between 0 and 1 for relevant values
    def sigmoid_mod(x, c=4, x_offset=0.0, offset=0.0, scale=1.0):
        return scale / (1.0 + np.exp(c * (x - x_offset))) + offset

    val = sigmoid_mod(
        (z - z_min) / (z_0 - z_min), x_offset=x_offset, offset=offset, c=5, scale=scale
    )
    return val
