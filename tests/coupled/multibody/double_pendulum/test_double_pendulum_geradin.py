...
geradin_FoR1 = np.array(
    [
        [0.00756934, 0.0266485],
        [0.134225, 0.0241027],
        [0.309222, 0.100987],
        [0.418117, 0.393606],
        [0.490855, 0.713752],
...
class TestDoublePendulum(unittest.TestCase):
    ...
    def setUp(self):
        ...
        l1 = 1.0
        m1 = 1.0
        theta_ini1 = 90.0 * deg2rad

        # Beam2
        nnodes2 = nnodes1
        l2 = l1
        m2 = m1
        theta_ini2 = 00.0 * deg2rad
        ...
        beam1.StructuralInformation.body_number = np.zeros(
            (beam1.StructuralInformation.num_elem,), dtype=int
        )
        ...
        beam2.StructuralInformation.generate_uniform_sym_beam(
            node_pos2,
            mass_per_unit_length,
            mass_iner,
            EA,
        ...
        beam2.StructuralInformation.body_number = np.zeros(
            (beam1.StructuralInformation.num_elem,), dtype=int
        )
        beam2.StructuralInformation.boundary_conditions[0] = 1
        beam2.StructuralInformation.boundary_conditions[-1] = -1
        beam2.StructuralInformation.lumped_mass_nodes = np.array(
            [nnodes2 - 1], dtype=int
        )
        ...
        SimInfo.define_uinf(np.array([0.0, 1.0, 0.0]), 1.0)
        ...
        SimInfo.solvers["SHARPy"]["log_folder"] = (
        ...
        MB2.FoR_position = np.array(
            [node_pos2[0, 0], node_pos2[0, 1], node_pos2[0, 2], 0.0, 0.0, 0.0]
        )
        MB2.FoR_velocity = np.zeros(
            (6,),
        )
    ...
    def run_and_assert(self, name):
        ...
        pos_tip_data = np.loadtxt(
            "%sstruct_pos_node%d.dat" % (output_path, nnodes1 * 2 - 1),
        )
    ...
    def test_doublependulum_ga(self):
        ...
        nb_solver_path = os.path.abspath(
        ...
        ga_solver_path = os.path.abspath(
            os.path.dirname(os.path.realpath(__file__)) + "/" + name_ga + ".sharpy"
        )
        ...
        nb_pos_tip_data = np.loadtxt(
            "%sstruct_pos_node%d.dat" % (nb_output_path, nnodes1 * 2 - 1),
        )
        ...
        ga_pos_tip_data = np.loadtxt(
            "%sstruct_pos_node%d.dat" % (ga_output_path, nnodes1 * 2 - 1),
        )
