...
class TestFloatingForces(unittest.TestCase):
    ...
    def test_compute_xf_zf(self):
        ...
        EA = 384243000.0  # Extensional stiffness
    ...
    def test_generate_mooringlinefd(self):
        ...
        w = 698.094  # Aparent weight 77.7066*9.81 # Apparent mass per unit length times gravity
        EA = 384243000.0  # Extensional stiffness
        cb = 0.0  # Seabed friction coefficient

        zf = 320.0 - 70.0

        # xf0 = 853.87
        # xf_list = np.arange(653.00, 902.50 + 1., 1.)
        xf_list = np.arange(653.00, 902.50, 10.0)
        npoints = xf_list.shape[0]
        output = np.zeros((npoints, 4))
        for i in range(npoints):
            hf, vf = ff.quasisteady_mooring(
                xf_list[i], zf, l, w, EA, cb, hf0=None, vf0=None
            )
            # print(xf0, zf0, hf0, vf0)
            lb = np.maximum(l - vf / w, 0)
            # print("Suspended lenght = %f" % (l - lb))
            output[i, :] = np.array(
                [xf_list[i], np.sqrt(vf**2 + hf**2) * 1e-3, hf * 1e-3, (l - lb)]
            )
        ...
        if save_txt:
            np.savetxt(
                "sharpy_mooringlinefd.txt",
                output,
                header="# DISTANCE(m) TENSION(kN) HTENSION(kN) SUSPL(m)",
            )
    ...
    def test_change_system(self):
        ...
        wt_matrix_num = np.zeros(
            (6, 6),
        )
        for idof in range(6):
            for jdof in range(6):
                wt_matrix_num[idof, jdof] = 10.0 * idof + jdof
        ...
        for idof in range(6):
            for jdof in range(6):
                self.assertEqual(
                    wt_matrix_num[idof, jdof], undo_sharpy_matrix[idof, jdof]
                )
    ...
    def test_time_wave_forces(self):
        ...
        xi[0, 0] = 1.0 + 0j
        xi[1, 0] = 1.0 + 0j
        w_xi = np.array([0.0, 4.0])
        wave_force = np.zeros((ntime_steps, nrealisations), dtype=np.complex)
        for ireal in range(nrealisations):
            wave_force[:, ireal] = ff.time_wave_forces(Tp, Hs, dt, time, xi, w_xi)[
                :, 0
            ]  # Keep only on dimension
        ...
        for ireal in range(nrealisations):
            ns[:, ireal] = (
                dt
                / ntime_steps
                * np.abs(fft(wave_force[:, ireal])[: ntime_steps // 2]) ** 2
            )
            ns[1:, ireal] *= 2
        # To rad/s
        ns /= 2.0 * np.pi
        w_ns = (np.fft.fftfreq(ntime_steps, d=dt)[: ntime_steps // 2]) * 2.0 * np.pi
        ...
        for iomega in range(w_js.shape[0]):
            error = (
                np.interp(w_js[iomega], w_ns, avg_noise_spectrum)
                - zero_noise_spectrum[iomega]
            )
            if error > 0.1:
        ...
        if save_fig:
            np.savetxt(
                "zero_noise_jonswap.txt", np.column_stack((w_js, zero_noise_spectrum))
            )
            np.savetxt("realisations_jonswap.txt", np.column_stack((w_ns, np.abs(ns))))
            np.savetxt(
                "average_jonswap.txt", np.column_stack((w_ns, avg_noise_spectrum))
            )
