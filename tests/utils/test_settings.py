...
class TestSettings(unittest.TestCase):
    ...
    def test_settings_to_custom_types(self):
        ...
        # integer variable
        self.assertEqual(
            in_dict["integer_var"], 1234, "Integer test for assigned values not passed"
        )
        # float variable
        self.assertEqual(
            in_dict["float_var"], 1.234, "Float test for assigned values not passed"
        )
        # string variable
        self.assertEqual(
            in_dict["str_var"], "aaaa", "String test for assigned values not passed"
        )
        # bool variable
        self.assertEqual(
            in_dict["bool_var"], True, "Bool test for assigned values not passed"
        )
        # list variable
        for i in range(4):
            self.assertEqual(
                in_dict["list_var"][i],
                split_list[i],
                "List test for assigned values not passed",
            )
        # float list variable
        for i in range(3):
            self.assertEqual(
                in_dict["float_list_var"][i],
                split_float_list[i],
                "Floating point list test for assigned values not passed",
            )
        ...
        # integer variable
        self.assertEqual(
            in_default_dict["integer_var"],
            default_dict["integer_var"],
            "Integer test for default values not passed",
        )
        # float variable
        self.assertEqual(
            in_default_dict["float_var"],
            default_dict["float_var"],
            "Float test for default values not passed",
        )
        # string variable
        self.assertEqual(
            in_default_dict["str_var"],
            default_dict["str_var"],
            "String test for default values not passed",
        )
        # bool variable
        self.assertEqual(
            in_default_dict["bool_var"],
            default_dict["bool_var"],
            "Bool test for default values not passed",
        )
        # list(str) variable
        for i in range(2):
            self.assertEqual(
                in_default_dict["list_var"][i],
                default_dict["list_var"][i],
                "String list test for default values not passed",
            )
        # list(float) variable
        for i in range(2):
            self.assertEqual(
                in_default_dict["float_list_var"][i],
                default_dict["float_list_var"][i],
        ...
        with self.assertRaises(exceptions.NoDefaultValueException):
            ...
            result = settings.to_custom_types(
                temp_in_dict, types_dict, temp_default_dict
            )
