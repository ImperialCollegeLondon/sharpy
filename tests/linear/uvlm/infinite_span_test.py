...
class Test_infinite_span(unittest.TestCase):
	...
	def setUp_from_params(self,Nsurf,integr_ord,RemovePred,UseSparse):
		'''
		Builds sharpy solution for an infinite span flat wing at zero incidence
		'''

		# Flags
		self.ProducePlots=False
		self.DeleteOutput=True

		# Define Parametrisation
		M,N,Mstar_fact=8,4,50
		...
		self.case_main=self.case_code+'_a%.2d_Nsurf%.2dM%.2dN%.2dwk%.2d'\
							%(int(np.round(100*Alpha0Deg)),Nsurf,M,N,Mstar_fact)
		...
		ws.config['LinearUvlm'] = {	'dt': ws.c_ref/M/Uinf0,
	...
	def test_wagner(self):
		...
		# - set linearsation point at 0 incidence
		# - perturb. state so as to produce a small angle of attack. This is 
		achieved combining changes of:
			- wing lattice orientation
			- wing lattice speed
			- incoming flow orientation
		# - compare lift to Wagner's analytical solution
		...
		for Nsurf in [1,2]:
			for integr_ord in [1,2]:
				for RemovePred in [False,True]:
					for UseSparse in [False,]:
						with self.subTest(
							Nsurf=Nsurf,integr_ord=integr_ord,
							RemovePred=RemovePred,UseSparse=UseSparse):
							self.run_wagner(Nsurf,integr_ord,RemovePred,UseSparse)


	def run_wagner(self,Nsurf,integr_ord,RemovePred,UseSparse):
		'''
		see test_wagner
		'''
		...
		### ----- Step input
		delta_AlphaDeg=1e-2
		delta_AlphaRad=1e-2*np.pi/180.
		dcrv=np.array([0.,delta_AlphaRad,0.])
		uvec0=np.array([Uinf0,0,0])
		uvec=np.dot(algebra.crv2rotation(dcrv).T,uvec0)
		duvec=uvec-uvec0
		...
		Cl_inf=delta_AlphaRad*np.pi*2.
		er_f=np.abs(Ftot_ste[2]/Cl_inf-1.)
		assert (er_f < 1e-2),\
			'Error of lift coefficient (%.2e) too large!'%(er_f,) +\
			'Verify linuvlm.Dynamic.'

		# ... and finally compare against non-linear UVLM
		case_pert='wagner_a%.2d_Nsurf%.2dM%.2dN%.2dwk%.2d'\
					   %(int(np.round(100*delta_AlphaDeg)),Nsurf,M,N,Mstar_fact)
		ws_pert=flying_wings.QuasiInfinite(  
								M=M,N=N,Mstar_fact=Mstar_fact,n_surfaces=Nsurf,
								u_inf=Uinf0,alpha=delta_AlphaDeg,
								aspect_ratio=1e5,
		...
		ws_pert.config['SHARPy']['flow']=['BeamLoader','AerogridLoader','StaticUvlm']
		...
		for ss in range(Nsurf):
			for nn in range(N//Nsurf+1):	
				for mm in range(M+1):
					Ftot_ste_ref+=tsaero.forces[ss][:3,mm,nn]
		...
		er_f=np.max(np.abs(Ftot_ste-Ftot_ste_ref))/Fmag
		er_m=np.max(np.abs(Mtot_ste-Mtot_ste_ref))/Fmag/ws.c_ref
		assert (er_f < 1e-5 and er_m <1e-7),\
		...
		Clv_an=an.wagner_imp_start(delta_AlphaRad,Uinf0,ws.c_ref,tv)
		assert np.abs(Clv_an[-1]/Cl_inf-1.)<1e-2,\
			'Did someone modified this test case?! The time should be enough to reach '\
		...
		for tt in range(1,NT):
			...
			xold=xnew
			# print('%.5d\t%.3f\t%.3e' %(tt,tv[tt],change))
			...
			Er_f_tot[tt]=np.abs(Ftot[tt,2]/Clv_an[tt]-1.)
		...
		# generate figure
		clist=['#003366','#CC3333','#336633','#FF6600']
		fontlabel=22
		...
		if self.ProducePlots:
			fig = plt.figure('Lift time-history',(10,5))
			ax=fig.subplots(1,1)
			ax.plot(sv,Clv_an/Cl_inf,lw=4,ls='-',alpha=0.7,color='r',label=r'Wagner')
			ax.plot(sv,Ftot[:,2]/Cl_inf,lw=2,ls=':',alpha=0.7,color='k',label=r'Total')
			cc=0
			for ss in range(Nsurf):
				for nn in Nplot:
					ax.plot(sv,Fsect[:,ss,2,nn]/Cl_inf,
			...
			ax.set_xlabel(r'normalised time $t=2 U_\infty \tilde{t} / c$')
			ax.set_ylabel(r'$C_l/C_{l_\infty}$')
			ax.set_xlim(0,sv[-1])
			ax.set_ylim(0,1.2)
			plt.legend(ncol=3)
			# plt.show()
