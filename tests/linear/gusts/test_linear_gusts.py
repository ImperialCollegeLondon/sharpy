...
class TestGolandControlSurface(unittest.TestCase):
    ...
    def run_sharpy(self, flow, **kwargs):
        ...
        if not restart:
            ...
            with open(
                self.route_test_dir + "/output/{:s}.pkl".format(ws.case_name), "rb"
            ) as f:
                data = pickle.load(f)
        ...
        for test in test_conditions:
            with self.subTest(test):
                ...
                np.testing.assert_array_almost_equal(
                    u_inf_ext[1 + predictor_offset :, 0, 0],
                ...
                for i_chord in range(1, u_inf_ext.shape[1]):
                    np.testing.assert_array_almost_equal(
                        u_inf_ext[
                            predictor_offset + 1 + i_chord : -i_chord, i_chord, 0
                        ],
                        u_inf_ext[
                            predictor_offset + 1 + i_chord - 1 : -(i_chord + 1),
                            i_chord - 1,
                            0,
                        ],
                    )
...
class TestGusts(unittest.TestCase):
    def test_campbell(self):
        ...
        np.testing.assert_array_almost_equal(
            G_ct[0, 0, :].real, G_dt[0, 0, :].real, decimal=3
        )
