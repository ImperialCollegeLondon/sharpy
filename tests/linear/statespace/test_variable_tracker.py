...
class TestVariables(unittest.TestCase):
    ...
    def setUp(self):
        ...
        input_variables_list = [
            InputVariable("zeta", size=3 * Kzeta, index=0),
    ...
    def test_initialisation(self):
        ...
        assert variable_names == [
            ...
            "zeta_dot",
            "u_gust",
        ], "Error sorting indices in initialisation"
    ...
    def test_modify(self):
        ...
        assert (
            self.input_variables.get_variable_from_name("new_u_gust").size == 5
        ), "Variable not modified correctly"
    ...
    def test_add(self):
        ...
        new_variable_repeated_index = InputVariable(
            "control_surface_2", size=2, index=3
        )
        try:
            ...
            raise IndexError(
                "Variable was added when it should have been rejected because of"
                " repeated index"
            )
        ...
        assert (
            self.input_variables[0].name == new_variable_inbetween.name
        ), "New variable not added at the start of the list"

        self.input_variables.add("var_from_string", size=3, index=10)
        assert (
            self.input_variables[-1].name == "var_from_string"
        ), "Variable from string not added correctly"
    ...
    def test_append(self):
        ...
        assert (
            self.input_variables[-1].name == "last_var"
        ), "Variable not properly appended"
        ...
        try:
            ...
            raise TypeError(
                "Error. Able to append an output variable to an input variable"
            )

        self.input_variables.append(
            InputVariable(name="last_last_var", size=2, index=0)
        )

        assert (
            self.input_variables[-1].name == "last_last_var"
        ), "Variable not properly appended"

    def test_merge(self):
        second_variables_list = [
            InputVariable("eta", size=3, index=0),
    ...
    def test_transform(self):
        new_vector = LinearVector.transform(
            self.input_variables, to_type=OutputVariable
        )

        LinearVector.check_same_vectors(new_vector, self.input_variables)
        assert (
            new_vector.num_variables == self.input_variables.num_variables
        ), "Number of variables not equal"
        assert new_vector.size == self.input_variables.size, "Size not equal"

        for i_var in range(new_vector.num_variables):
            assert (
                new_vector[i_var].name == self.input_variables[i_var].name
            ), "Name does not match"
            assert (
                new_vector[i_var].index == self.input_variables[i_var].index
            ), "Index does not match"
            assert (
                new_vector[i_var].size == self.input_variables[i_var].size
            ), "Size does not match"

        new_vector.append(OutputVariable("last_var", size=2, index=0))

        assert (
            new_vector.num_variables == self.input_variables.num_variables + 1
        ), "Number of variables got updated in the original object"
        assert new_vector[-1].name == "last_var", "Variable not correctly appended"
        assert (
            new_vector[-1].index == new_vector.num_variables - 1
        ), "Variable not correctly appended"
    ...
    def test_save_to_h5(self):
        ...
        loaded_variable = LinearVector.load_from_h5_file(
