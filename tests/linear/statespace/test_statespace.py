...
from sharpy.linear.src.libss import (
    StateSpace,
    SSconv,
    compare_ss,
    scale_SS,
...
class Test_dlti(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.SS.input_variables = LinearVector(
            [
                InputVariable("input1", size=3, index=0),
                InputVariable("input2", size=2, index=1),
            ]
        )
        self.SS.state_variables = LinearVector(
            [StateVariable("state1", size=3, index=0)]
        )
        self.SS.output_variables = LinearVector(
            [
                OutputVariable("output1", size=3, index=0),
    ...
    def test_SSconv(self):
        ...
        SSpr2 = StateSpace(
            *SSconv(libsp.csc_matrix(A), B, libsp.csc_matrix(B1), C, D), dt=0.3
        )
        SSpr3 = StateSpace(
            *SSconv(libsp.csc_matrix(A), libsp.csc_matrix(B), B1, C, D), dt=0.3
        )
        SSpr4 = StateSpace(
            *SSconv(
                libsp.csc_matrix(A), libsp.csc_matrix(B), libsp.csc_matrix(B1), C, D
            ),
            dt=0.3
        )
    ...
    def test_addGain(self):
        ...
        gain_in = Gain(Kin)
        gain_in.input_variables = LinearVector(
            [InputVariable("input1", size=5, index=0)]
        )
        gain_in.output_variables = LinearVector(
            [OutputVariable("output1", size=Nu, index=0)]
        )

        gain_out = Gain(Kout)
        gain_out.input_variables = LinearVector.transform(
            self.SS.output_variables, InputVariable
        )
        gain_out.output_variables = LinearVector(
            [OutputVariable("final_output", size=gain_out.outputs, index=0)]
        )
    ...
    def test_couple(self):
        ...
        SS1 = random_ss(Nx1, Nu1, Ny1, dt=0.2)
        SS2 = random_ss(Nx2, Nu2, Ny2, dt=0.2)

        SS1sp = StateSpace(
            libsp.csc_matrix(SS1.A),
            libsp.csc_matrix(SS1.B),
            libsp.csc_matrix(SS1.C),
            libsp.csc_matrix(SS1.D),
            dt=dt,
        )
        SS2sp = StateSpace(
            libsp.csc_matrix(SS2.A),
            libsp.csc_matrix(SS2.B),
            libsp.csc_matrix(SS2.C),
            libsp.csc_matrix(SS2.D),
            dt=dt,
        )
    ...
    def test_join(self):
        Nx, Nu, Ny = 4, 3, 2
        SS_list = [random_ss(Nx, Nu, Ny, dt=0.2) for ii in range(3)]

        wv = [0.3, 0.5, 0.2]
        SSjoin = join(SS_list, wv)

        kv = np.array([0.0, 1.0, 3.0])
    ...
    def test_remove_inputs(self):
        ...
        assert self.SS.B.shape == (
            Nx,
            self.SS.input_variables.size,
        ), "B matrix not trimmed correctly"
        assert self.SS.D.shape == (
            Ny,
            self.SS.input_variables.size,
        ), "D matrix not trimmed correctly"

        assert (
            self.SS.input_variables[0].rows_loc == rows_loc[0]
        ), "Rows of input 1 not retained correctly"
        assert (
            self.SS.input_variables[1].rows_loc == rows_loc[2]
        ), "Rows of input 3 not retained correctly"

        # sparse system
        self.SSsp.remove_inputs("input2", "input4")
        assert self.SSsp.B.shape == (
            Nx,
            self.SSsp.input_variables.size,
        ), "Bsp matrix not trimmed correctly"
        assert self.SSsp.D.shape == (
            Ny,
            self.SSsp.input_variables.size,
        ), "Dsp matrix not trimmed correctly"

        assert (
            self.SSsp.input_variables[0].rows_loc == rows_loc[0]
        ), "Rows of input 1 not retained correctly in sparse system"
        assert (
            self.SSsp.input_variables[1].rows_loc == rows_loc[2]
    ...
    def test_series(self):
        ...
        SS2.output_variables = LinearVector(
            [
                OutputVariable("input1", size=3, index=0),
        ...
        assert (
            SSnew.outputs == self.SS.outputs
        ), "Number of outputs not the same as self.SS"
...
class TestStateSpaceManipulation(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.ss.state_variables = LinearVector(
            [StateVariable("state1", size=Nx, index=0)]
        )
        self.ss.output_variables = LinearVector(
            [
                OutputVariable("output1", size=1, index=0),
    ...
    def test_retain_channels(self):
        ...
        assert self.ss.inputs == len(
            retain_input_channels
        ), "Number of inputs not correct"
        assert self.ss.input_variables[0].size == 1, "input1 not correct size"
        assert (
            self.ss.input_variables[1].name == "input3"
        ), "input2 not properly removed"

        retain_output_channels = [0, 5]
        self.ss.retain_inout_channels(retain_output_channels, where="out")
        assert self.ss.outputs == len(
            retain_output_channels
        ), "Number of outputs not correct"
        assert self.ss.output_variables[0].size == 1, "output1 not correct size"
        assert (
            self.ss.output_variables[1].name == "output3"
        ), "output2 not properly removed"
    ...
    def test_input_output(self):
        ...
        LinearVector.check_same_vectors(
            self.ss.input_variables, loaded_ss.input_variables
        )

        LinearVector.check_same_vectors(
            self.ss.output_variables, loaded_ss.output_variables
        )

        LinearVector.check_same_vectors(
            self.ss.state_variables, loaded_ss.state_variables
        )
...
class TestGains(unittest.TestCase):
    ...
    def check_gains_equal(gain1, gain2):
        ...
        LinearVector.check_same_vectors(gain1.input_variables, gain2.input_variables)
        LinearVector.check_same_vectors(gain1.output_variables, gain2.output_variables)
