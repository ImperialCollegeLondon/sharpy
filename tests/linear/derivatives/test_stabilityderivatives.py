...
class TestLinearDerivatives(unittest.TestCase):
    ...
    def run_sharpy(self, alpha_deg, flow, target_system, not_run=False):
        ...
        ws.config["StabilityDerivatives"] = {
            "u_inf": ws.u_inf,
            "c_ref": ws.main_chord,
            "b_ref": ws.wing_span,
    ...
    def run_case(self, target_system):
        ...
        nonlinear_sim_flow = (
            ...
            + nonlinear_solver
            + ["AeroForcesCalculator", "SaveParametricCase"]
        )
        ...
        for ith, alpha in enumerate(alpha_vec):
            if alpha == alpha_deg_ref:
                ...
                case = self.run_sharpy(
                    alpha,
                    flow=nonlinear_sim_flow,
                    target_system=target_system,
                    not_run=not_run,
                )
                case_name_db.append(case.case_name)
                case_name = case.case_name
            nlin_forces = np.loadtxt(
                self.route_test_dir
        ...
        linss_data = scio.loadmat(
            self.route_test_dir
            + "/output/{:s}/savedata/{:s}.linss.mat".format(
                ref_case_name, ref_case_name
            )
        )

        # Steady State transfer function
        if target_system == "aerodynamic":
            ss = libss.StateSpace.load_from_h5(
                self.route_test_dir
        ...
        phi = libss.Gain.load_from_h5(
            self.route_test_dir
            + f"/output/{ref_case_name}/"
            + f"save_pmor_data/{ref_case_name}_modal_structrob.h5"
        ).value.real  # other Gain features not needed
        ...
        for i_alpha, dalpha in enumerate(dalpha_vec):
            ...
            Vp2 = algebra.euler2rot(euler0 * 0).T.dot(V0) + algebra.der_Peuler_by_v(
                euler0 * 0, V0
            ).dot(deuler)
            ...
            F0A = (
                linss_data["forces_aero_beam_dof"][0, :3].real / phi[-9, 0]
            )  # A - forces at the linearisation
            M0A = (
                linss_data["forces_aero_beam_dof"][0, 3:6].real / phi[-6, 3]
            )  # A - forces at the linearisation
            ...
            LD = (
                LD0 + algebra.der_Ceuler_by_v(euler0, F0A).dot(deuler) + cga.dot(flin)
            )  # stability axes
            forces[i_alpha, 1:] = LD / qS

            u_body = np.zeros_like(u)
            u_body[vz_ind] = dvz / phi[-7, 2]
            body_forces[i_alpha, 1:] = (
                H0.dot(u_body)[:3].real / phi[-9, 0] / qS
            )  # C_Z_w in A frame
            if self.print_info:
        ...
        with h5py.File(
            ...
            + "/output/"
            + ref_case_name
            + "/derivatives/aerodynamic_stability_derivatives.h5",
            "r",
        ) as f:
            sharpy_force_angle = h5utils.load_h5_in_dict(f)["force_angle_velocity"]

        linsubtests = ((2, cla, "lift"), (0, cda, "drag"), (4, cma, "moment"))
        for test in linsubtests:
            with self.subTest(test):
                try:
                    np.testing.assert_almost_equal(
                        sharpy_force_angle[test[0], 1], test[1], decimal=3
                    )
                    ...
                    print(
                        "Pct error",
                        np.abs(sharpy_force_angle[test[0], 1] - test[1])
                        / test[1]
                        * 100,
                    )
        ...
        for test in nonlinsubtests:
            with self.subTest(test):
                try:
                    np.testing.assert_almost_equal(
                        sharpy_force_angle[test[0], 1], test[1], decimal=2
                    )
                except AssertionError as e:
