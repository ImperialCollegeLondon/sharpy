...
base_mass_main = np.diag(
    [m_bar_main, m_bar_main, m_bar_main, j_bar_main, 0.5 * j_bar_main, 0.5 * j_bar_main]
)
...
base_mass_tail = np.diag(
    [m_bar_tail, m_bar_tail, m_bar_tail, j_bar_tail, j_bar_tail * 0.5, j_bar_tail * 0.5]
)
...
def generate_dyn_file():
    ...
    if with_forced_vel:
        ...
        for it in range(n_tstep):
            ...
            forced_for_vel[it, 3] = (
                2 * np.pi / period * amplitude * np.sin(2 * np.pi * dt * it / period)
            )
            forced_for_acc[it, 3] = (
                (2 * np.pi / period) ** 2
                * amplitude
                * np.cos(2 * np.pi * dt * it / period)
            )
...
def generate_fem():
    ...
    # outer right wing
    beam_number[we : we + n_elem_main1] = 0
    y[wn : wn + n_node_main2 - 1] = (
        y[wn - 1]
        + np.linspace(0.0, np.cos(lambda_dihedral) * span_main2, n_node_main2)[1:]
    )
    z[wn : wn + n_node_main2 - 1] = (
        z[wn - 1]
        + np.linspace(0.0, np.sin(lambda_dihedral) * span_main2, n_node_main2)[1:]
    )
    ...
    x[wn : wn + n_node_fuselage - 1] = np.linspace(
        0.0, length_fuselage, n_node_fuselage
    )[1:]
    z[wn : wn + n_node_fuselage - 1] = np.linspace(
        0.0, offset_fuselage, n_node_fuselage
    )[1:]
    ...
    z[wn : wn + n_node_fin - 1] = (
        z[end_of_fuselage_node] + np.linspace(0.0, fin_height, n_node_fin)[1:]
    )
...
def generate_aero_file():
    ...
    control_surface_hinge_coord[
        0
    ] = -0.25  # nondimensional wrt elastic axis (+ towards the trailing edge)
    ...
    control_surface_hinge_coord[
        1
    ] = -0.0  # nondimensional wrt elastic axis (+ towards the trailing edge)
...
def generate_solver_file():
    ...
    settings["DynamicCoupled"] = {
        ...
        "network_settings": {
            ...
            "input_network_settings": {"address": "127.0.0.1", "port": 64011},
