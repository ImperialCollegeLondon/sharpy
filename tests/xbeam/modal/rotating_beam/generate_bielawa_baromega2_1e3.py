...
def generate_aero_file():
    ...
    with h5.File(route + "/" + case_name + ".aero.h5", "a") as h5file:
        ...
        airfoils_group.create_dataset(
            "0", data=np.column_stack((np.linspace(0.0, 1.0, 10), np.zeros(10)))
        )

        # chord
        chord_input = h5file.create_dataset(
            "chord",
            data=np.ones(
                (num_elem, num_node_elem),
            ),
        )
        dim_attr = chord_input.attrs["units"] = "m"

        # twist
        twist_input = h5file.create_dataset(
            "twist",
            data=np.zeros(
                (num_elem, num_node_elem),
            ),
        ...
        airfoil_distribution_input = h5file.create_dataset(
            "airfoil_distribution", data=np.zeros((num_elem, num_node_elem), dtype=int)
        )

        surface_distribution_input = h5file.create_dataset(
            "surface_distribution", data=np.zeros((num_elem,), dtype=int)
        )
        surface_m_input = h5file.create_dataset(
            "surface_m", data=np.ones((1,), dtype=int)
        )
...
cout.cout_wrap(
    'Reference for validation: "Rotary wing structural dynamics and aeroelasticity",'
    " R.L. Bielawa. AIAA education series. Second edition",
    1,
)
