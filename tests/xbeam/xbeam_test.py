...
class TestDynamicCoupledConfigurationXbeam(unittest.TestCase):
    """
    Tests the xbeam library for the dynamic multibeam example
    """

    @classmethod
    def setUpClass(cls):
        # run all the cases generators
        case = 'coupled_configuration_steady'
        mod = importlib.import_module('tests.xbeam.' + case + '.generate_' + case)
    ...
    def test_dynamicmultibeam(self):
        import sharpy.sharpy_main
        # suppress screen output
        # sharpy.sharpy_main.cout.cout_quiet()
        solver_path = os.path.abspath(os.path.dirname(os.path.realpath(__file__)) +
                                      '/coupled_configuration_steady/coupled_configuration_steady.solver.txt')
        sharpy.sharpy_main.main(['', solver_path])
        # sharpy.sharpy_main.cout.cout_talk()
