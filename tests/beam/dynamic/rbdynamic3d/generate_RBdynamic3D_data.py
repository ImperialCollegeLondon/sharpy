...
"""
3D rigid body motion beam test case
...
def generate_dyn_file():
    ...
    if with_dynamic_forces:
        ...
        f = 20
        mom2 = 100
        x_force = f*np.sin(angle)
        z_force = f*np.cos(angle)
        x_mom2 = -mom2*np.cos(angle)
        z_mom2 = -mom2*np.sin(angle)
        dynamic_forces = np.zeros((num_node, 6))
        dynamic_forces[0, 0] = -x_force
        dynamic_forces[0, 2] = z_force
        dynamic_forces[0, 4] = 200
        dynamic_forces[0, 3] = x_mom2
        dynamic_forces[0, 5] = z_mom2
        # dynamic_forces[num_node/2, 4] = 100
        force_time = np.zeros((num_steps, ))
        i_top = int(2.5/dt)
        i_bottom = int(5/dt)
        force_time[:i_top] = np.linspace(0, 1, i_top)
        force_time[i_top:i_bottom] = np.linspace(1, 0, i_bottom - i_top)
...
def generate_fem_file():
    ...
    global num_node_elem
    global num_node
    length = np.linalg.norm(np.array([6, 8, 0]))

    num_node = (num_node_elem - 1)*num_elem + 1
    # import pdb; pdb.set_trace()
    angle = np.arctan(8.0/6.0)
    x = -1*(np.linspace(0, length, num_node))*np.cos(angle)
    y = np.zeros((num_node,))
