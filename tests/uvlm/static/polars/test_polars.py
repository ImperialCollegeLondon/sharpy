...
class TestAirfoilPolars(unittest.TestCase):
    ...
    def test_infinite_wing(self):
        ...
        case_header = gw.get_case_header(
            polar=True,
            infinite_wing=True,
            compute_uind=False,
            high_re=False,
...
def process_case(path_to_case):
    ...
    inertial_forces = np.loadtxt(
        f"{path_to_case}/forces/forces_aeroforces.txt",
        skiprows=1,
        delimiter=",",
        dtype=float,
    )[1:4]
    inertial_moments = np.loadtxt(
        f"{path_to_case}/forces/moments_aeroforces.txt",
        skiprows=1,
        delimiter=",",
        dtype=float,
    )[1:4]
...
class TestStab(unittest.TestCase):
    def test_stability(self):
        ...
        with self.subTest(msg="Freestream test"):
            assert dir_urel[2] == np.sin(
                alpha
            ), "z component of freestream not properly rotated"
        with self.subTest(msg="Freestream test"):
            assert dir_urel[0] == np.cos(
                alpha
            ), "x component of freestream not properly rotated"
        ...
        for ax in [0, 2]:
            with self.subTest(msg="X_s", ax=ax):
                assert (
                    c_bs.dot(np.eye(3)[0])[ax] > 0
                ), f"{ax}_b component of X_s not correct"

        # Checking Z_s
        ax = 0
        with self.subTest(msg="Z_s", ax=ax):
            assert (
                c_bs.dot(np.eye(3)[2])[ax] < 0
            ), f"{ax}_b component of Z_s not correct"
