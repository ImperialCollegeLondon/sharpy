...
np.savetxt(
    "0p1_step_gust.txt", gust, header="Time[s] DeltaUx[m/s] DeltaUy[m/s] DeltaUz[m/s]"
)

# Analytical kussner function as a function of the non-dimensional time
kussner_function = np.array(
    [
        [2.50000000e-01, 2.24788834e-01],
        [5.00000000e-01, 3.08322030e-01],
        [7.50000000e-01, 3.69409233e-01],
        [1.00000000e00, 4.18591199e-01],
        [1.25000000e00, 4.59608283e-01],
        [1.50000000e00, 4.94597861e-01],
        [1.75000000e00, 5.24998850e-01],
        [2.00000000e00, 5.51822632e-01],
        [2.25000000e00, 5.75792734e-01],
        [2.50000000e00, 5.97434419e-01],
        [3.25000000e00, 6.51811966e-01],
        [4.00000000e00, 6.94721054e-01],
        [5.00000000e00, 7.39586015e-01],
        [7.50000000e00, 8.13230851e-01],
        [1.00000000e01, 8.56395643e-01],
        [1.25000000e01, 8.84426328e-01],
        [1.50000000e01, 9.04257362e-01],
        [1.75000000e01, 9.19155200e-01],
        [2.00000000e01, 9.30758783e-01],
        [0.00000000e00, 6.43293001e-02],
        [2.50000000e01, 9.47358507e-01],
        [3.00000000e01, 9.58126128e-01],
        [3.50000000e01, 9.65279097e-01],
        [4.00000000e01, 9.70275111e-01],
        [4.50000000e01, 9.74053377e-01],
        [5.00000000e01, 9.77137592e-01],
        [6.00000000e01, 9.81903866e-01],
        [7.00000000e01, 9.84820831e-01],
        [8.00000000e01, 9.86297902e-01],
        [9.00000000e01, 9.87557134e-01],
        [9.97500000e01, 9.89298872e-01],
    ]
)
...
class TestWakeCFLn1(unittest.TestCase):
    ...
    uinf_dir = np.array([1.0, 0.0, 0.0])  # Flow velocity direction
    air_density = 1.225  # Flow density

    aoa_ini_deg = 1.0  # Initial angle of attack

    wake_chords = 100  # Length of the wake in airfoil chords units

    # Time discretization
    final_ndtime = 12.0  # Final non-dimensional time
    time_steps_per_chord = (
        8  # Number of time steps required by the flow to cover the airfoil chord
    )
    ...
    def generate_geometry(self):
        ...
        len_first_panel = (self.chord - self.dt * self.uinf) / (
            self.num_chord_panels - 1
        )
        self.offset = self.offset_time_steps * self.dt * self.uinf + (
            -0.25 * self.chord + 0.75 * len_first_panel
        ) * np.cos(
            self.aoa_ini_deg * deg2rad
        )  # should be 0.5*len_first_panel but to make sure it happens I increase it a bit
        ...
        airfoil_camber[0, :, 0] = np.linspace(0.0, 1.0, 100)
        ...
        airfoil.StructuralInformation.generate_uniform_sym_beam(
            nodes,
        ...
        airfoil.StructuralInformation.structural_twist = (
            0.0 * deg2rad * np.ones_like(airfoil.StructuralInformation.structural_twist)
        )
        airfoil.StructuralInformation.boundary_conditions = np.zeros(
            (airfoil.StructuralInformation.num_node), dtype=int
        )
    ...
    def generate_files(self, case_header, airfoil):
        ...
        if case_header == "traditional" or case_header == "statespace_cfl1":
            ...
            SimInfo.solvers["AerogridLoader"]["wake_shape_generator_input"] = {
                ...
                "ndx1": int(
                    (self.wake_chords * self.chord)
                    / (self.chord / self.num_chord_panels)
                ),
                "r": 1.0,
                "dxmax": 10 * self.chord,
            }
            ...
            while length < (self.wake_chords * self.chord):
                mstar += 1
                length += hw.get_deltax(
                    mstar, wsg_in["dx1"], wsg_in["ndx1"], wsg_in["r"], wsg_in["dxmax"]
                )
        ...
        ] = SimInfo.solvers["RigidDynamicPrescribedStep"]
        ...
        if case_header in ["traditional", "new_nonlinear"]:
            ...
            SimInfo.solvers["StepLinearUVLM"][
                "velocity_field_generator"
            ] = "GustVelocityField"
            SimInfo.solvers["StepLinearUVLM"]["velocity_field_input"] = {
                "u_inf": self.uinf,
                "u_inf_direction": self.uinf_dir,
                "gust_shape": "time varying",
                "offset": self.offset,
        ...
        SimInfo.dynamic_forces = np.zeros(
            (time_steps, airfoil.StructuralInformation.num_node, 6), dtype=float
        )
        ...
        if "statespace" in case_header:
            ...
            SimInfo.solvers["Modal"] = {
                "save_data": "off",
                "NumLambda": 50,
                "rigid_body_modes": "off",
            }

            SimInfo.solvers["LinearAssembler"] = {
    ...
    def run_test(self, case_header):
        ...
        lift = sharpy_output.structure.timestep_info[-1].steady_applied_forces[
            self.nodes_AR // 2, 2
        ]
        lift += sharpy_output.structure.timestep_info[-1].unsteady_applied_forces[
            self.nodes_AR // 2, 2
        ]
        ...
        kussner_cl_clss += (
            ...
            * daoa
            * deg2rad
            * np.interp(nd_time, kussner_function[:, 0], kussner_function[:, 1])
        )
